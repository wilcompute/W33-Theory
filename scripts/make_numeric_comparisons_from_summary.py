...
possible = [ROOT / 'SUMMARY_RESULTS.json', ROOT / 'claude_workspace' / 'SUMMARY_RESULTS.json']
summary_path = next((p for p in possible if p.exists()), None)
if summary_path is None:
    raise FileNotFoundError("SUMMARY_RESULTS.json not found in repo root or claude_workspace")
    ...
    if isinstance(kr, dict):
        for key, val in kr.items():
            ...
            if 'desi' in k_lower or ('dark' in k_lower and 'energy' in k_lower):
                ...
                if m_key and p_key:
                    try:
                        m = float(dd[m_key]); p = float(dd[p_key])
                        res.append({'file': fname, 'name': 'DESI w0', 'measured': m, 'predicted': p, 'diff': p-m, 'pct': (abs(p-m)/abs(m))*100 if m!=0 else None})
                    except Exception:
                        pass

...
for sol in solutions:
    ...
    for idx, r in enumerate(E8):
        ...
        if all(recon[i] == r[i] for i in range(8)) and all(xi.denominator == 1 for xi in x):
            in_span.append(idx)
    results.append({'nodes': nodes, 'in_span_count': len(in_span), 'in_span_indices': in_span[:20]})

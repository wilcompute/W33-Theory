...
files = [repo / 'W33_FORMAL_THEORY.tex', repo / 'W33_TOE_COMPLETE_v31_EXECUTEDPATCH.tex']
...
for f in files:
    ...
    for m in re.finditer(r"\\begin\{(theorem|lemma|proposition|claim)\}(.{0,500}?)(?:\\end\{\1\})", txt, re.S):
        env, body = m.group(1), m.group(2)
        labels = re.findall(r"\\label\{([^}]+)\}", body)
        nums = re.findall(r"\b(\d{1,6}[,\d{0,3}]*)\b", body)
        claims.append({'file': str(f.relative_to(repo)), 'env': env, 'labels': labels, 'numbers': nums, 'excerpt': body.strip()[:200]})
    # TODO/NB comments
    for m in re.finditer(r"%+\s*(TODO|NOTE|HINT|FIXME)[:\-]?\s*(.*)", txt):
        claims.append({'file': str(f.relative_to(repo)), 'todo_type': m.group(1), 'message': m.group(2).strip()})

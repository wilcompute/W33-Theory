...
tri=repo/'bundles'/'v23_toe_finish'/'v23'/'Q_triangles_with_centers_Z2_S3_fiber6.csv'
triangles=list(load_triangles(tri))
neigh=neighborhoods_from_triangles(triangles)
gens_map=point_hyperplanes(neigh)
points_sorted = sorted(set().union(*(s for _, s in sorted(gens_map.items(), key=lambda x:x[0]))))

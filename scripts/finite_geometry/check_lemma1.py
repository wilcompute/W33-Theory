...
def main():
    ...
    non_neighbors = [ [j for j in range(n) if j != i and adj[i][j]==0] for i in range(n)]
    ...
    summary = {
        'n_vertices': n,
        'degree_distribution': {'min': min(degrees), 'max': max(degrees), 'deg_list': degrees},
        'degree_ok': degree_ok,
        'non_neighbor_counts_example': {'vertex_0_count': nn_counts[0], 'all_counts': nn_counts[:8]},
        'non_neighbor_count_ok': nn_count_ok,
        'triangles_in_non_neighbors_stats': {'min': tri_min, 'max': tri_max, 'counts_sample': tri_counts[:8]},
        'tri_ok': tri_ok,

...
tomotope_summary = repo / 'data' / 'maniplex_tables' / 'tomotope_permutation_summary.json'
...
with repetend_csv.open('r', encoding='utf-8') as f:
    ...
    for row in r:
        ...
        rows.append({'d':d, 'repetend':repetend, 'mod12':int(row['mod12']), 'mod24':int(row['mod24']), 'mod192':int(row['mod192']), 'full_repetend':bool(int(row['full_repetend']))})

# load tomotope info
try:
    tom = json.loads(tomotope_summary.read_text(encoding='utf-8'))
    tom_group_size = tom.get('group_size')
    tom_orbit_sizes = {int(k): len(v) for k,v in tom.get('orbits', {}).items()} if 'orbits' in tom else {}
except Exception:
    tom = {}
    tom_group_size = None
    tom_orbit_sizes = {}
...
for res in residues:
    ...
    summary['by_residue'][res] = {'n': n, 'div12': int(div12), 'eq6': int(eq6), 'fulls': int(fulls), 'fraction_div12': div12/n, 'fraction_eq6': eq6/n, 'fraction_full': fulls/n}
...
# Save summary
out = {'tomotope_group_size': tom_group_size, 'by_residue': summary['by_residue'], 'fisher_div12': fisher_results}

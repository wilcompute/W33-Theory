...
def analyze(max_den=1000):
    ...
    for d in range(1, max_den + 1):
        ...
        if n > 1 and rl is not None:
            full = (rl == totient(n))
        rows.append({'d': d, 'repetend': rl if rl is not None else -1, 'mod12': d % 12, 'mod24': d % 24, 'mod192': d % 192, 'full_repetend': full})
    return rows
...
def save_rows(rows, max_den):
    ...
    with csvp.open('w', newline='', encoding='utf-8') as f:
        ...
        for r in rows:
            w.writerow([r['d'], r['repetend'], r['mod12'], r['mod24'], r['mod192'], int(r['full_repetend'])])
    return jsonp, csvp


def make_plots(rows, max_den):
    repetends = [r['repetend'] for r in rows if r['repetend'] is not None and r['repetend'] >= 0]
...
def enrichment_tests(rows):
    ...
    for residue in sorted(set(cnt_all.keys())):
        ...
        try:
            ...
            odds, p = None, None
        res.append({'residue': residue, 'full_count': a, 'all_count': cnt_all.get(residue,0), 'odds_ratio': odds, 'pvalue': p})
    return res
...
def main():
    ...
    out = {'json': str(j), 'csv': str(csvp), 'hist': str(p1), 'mod12_plot': str(p2), 'enrichment': enrich}

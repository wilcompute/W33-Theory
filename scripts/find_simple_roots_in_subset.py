...
for trial in range(20):
    ...
    if len(indecomp) == 6:
        ...
        for i in range(6):
            for j in range(6):
                M[i][j] = 2 * sum(indecomp[i][k]*indecomp[j][k] for k in range(8)) / sum(indecomp[i][k]*indecomp[i][k] for k in range(8))
        results.append({'trial': trial, 'coeffs': coeffs, 'indecomp_cartan': [[float(x) for x in row] for row in M]})
        print(f'Trial {trial}: Found indecomp=6, Cartan matrix:')
        for row in M:
            print(row)
    else:
        print(f'Trial {trial}: indecomp={len(indecomp)}')
Path('PART_CVII_e6_indec_simple_trials.json').write_text(json.dumps(results, indent=2))

            ...
            for d in neighs_list:
                ...
                for e in cand4:
                    ...
                    for f in neighbor_list[e]:
                        ...
                        for i in range(6):
                            for j in range(6):
                                M[i,j] = 2 * DP[idxs[i], idxs[j]] / DP[idxs[i], idxs[i]]
                        ...
                        for i in range(6):
                            for j in range(6):
                                if i==j:
                                    ...
                                    expected = -1.0 if (min(i,j), max(i,j)) in E6_edges else 0.0
                                    if not np.isclose(M[i,j], expected):
                                        ok = False
                        if ok:
                            sol = {'center': int(c), 'nodes':[int(x) for x in idxs]}
                            # deduplicate by nodes set
                            nodeset = set(sol['nodes'])
                            if not any(set(s['nodes'])==nodeset for s in solutions):
                                solutions.append(sol)
                                print('Found solution:', sol)
                                Path('PART_CVII_e6_in_e8_backtrack.json').write_text(json.dumps(solutions, indent=2))

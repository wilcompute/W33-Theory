...
summary_keys = ['key_finding', 'key_insight', 'key_finding', 'key_result', 'key_results', 'title', 'summary']
...
for jf in json_files:
    ...
    try:
        ...
        part_number = data.get('part_number') if data.get('part_number') is not None else None
            ...
            if isinstance(p, int):
                ...
                try:
                    ...
                    def roman_to_int(s: str) -> int:
                        ...
                        total = 0
                        while i < len(s):
                            if i+1 < len(s) and roman_map.get(s[i]) < roman_map.get(s[i+1], 0):
        ...
        def scan_for_pairs(d):
            ...
            keys = set(d.keys())
            measured_keys = [k for k in keys if 'meas' in k.lower() or 'observ' in k.lower() or 'experimental' in k.lower()]
            pred_keys = [k for k in keys if 'pred' in k.lower() or 'w33' in k.lower() or 'predicted' in k.lower() or 'value' in k.lower()]
            for mk in measured_keys:
                for pk in pred_keys:
                    try:
                        ...
                        comparisons.append({'measured_key': mk, 'predicted_key': pk, 'measured': m, 'predicted': p, 'difference': diff, 'percent_diff': pct})
                    except Exception:
                        continue
        scan_for_pairs(data)
        if comparisons:
        ...
        try:
            ...
            if isinstance(kr, dict) and 'desi_dark_energy' in kr:
                ...
                if 'w0_measured' in dd and 'w0_w33_predicted' in dd:
                    ...
                    diff = p - m
                    pct = (abs(diff) / abs(m)) * 100 if m != 0 else None
                    entry.setdefault('comparisons', []).append({'name': 'DESI w0', 'measured': m, 'predicted': p, 'difference': diff, 'percent_diff': pct})
        except Exception:
            pass

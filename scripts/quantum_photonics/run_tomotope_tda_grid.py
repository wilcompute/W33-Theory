...
for modes in range(4,9):
    ...
    for kind, subset in [('structured', structured), ('random', random_subset)]:
        ...
        results.append({'modes': modes, 'kind': kind, 'shots': shots, 'js_w_sf': float(js), 'mean_h1_walrus': mw, 'mean_h1_sf': ms, 'walrus_samples': sum(walrus_counts.values()), 'sf_samples': sum(sf_counts.values())})

...
for rate in mutation_rates:
    ...
    results.append({'rate': float(rate), 'mean_kmer_top5': top5, 'h1_features': int(len(dgms[1]))})
...
for i in range(len(mutation_rates)-1):
    ...
    m = 0.5*(mk_a + mk_b)
    js_val = 0.5*(entropy(np.maximum(mk_a,1e-12), np.maximum(m,1e-12)) + entropy(np.maximum(mk_b,1e-12), np.maximum(m,1e-12)))

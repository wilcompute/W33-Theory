...
try:
    ...
    sf_samples = sample_gbs(modes=mode, squeezing=0.6, U=U, backend='gaussian', shots=shots)
...
m = 0.5*(p_w + p_s)
js = 0.5*(entropy(np.maximum(p_w,1e-12),np.maximum(m,1e-12)) + entropy(np.maximum(p_s,1e-12),np.maximum(m,1e-12)))
...
for b in range(min(bootstrap, 500)):
    ...
    p_w_b = draw / shots
    m_b = 0.5*(p_w_b + p_s)
    js_b = 0.5*(entropy(np.maximum(p_w_b,1e-12),np.maximum(m_b,1e-12)) + entropy(np.maximum(p_s,1e-12),np.maximum(m_b,1e-12)))
    boot_js.append(js_b)
...
out_dir.joinpath('tomotope_tda_followup_summary.json').write_text(json.dumps(summary, indent=2))

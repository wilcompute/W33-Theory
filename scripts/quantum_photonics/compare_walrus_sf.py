...
for modes in modes_list:
    ...
    try:
        sf_samples = sample_gbs(modes=modes, squeezing=0.6, U=U, backend='gaussian', shots=shots)
    ...
    mw = sum(sum(p)*c for p,c in walrus_counts.items())/max(1,sum(walrus_counts.values()))
    ms = sum(sum(p)*c for p,c in sf_counts.items())/max(1,sum(sf_counts.values()))

    results.append({'modes': modes, 'shots': shots, 'js': float(js), 'mean_clicks_walrus': float(mw), 'mean_clicks_sf': float(ms)})

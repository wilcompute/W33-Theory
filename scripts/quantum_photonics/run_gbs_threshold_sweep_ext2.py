...
out_json = repo / 'bundles' / 'v23_toe_finish' / 'v23' / 'gbs_threshold_sweep_ext_adaptive.json'
out_png = repo / 'bundles' / 'v23_toe_finish' / 'v23' / 'gbs_threshold_js_vs_eta_ext_adaptive.png'
out_pdf = repo / 'bundles' / 'v23_toe_finish' / 'v23' / 'gbs_threshold_summary_adaptive.pdf'
...
try:
    for squeezing in squeezings:
        for modes in modes_list:
            ...
            for eta in etas:
                ...
                results.append({'modes': modes, 'squeezing': squeezing, 'eta': eta, 'shots': shots, 'js': float(js), 'top_patterns': [(list(k), int(v), v/shots) for k,v in topk]})
    ...
    if len(results) > 0:
        ...
        fig, axs = plt.subplots(len(squeezings_present), 1, figsize=(6, 3*len(squeezings_present)), sharex=True)
        if len(squeezings_present) == 1:
            axs = [axs]
        for i, squeezing in enumerate(squeezings_present):
            ax = axs[i]
            modes_list_present = sorted(list({r['modes'] for r in results if r['squeezing']==squeezing}))
            for modes in modes_list_present:
                xs = [r['eta'] for r in results if r['modes']==modes and r['squeezing']==squeezing]
                ys = [r['js'] for r in results if r['modes']==modes and r['squeezing']==squeezing]

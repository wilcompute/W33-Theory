...
for modes in modes_list:
    for r in squeezings:
        ...
        samples_fock = sample_gbs(modes, r, U, backend='fock', shots=shots, cutoff=cutoff)
        ...
        results.append({'modes': modes, 'squeezing': r, 'shots': shots, 'kl_gauss': kl_gauss, 'kl_fock': kl_fock})
        print('modes', modes, 'r', r, 'kl_gauss', kl_gauss, 'kl_fock', kl_fock)

open('bundles/v23_toe_finish/v23/gbs_sweep_results.json','w').write(json.dumps(results, indent=2))

...
| Symplectic spread / MUB frames | [`scripts/w33_symplectic_spread_frame_audit.py`](../scripts/w33_symplectic_spread_frame_audit.py) | Consolidated executable spread audit showing that the exact kernel has `36` spreads, each giving a full `10`-basis two-qutrit stabilizer MUB frame and decomposing as one anchor line plus `9` affine measurement lines |
| Holonet wrapper | [`analysis/holonet_wrap.py`](../analysis/holonet_wrap.py) | Wraps arbitrary classical commands into the W(3,3) packet envelope and compiles the resulting control DAG under `--optimize active-ticks` or `--optimize clock-slots`; the Rule-110 demo shows `8/13` active-policy ticks/slots versus `7/7` clock-policy ticks/slots |
| VM demo launcher | [`analysis/holonet_vm_demo_launcher.py`](../analysis/holonet_vm_demo_launcher.py) | One-button VM demo that runs the interface proof, substrate stub, side-channel suite, and both wrapped Rule-110 scheduler policies |
| Spread/contextual microkernel bridge | [`analysis/w33_spread_contextual_microkernel_bridge.py`](../analysis/w33_spread_contextual_microkernel_bridge.py) | Executable Holonet bridge showing that `36` spreads equal the KS classical ceiling `36`, the contextual deficit is `4 = mu`, all 40 point-stars realize the deficit as a double-occupancy defect, and the current wrapper demo refines from 33 packets into 56 ordered hop-line operations scheduled by a 14-tick line-context microkernel |
| Line-context compiler | [`analysis/w33_line_context_compiler.py`](../analysis/w33_line_context_compiler.py) | Compiler from wrapped Holonet packets to packet DAG, hop-line DAG, and verified spread-clock microcode; project-venv SciPy certifies the 14-active-tick optimum for the current demo and the compiler also emits a 15-slot clock-native schedule |
| Defect/spread tensor | [`analysis/w33_defect_spread_tensor.py`](../analysis/w33_defect_spread_tensor.py) | Colored 40-by-36 defect/spread bus with 1440 line-clock slots; row equalities reconstruct `SRG(40,12,2,4)`, while the 36 spread frames have 1-or-4 line overlaps and a 15-regular 4-overlap spectrum `{15, 3^15, -3^20}` |
| Spread clock graph | [`analysis/w33_spread_clock_graph.py`](../analysis/w33_spread_clock_graph.py) | Executable 36-frame clock primitive: spread 4-overlap is `SRG(36,15,6,6)` with diameter 2; the 8-tick site OS embeds in 10 clock slots, the active-optimal 14-tick microkernel in 22 slots, and the clock-native schedule in 15 slots |
| Clock policy stress | [`analysis/w33_clock_policy_stress.py`](../analysis/w33_clock_policy_stress.py) | Scales the wrapped UOR packet DAG by `1x..6x`; the certified anchor is 14 active ticks / 22 clock slots versus 15 clock-native ticks / 15 slots, and the scaled deterministic rows stay connector-free while saving 8, 15, 21, 25, 25, and 38 elapsed frame-clock slots |
| Packet latency benchmark | [`analysis/w33_packet_latency_benchmark.py`](../analysis/w33_packet_latency_benchmark.py) | Converts schedules into per-packet completion times; the certified one-copy row has clock-native completing `28/33` packets earlier, and deterministic `1x..4x` rows complete `23,52,80,106` packets earlier |
| Witting deck / spread-control layer | [`scripts/w33_witting_deck_control_audit.py`](../scripts/w33_witting_deck_control_audit.py) | Consolidated executable audit showing that the Witting paper's `40` quantum cards become `36` exact full decks on the same `W(3,3)` kernel, with rigid `1/4` deck overlaps and derived `216` five-deck sweeps / `135` four-deck control packets |
| Balanced packet shell bridge | [`scripts/w33_witting_balanced_packet_shell_audit.py`](../scripts/w33_witting_balanced_packet_shell_audit.py) | Consolidated executable audit showing that the `27` balanced four-deck packets reconstruct the local `H27` shell and, via exact rank-intersection relations, recover both the Schläfli graph and its intersection complement |
| Packet Heisenberg chart | [`scripts/w33_witting_packet_heisenberg_chart_audit.py`](../scripts/w33_witting_packet_heisenberg_chart_audit.py) | Consolidated executable audit showing that the `27` balanced packets split canonically as a `9 × 3` fiber chart, with affine `F_3^2` triangle coordinates and a fiber-preserving isomorphism to the canonical local `H27` Heisenberg shell |
| Packet tritangent support bridge | [`scripts/w33_witting_packet_tritangent_support_audit.py`](../scripts/w33_witting_packet_tritangent_support_audit.py) | Consolidated executable audit showing that the `27` balanced packets already carry the full local `45 = 36 + 9` tritangent-support package, with each packet on `5` support triples and the same `45` triangles reappearing in the rank-intersection-`16` graph |
| Packet Hessian split bridge | [`scripts/w33_witting_packet_hessian_split_audit.py`](../scripts/w33_witting_packet_hessian_split_audit.py) | Consolidated executable audit showing that the `36` non-fiber packet triples already realize the full Hessian split as `12 × 3` affine lifts in `AG(2,3)`, grouped into `4` direction classes, with each packet on one affine lift per direction plus its unique fiber |
| Packet foliation / Pappus bridge | [`scripts/w33_witting_packet_foliation_incidence_audit.py`](../scripts/w33_witting_packet_foliation_incidence_audit.py) | Consolidated executable audit showing that the packet layer carries `5` canonical triple foliations, with every fiber-affine leaf-incidence graph equal to `3K_{3,3}` and every affine-affine leaf-incidence graph equal to the Pappus graph |
| Packet quotient-geometry bridge | [`scripts/w33_witting_packet_quotient_geometry_audit.py`](../scripts/w33_witting_packet_quotient_geometry_audit.py) | Consolidated executable audit showing that the `45` packet leaves are the exact quotient points and the `27` packet-induced `K5` cliques are the exact quotient lines of the dual `GQ(4,2)` geometry, recovering the exceptional `SRG(45,12,3,3)` and `SRG(27,10,1,5)` package from the Witting side alone |
| Packet transport-complement bridge | [`scripts/w33_witting_packet_transport_complement_audit.py`](../scripts/w33_witting_packet_transport_complement_audit.py) | Consolidated executable audit showing that the same `45` packet leaves already carry the exact transport complement `SRG(45,32,22,24)`, with the `27` packet lines becoming exact `5`-cocliques and every transport edge inducing a unique local `S3` packet-line matching |
| Packet transport local-system bridge | [`scripts/w33_witting_packet_transport_local_system_audit.py`](../scripts/w33_witting_packet_transport_local_system_audit.py) | Consolidated executable audit showing that the packet-side `S3` matchings already recover the exact `135 = 45 + 90` transport connection bundle, the signed holonomy operator, and the native `A2` standard sector, while raw six-permutation edge counts remain local-label dependent |
| Packet transport path-groupoid bridge | [`scripts/w33_witting_packet_transport_path_groupoid_audit.py`](../scripts/w33_witting_packet_transport_path_groupoid_audit.py) | Consolidated executable audit showing that the packet-side `A2` data already defines the exact transport path-groupoid representation, with no real flat section but a unique invariant line `[1,2]` and exact binary shadow `{1,2}` over `F3` |
| Packet transport ternary-line bridge | [`scripts/w33_witting_packet_transport_ternary_line_audit.py`](../scripts/w33_witting_packet_transport_ternary_line_audit.py) | Consolidated executable audit showing that the packet-side invariant line selects a canonical `81`-dimensional qutrit matter sector, and the full reduced `A2` fiber gives `162`, matching the flat internal dimension exactly |
| Packet transport ternary-extension bridge | [`scripts/w33_witting_packet_transport_ternary_extension_audit.py`](../scripts/w33_witting_packet_transport_ternary_extension_audit.py) | Consolidated executable audit showing that the packet-side reduced transport fiber is already the exact non-split extension `0 -> 1 -> rho -> sgn -> 0`, which tensors with the `81`-qutrit logical sector to give the exact `81 -> 162 -> 81` matter-flavour bridge |
| Packet transport ternary-cocycle bridge | [`scripts/w33_witting_packet_transport_ternary_cocycle_audit.py`](../scripts/w33_witting_packet_transport_ternary_cocycle_audit.py) | Consolidated executable audit showing that the packet-side non-split extension is explicit as a genuine twisted cocycle and a square-zero rank-`81` nilpotent operator on the `162`-sector |
...
| Exact `q=3` master lock | [`scripts/w33_q3_master_lock_audit.py`](../scripts/w33_q3_master_lock_audit.py) | Consolidated executable `q=3` overdetermination audit across the local kernel, the Parseval target-side geometry and Naimark shadow, the corrected spectral/Ihara layer, the continuum and electron seed packets, and the transport holonomy reduction |
...
| Standard Model minimal magic | [`scripts/w33_standard_model_minimal_magic_audit.py`](../scripts/w33_standard_model_minimal_magic_audit.py) | Atomicity refinement of the non-Clifford frontier: two independent quartic atoms, with any canonical mixing jumping to octic or degree-16 synthesis |
| Temporal / spectral toroidal computer | [docs/temporal_spectral_toroidal_computer.html](./temporal_spectral_toroidal_computer.html) | Public architecture note for the conservative photonic realization: one photon with temporal and synthetic-frequency qutrits on a discrete `3 x 3` torus, exact Clifford/measurement layers, and a two-quartic nonlinear frontier |

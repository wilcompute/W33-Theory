...
\begin{result}[The tax, fully solved --- and it is the interrupt controller]
The classical layer's contextuality deficit is completely structured: every optimal Kochen--Specker
assignment fails exactly one \emph{movable point-star} (exhaustive enumeration; the deficit is $q+1$
for odd $q$, zero for even); every spread-clock frame runs at exactly $9/10$ under any optimum; the
$800$ optima form four $\mathrm{PSp}(4,3)$ orbits ($360+40+360+40$) whose special members are the
classical \emph{perp states} $\Gamma(p)$ (the defining GQ axiom is their optimality proof); and the
$q^2$ ground states per node carry the affine plane $\mathrm{AG}(2,q)$ --- at $q=3$ the Hesse
configuration, the single-qutrit Wigner phase space, with the four defect contexts as its striations.
Operationally this \emph{is} the machine's interrupt architecture: the vector table is the nine-entry
closed form $\mathrm{ground}(T)=T\cup(\Gamma(p)\setminus T^\perp)$; the escalation budget (where the
priced $9^t$ resource is spent) is one star $=1/10$ of the fabric; and the migration price law makes
the defect cheapest to move along fabric edges ($3$ rays, via the ground's own center quad) --- cheaper
than re-vectoring in place ($6$).  A seeded controller run holds every theorem as a live invariant.
Witnesses: \path{w33_contextuality_tax}, \path{w33_spread_star_anatomy}, \path{w33_tax_orbits},
\path{w33_perp_states}, \path{w33_ground_affine_plane}, \path{w33_interrupt_controller}.
\end{result}

\begin{result}[Compiled defect runtime stack]
The movable defect has a compiled runtime law.  For each active center, the $27$ safe non-neighbours
split into nine Hesse/affine-plane phase rows, and each row exposes four cheap apertures, one on each
star striation.  Thus
\[
40\cdot9\cdot4=1440=3\cdot480,
\]
so every directed fabric edge has exactly three compiled phase rows.  The packet kernel now uses the
selector
\[
\phi=\mathrm{table}[(p,q)][c(p,q)\bmod 3]
\]
for relocation phase accounting.  Repeated use of a fixed edge is minimax-balanced: the maximum row reuse
over a run of length $L$ is the pigeonhole optimum $\lceil L/3\rceil$.  The page side has a constant
nine-point churn for every relocation, but edge moves preserve phase locality: six phase triples survive
intact and three rebuild whole, while non-edge moves rebuild one point inside each of the nine triples.
This is the direct runtime chain
\[
TD(4,3)\to 3\hbox{-slot scheduler}\to \lceil L/3\rceil\hbox{ run law}\to 9\hbox{-point cache law}.
\]
Witnesses: \path{bt1807_defect_phase_plane_transversal_design}, \path{bt1808_td43_edge_scheduler},
\path{bt1818_compiled_packet_kernel_controller}, \path{w33_packet_vm_kernel},
\path{bt1824_executable_packet_replay}, \path{bt1825_aperture_shot_table_exporter}.
\end{result}
...
the aperture shot table is a readout scaffold, not data; the quantum advantage is the priced $9^t$ dial,
run here only for small $t$. ``Architecture of life'' means the von~Neumann architecture of
self-reproduction (the rigorous computational notion), not biology.
\end{honestbox}
...
\path{holonet_ks_experiment.py}, \path{w33_architecture_capstone.py},
\path{w33_packet_vm_kernel.py}, \path{bt1824_executable_packet_replay.py},
\path{bt1825_aperture_shot_table_exporter.py}. \textbf{Standard results:}

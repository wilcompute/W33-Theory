...
% ======================================================================
\input{manuscripts/tex/part00_the_layer_map}
...
\headline{A frame forgets its origin by $1/e$ in \textbf{$9.4$ instructions} and is
statistically gone after \textbf{$15$} --- a count fixed at L1 by the geometry, on every
realization. Only its conversion to time is part-specific: $71.8$\,ns at the measured
$208.86$\,MHz on the iCE40, and something else on anything else.}
...
every access and refuses the ones that cross a boundary. The finite instruction algebra
below supplies a useful noninterference invariant for a restricted trace, but it does not
remove the need to enforce which trace actually runs.
\end{plain}
...
\smallskip\noindent
\textbf{Within this four-opcode model, a verified trace containing no $\mathrm{CX}$ has
no direct instruction that couples the two halves.} This is an algebraic property of that
selected trace, not a complete isolation boundary. A real hypervisor must still prevent
unverified decode or control paths from issuing $\mathrm{CX}$ and must cover memory,
devices, scheduling, malformed guests, and side channels. The algebra makes one policy
easy to state and test; it does not replace its enforcement.
\end{spec}
...
digest path. At six levels the uniform graph denotes $105{,}025{,}641$ internal network VMs
plus $4{,}096{,}000{,}000$ addressable leaf VMs---$4{,}201{,}025{,}641$ stateful VMs in
all---with seven uniform state blobs. In the certified fresh transition a depth-six send
creates seven path-state blobs plus one receipt and execution creates seven path-state blobs;
content-identical replay may allocate none. An independent GAP witness proves the logical
W33 line-route bound $2n$.
...
The engineering consequence is narrower and still useful. At the abstract $40$-symbol
layer, a channel fault constrained to substitute one valid symbol for another never leaves
the address alphabet: it becomes a legal packet addressed somewhere else. That does
\emph{not} hold for an arbitrary corrupted bit or byte stream. The eight-bit packing below
uses only $40$ of $256$ patterns, so framing and decode must reject the other $216$, just as
the executable HoloBox ISA rejects an unknown opcode. Geometry removes holes from the ideal
$40$-ary substitution model; it does not eliminate the decoder's error state.
...
\begin{gotwrong}[What this does \emph{not} mean]
An earlier draft of this section said the closure ``sharpens Pass 2836's \emph{compute is
exactly zero}: that holds of the reversible closure, and the shipped four-opcode ISA does
not have it.'' \textbf{That is wrong, and it inverts the actual situation.}

Every opcode of every machine here is a bijection on the $81$ frames. A program is a
composition of bijections, so it erases nothing and its Landauer cost is zero --- on the
four-opcode machine exactly as much as on the closure. Checked directly: a
$200$-instruction random program maps the $81$ states onto $81$ states on both machines.
\emph{Compute is exactly zero on all four designs}, and the closure buys none of it.
\end{gotwrong}

\begin{plain}
So what does the closure buy? The entropy production above is a property of a different
object: the Markov chain you get by driving the machine with \emph{uniformly random}
instructions. That chain fails detailed balance on the four-opcode machine and satisfies it
on the closure.

It is a real difference and a useful one, but it is about the instruction set as a source of
randomness, not about running a program. Read correctly it says: watch a random instruction
stream on machine A and you can tell which way time runs; watch one on machine D and you
cannot. For a scrambler, a physical randomness source, or anything whose security rests on
trajectory indistinguishability, that is the whole point. For a machine executing a program,
it is not a dissipation at all.

The honest form of the trade is therefore narrower than ``buy reversibility'': you are
buying \textbf{time-symmetry of the random-instruction walk}, and \S\ref{sec:budget} prices
it, relative to the four-opcode machine's $103$ cells, at $2.00\times$ the cells.
\end{plain}
...
\end{tabular}
\end{center}
\textbf{Measured against machine A} ($103$ cells), time-symmetry of the random-instruction
walk costs $2.00\times$, removing the $p/f$ bias costs $1.28\times$, and both together cost
$2.33\times$. (Not ``thermodynamic reversibility'' --- see the correction above: every
machine here computes at zero Landauer cost already.)
...
\item \textbf{The universality gap is now a single, well-posed question.} It has narrowed
      ...
      \end{itemize}
      \emph{Update (Pass 20260906):} the asked-for exhaustive search over a chosen family
      has now run: all $961$ copy-permutation-symmetric stabilizer groups ($27{,}391$
      signed projectors, exact integer arithmetic, independently dense-confirmed).
      \textbf{Acceptance witnesses exist} --- $104$ across the $36$ rays --- but
      every returned hit has six independent generators on six qubits. Its
      projector therefore has rank one and leaves zero logical qubits: the
      accepted output is a stabilizer state, not a distilled magic ray.
      This is the same obstruction already recorded in the three-copy section
      and Passes 2933, 2977 and 2990. The follow-up
      \texttt{w33\_three\_copy\_output\_rank\_audit.py} checks every hit.
      \emph{Useful super-linear distillation remains open.} Nonzero acceptance
      of the clean input does not establish preservation of its magic at the
      output. The enumeration adds a symmetric-family result, not a useful
      protocol awaiting only a decoder. Asymmetric protocols and other
      correction conditions are not excluded; injection remains refused.
...
% ---------------------------------------------------------------------------
\appendix
\input{analysis/W33_BLUEPRINT_RECOVERED_APPENDIX}%
\end{document}

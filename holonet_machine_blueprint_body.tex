...
\item \textbf{The universality gap is now a single, well-posed question.} It has narrowed
      ...
      \textbf{Witnesses exist} --- $104$ across the $36$ rays.  The condition IS
      satisfiable at three copies; the open problem is now the decoder, the yield, and
      the acceptance inequality, not existence.  The controller still refuses injection
      until those are certified.
      \emph{The question as originally posed} --- does any stabilizer protocol on three
      or more copies annihilate every single-error input while keeping the clean one ---
      is therefore answered in the affirmative for the maximal symmetry-respecting
      family.  What remains open is everything downstream of existence: an explicit
      decoder, the distillation yield of the witnessed branch, and whether the
      acceptance inequality is met at the output.  Until those are answered the
      controller still refuses injection.

...
\emph{this} graph is the exceptional one. What distinguishes $W(3,3)$ among the $28$ is its
automorphism group, which the zeta never sees.
\end{spec}
...
It also sharpens \S\ref{sec:budget}'s comparison, though not in the direction one first
...
% =====================================================================================
\section{Every bit in the machine}\label{sec:budget}
% =====================================================================================
...
claimed $79$ --- and $78 = \dim E_6$ is this project's standing count, so the numerator was
right and the denominator was wrong. Corrected, $W(3,3)$ gives $78/78$ and the Levi graph
$156/156$, both satisfying the graph RH, and the instruction graph $0/160$, failing it.
\textbf{Calibrate every layer against a known answer, not merely the bottom one.}
...
\begin{spec}[Pass 4246 --- the deficit is a constant, not a quirk]
Across all $24$ minimum-size universal generating sets, the minimum stir --- the fraction of
frames on which \emph{some} opcode moves the worst-served coordinate --- is
\[
  \tfrac{2}{3}\ \text{ exactly, in all } 24 .
\]
Not ``most'', not ``on average''. Four generators cannot fully stir four coordinates of this
register, and the deficit is identical every time. The frozen coordinate is a property of
the number four, not of the choice.
\end{spec}
...
\begin{spec}[Pass 4242 --- the instruction stream has an entropy, and it meets Landauer]
$\log_2 \rho(B) = 2.5228$ bits of trajectory entropy per instruction --- more than the
$\log_2 4 = 2$ bits the opcode count suggests, because the graph is undirected and a
non-backtracking walk chooses among up to seven continuations, not four. The machine's
trajectory space is richer than its instruction encoding.

Against Pass 2836's exact $8/3$ bits erased per support readout, a readout costs the
entropy of $1.06$ instructions. That is a thermodynamic cadence bound --- and it is
\emph{weaker} than Pass 2867's mixing bound of $15$ instructions. \textbf{The machine is
limited by how fast it forgets, not by how fast it generates.} Both numbers are exact, and
they were computed six hundred passes apart from unrelated objects.
\end{spec}
...
\begin{tabular}{@{}lrrr@{}}
\toprule
design & opcodes & cells & wires\\
\midrule
four-opcode ISA & $4$ & $103$ & $137$\\
reversible closure & $8$ & $201$ & $269$\\
\bottomrule
\end{tabular}
\end{center}
\textbf{Thermodynamic reversibility costs $1.95\times$ the cells.} The datapath is shared;
only the decode widens.

The RTL was verified before the count was believed: a $40$-instruction random program run
under \texttt{iverilog} ends at frame $(2,2,1,0)$, exactly matching the group computation in
Python. The simulation also reproduces Pass 4204 independently --- from the origin, all
three linear opcodes leave the register at $(0,0,0,0)$ and only the load port moves it.

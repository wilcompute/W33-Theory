...
direction} they move it in, and one direction is nearly frozen. The prescription that
follows is correspondingly different from the one we expected: what the design needs is an
opcode that moves $x_2$ unconditionally --- the shear $S_f$ does exactly that --- rather
than a second load port. Pass 4225 showed one load port suffices for \emph{reachability};
this shows reachability was never the constraint on mixing.
\end{plain}
...
than derived from a presentation; and every one of them is a peephole optimisation, since
any occurrence in a stream is dead code by construction.
\end{spec}

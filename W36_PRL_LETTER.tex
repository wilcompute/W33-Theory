...
$\{k,r,s\}=\{12,2,-4\}$ with multiplicities $\{1,27,12\}$.
From these six integers we obtain:
...
The multiplicities satisfy $1 + 27 + 12 = 40 = n_v$ (spectral
completeness).  The three multiplicities are not coincidental:
$f_r = 27 = \dim(\Esix)$ in the fundamental representation, and
$f_s = 12$ equals the number of SM gauge bosons
($1_\gamma + 3_{W/Z} + 8_g$).  Figure~\ref{fig:spectral} illustrates
the eigenvalue tower.

\begin{figure}[h]
  \centering
  \includesvg[width=\columnwidth]{figures/fig1_spectral_diagram}
  \caption{\textbf{W(3,3) eigenvalue tower.}
  Three spectral levels at $\lambda=12$, $r=2$, $s=-4$ with
  multiplicities $\{1,27,12\}$. Spectral gaps $\Delta_1=10$,
  $\Delta_2=6$.  The master scale $E=480=|\Phi(E_8)|$ is indicated.}
  \label{fig:spectral}
\end{figure}
...
$f_r$ (matter)      & $27$           & $\dim(E_6)=27$    & exact \\
$f_s$ (gauge)       & $12$           & SM gauge bosons   & exact \\
\bottomrule
\end{tabular}
\end{table}
...
three spectral sectors $\{f_k, f_r, f_s\} = \{1, 27, 12\}$.
...
\begin{figure}[h]
  \centering
  \includesvg[width=\columnwidth]{figures/fig2_predictions_timeline}

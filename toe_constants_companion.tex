...
The dimensionless predictions are the core claims of this companion.
The newer measured/SI constant arithmetic is included only as a
\emph{unit-gauge witness} layer: it records exact substrate factorizations
of conventional or rounded decimal mantissas after a unit convention is
chosen, but it is not promoted as a unit-independent physical prediction.
...
\begin{theorem}[Electroweak mass tower]\label{thm:ew_mass_tower}
...
\section{Unit-Gauge Witness Layer}\label{sec:unit_gauge_witness_layer}

The constants above are dimensionless predictions or dimensionless mass
ratios.  MCCCLXXXIV--MCCCLXXXV add a different, stricter layer:
dimensionful SI or conventional constants may have substrate-clean
\emph{unit-scaled display integers}, but those integers depend on the
chosen unit gauge.  They are evidence of compatibility with the SI/CODATA
metrology layer, not replacements for dimensionless predictions.

\begin{theorem}[Measured and derived constants as unit-gauge witnesses]\label{thm:unit_gauge_witnesses}
The verified six-witness orbit is:
...
\midrule
$G$ & CODATA rounded mantissa & $667430$ &
$2\cdot5\cdot31(73\cdot29+36)$ \\
$g_0$ & conventional exact & $980665$ &
$5\cdot7(73(27\cdot13+32)+6\cdot10)$ \\
$1\,\mathrm{atm}$ & conventional exact & $101325$ &
$3\cdot5^2\cdot7(2^7+5\cdot13)$ \\
$m_pc^2$ in rounded keV & CODATA rounded mantissa & $938272$ &
$2^5(10^2+3^2)(4^4+13)$ \\
Faraday $F$ & SI-derived, rounded display & $9648533$ &
$31(4\cdot137-1)(4\cdot137+3\cdot7)$ \\
$R\cdot10^6$ & SI-derived, rounded display & $8314463$ &
$(5\cdot13+2\cdot7\cdot73)(7\cdot1111-128)$ \\
\bottomrule
\end{tabular}
\end{center}
The molar-gas lock uses
\[
  L_{\mathrm{eff}}=p_{\mathrm{Ih}}\big((k-\lambda)^2+1\big)=11\cdot101=1111.
\]
\end{theorem}

\begin{corollary}[MCCCLXXXV boundary]\label{cor:unit_gauge_boundary}
The six witnesses split as
\[
  \begin{gathered}
...
\[
  ...
  \end{aligned}
\]
This preserves all verified arithmetic while preventing the category
error of treating a dimensionful decimal mantissa as a unit-independent
prediction.
\end{corollary}
...
The companion therefore has two layers.  Dimensionless constants and
ratios are the prediction layer.  SI, conventional, and derived
dimensionful constants form a unit-gauge witness layer only after the
metrology convention has fixed the display integer.

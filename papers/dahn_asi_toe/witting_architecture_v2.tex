...
control symmetries, and an $11$-branch non-backtracking transport rule.
...
accelerators, storage, and fabric.
\end{guardrail}
...
\begin{designprinciple}[A4: Bell-Qutrit Storage Cell]
Each storage cell holds the temporal Bell qutrit
$\ket{\Omega} = q^{-1/2}\sum_j \ket{j}_p \ket{j}_f$ across past and future temporal
registers. Read is the Choi--Jamio\l{}kowski overlap; write is projection onto the future
register.
\end{designprinciple}
...
single-qutrit state space. A4 makes A6 (CSS code) trivial because Bell qutrits are
natively CSS-encoded. A5 (BIER multicast) makes A2 work at scale because group action
...
\node[align=left,anchor=west] at (4.4,-1.9) {\scriptsize $30$ conjugacy classes (ISA families)};
\end{tikzpicture}
...
edges and automorphism group $\Sp(4,\FF_3)$ of order $51{,}840$.}\label{fig:tile}
\end{figure}
...
{
  \draw[fill=violet!15,draw=violet!60!black] (0,\y) rectangle (\width,\y+0.5);
  \node[anchor=west] at (0.1,\y+0.25) {\scriptsize\bfseries \name};
  \node[anchor=west,align=left] at (\width+0.2,\y+0.25) {\scriptsize rank \rank, $\role$};
}
% labels
\node[align=left,font=\small] at (1,3.0) {Bose--Mesner three-channel bus};
\node[align=left,font=\scriptsize] at (1,-1.6) {bus width $\propto$ projector rank};
\end{tikzpicture}
...
The WRF ISA is the group $G = \Aut(\W) = \Sp(4, \FF_3) \cong W(E_6)$, of order
$|G| = 51{,}840$. Every instruction is an element $\sigma \in G$ that acts on an
edge-mode configuration $\mathcal{C} \subseteq E(\W)$, producing the new configuration
$\sigma(\mathcal{C})$. By the Triple Convergence theorem
$\#\mathrm{conj}\,G = h(E_8) = Z_{\mathrm{DW}}(\Sp(4,\FF_3); T^2) = 30$, so the ISA has
\emph{exactly} $30$ conjugacy classes---instruction \emph{families} at the
microarchitectural level number $30$, not $51{,}840$.

\subsection{\texorpdfstring{Microarchitecture: $30$ families, $8$ generators}{Microarchitecture: 30 families, 8 generators}}
...
\begin{itemize}[leftmargin=2em,nosep]
\item $30$ conjugacy-class fast paths (single-cycle dispatch),
...
\begin{align}
\text{Conjugacy cadence} \quad &=\ \TPS \cdot \frac{|G|}{h(E_8)}
...
\end{align}
The conjugacy multiplier $1728 = k^3 = j(i)$ is the modular $j$-invariant at $\tau = i$---a
...
\midrule
$2026$ & Software reference fabric: UOR-ADDR CIDs, Atlas-$12288$ frame tests, HLIX/HCP
projection containers, Smart Asset workload envelopes, and Oko BLA simulation wired into
one reproducible benchmark harness.\\
$2027$ & FPGA Witting tile prototype with $40$ trit/logical cores, three-channel
Bose--Mesner routing emulator, hardware hash path, CSS syndrome path, and BIER multicast
testbed.\\
...
$51{,}840$ and conjugacy cardinality $30$. Every subsystem of WRF---the Atlas-$12288$
memory frame, the Bose--Mesner three-channel bus, the spectral cache, the Bell-qutrit
storage cell, the CSS code, the group-action ISA, the BIER fabric, the projection
...
\item[Aut$(W(3,3))$] The full automorphism group of the W(3,3) symplectic polar space;
isomorphic to $\Sp(4,\FF_3) \cong W(E_6)$; order $51{,}840$.
...
$\Aut(\W)$ has $30$ conjugacy classes.
...
$1728$  & $k^3 = j(i)$                     & $|\Aut|/h(E_8)$, conjugacy multiplier \\
$30$    & $q\Phi_4 = h(E_8)$               & Triple Convergence (ISA family count) \\

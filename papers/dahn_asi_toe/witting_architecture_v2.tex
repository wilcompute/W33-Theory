...
\item one receipt-bearing macro/inverse pair for the measured diameter tail,
\item Cayley-graph distance lookup for compiler scheduling.
\end{itemize}

BT157 then stress-tests the word metric instead of assuming it \cite{wrf-bt157}. For the
eight transvection centers used in the local qutrit realisation, the exact all-element BFS
over $51{,}840$ elements gives:
\[
\operatorname{diam}_{8{\rm fwd}}=9=q^2,\qquad
\operatorname{diam}_{16{\rm inv}}=7=q!+1,\qquad
\operatorname{diam}_{16{\rm inv}+M^{\pm1}}=6=q!.
\]
The macro $M$ is an order-$9$ element from the inverse-complete distance-$7$ tail, with
one verified word
\[
M=T_1T_2T_1^{-1}T_6T_4^{-1}T_5T_6^{-1}.
\]
Thus the raw $8$-lane compiler does \emph{not} have $q!$ depth. The exact architecture is
more specific: $8$ physical lanes, $16$ directed elementary pulses, and one
receipt-bearing macro pair collapse global dispatch to the master-equation depth $q!=6$.
...
The measured BT157 distance table also makes the compiler boundary explicit. With only
forward elementary lanes the distance distribution ends at $9$; with inverse-complete
directed pulses it ends at $7$ with a $151=4v-q^2$ element tail; adding the macro pair
removes that tail and leaves only $1217$ elements at depth $6$. So the compiler is not
``magically'' shallow; it is shallow because the finite table identifies and packages the
tail.

BT158 then audits the whole tail \cite{wrf-bt158}. Of the $151$ distance-$7$ candidates,
$143=p_{\mathrm{Ih}}\Phi_3=11\cdot13$ are admissible q!-restoring macros and
$8=2^q$ are forbidden, preserving diameter $7$. The forbidden pocket consists of four
involutions and four order-$12$ elements, all with trace $0$ modulo $3$. The two most
efficient admissible macros are both order-$9$ trace-$1$ elements and leave only $890$
symmetries at depth $6$. Thus WRF microcode has genuine finite design freedom: the macro
pair is chosen from a $143$-element admissible sieve, not from a single accidental element.

BT159 explains the forbidden pocket rather than discarding it \cite{wrf-bt159}. The eight
forbidden macros generate a $1{,}152$-element group, exactly the order of $W(F_4)$, split as
$576$ block-diagonal maps preserving the $2+2$ Lagrangian polarization and $576$
anti-diagonal maps swapping it. The obstruction pocket is therefore the finite
F4/24-cell/tomotope control normalizer. It fails as a single global diameter-collapse
macro because it remains inside the polarization normalizer, but it is precisely the
local control group for the 24-cell side of the architecture.
...
\bibitem{wrf-bt157}
Local WRF harness, \emph{BT157 Cayley compiler macro-depth correction},
\texttt{analysis/w33\_BREAKTHROUGH\_157\_cayley\_compiler\_macro\_depth.py}
with result JSON, generated June 4, 2026.

\bibitem{wrf-bt158}
Local WRF harness, \emph{BT158 macro-tail sieve},
\texttt{analysis/w33\_BREAKTHROUGH\_158\_macro\_tail\_sieve.py}
with result JSON, generated June 4, 2026.

\bibitem{wrf-bt159}
Local WRF harness, \emph{BT159 forbidden pocket F4 normalizer},
\texttt{analysis/w33\_BREAKTHROUGH\_159\_forbidden\_pocket\_f4\_normalizer.py}
with result JSON, generated June 4, 2026.
\end{thebibliography}

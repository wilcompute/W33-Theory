...
\item \textbf{A realistic vacuum.} Every parity built from the gauge torus, the space group and the
      established rotation symmetries is excluded, in $\Z_6$-I, $\Z_6$-II, $\Z_2\times\Z_6$ and
      $\Z_3\times\Z_6$ alike. What remains: $\Z_{12}$-I, where no rotation rule is established for its
      lattice ($9$ models undetermined); orbifold groups with the twist not yet scanned
      ($\Z_6\times\Z_6$, $\Z_2\times\Z_6$-II are running); vacua with non-singlet condensates; and a
      mechanism other than a parity for proton stability.
...
a parity from plane rotations reopens one model & it used an $R$-rule valid only on prime planes; with the established rules it closes (\S9.4)\\
\bottomrule
\end{tabularx}
\end{center}

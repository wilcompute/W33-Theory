...
\begin{tikzpicture}[
  ...
  \entry{6}{Handedness of the weak force (which chirality)}{rose}{NO-GO}
  \entry{7}{Proton stability (matter parity)}{gold}{CONDITIONAL}
  \entry{8}{A supersymmetric, realistic vacuum}{gold}{NOT YET}
...
it. \textsf{CONDITIONAL}: holds if a named open condition is met. \textsf{NO-GO}: proved
impossible from inside. \textsf{OPEN}: not addressed by any exact result.}
\label{fig:scorecard}
\end{figure}
...
\begin{tabularx}{\textwidth}{@{}p{3.2cm}Xp{2.3cm}@{}}
\toprule
\textbf{Requirement} & \textbf{Current exact status (class of $87$ $\Z_6$-I models)} & \textbf{Badge}\\
\midrule
...
$D$-flat directions & Exist in $23$ of $87$ ($\Z_6$-I) and $105$ of $128$ ($\Z_6$-II), after a
trace-weighting correction. & \tierC\\
Proton stability & A $\Z_2$ matter parity from $U(1)_{B-L}$ exists in all $87$; all bare
dangerous operators are matter-odd, all Yukawas and $\mu$ are even (criterion of
\cite{Lebedev2008}). Every $D$-flat support
examined contains a matter-odd singlet (sampled, not exhausted). & \tierO\ (conditional)\\
...
\begin{boundary}[The live question]
Is there a vacuum of this class that condenses only matter-even singlets and is both $F$-
and $D$-flat? If yes, the class survives with proton stability; if no, the class is
excluded. This is a finite, well-posed computation, and it is open.
\end{boundary}
...
``qutrit plane''. It cannot choose the handedness of the weak force, and no realistic
supersymmetric vacuum has yet been exhibited.
\end{keyidea}

...
Parity from rotations & Appeared in one model under the old per-plane rule; with the established
rules (Bizet et al.) it closes: $0$ of $128$. & \tierC\ (excluded)\\
...
\begin{tikzpicture}[
  ...
  \node[band=rose,minimum width=5.6cm] (e) at (0,-4.2) {parity from plane rotations \quad \textbf{0}};
  \node[band=gold,text width=5.2cm] (f) at (0,-5.35) {old per-plane rule: 1 model (withdrawn,\\ an incomplete $R$-rule; P10974)};
  \foreach \u/\v in {a/b,b/c,c/d,d/e} \draw[->,ink!50,line width=0.7pt] (\u) -- (\v);
  \draw[->,ink!50,dashed,line width=0.7pt] (e) -- (f);
  \node[tag] at (3.75,0) {5b3f3ad};
  \node[tag] at (2.95,-1.05) {P10960};
  \node[tag] at (2.25,-2.1) {P10960};
  \node[tag] at (2.95,-3.15) {P10960--10967};
  \node[tag] at (2.95,-4.2) {P10968, P10974};
  % side results
  ...
  \node[band=rose,text width=2.9cm,align=center] (z4) at (6.4,-7.35) {exotics massive: \textbf{0} of 5 decided\ (9 await an $R$-rule)};
  ...
  % Z3xZ6-row
  \node[font=\sffamily\bfseries\small,text=ink] at (0,-8.05) {$\Z_3\times\Z_6$ orbifolds};
  \node[band=sky,minimum width=3.2cm] (x1) at (-2.55,-8.8) {Standard Models \quad \textbf{5}};
  \node[band=rose,minimum width=2.3cm] (x2) at (0.45,-8.8) {any parity \quad \textbf{0}};
  \draw[->,ink!50,line width=0.7pt] (x1) -- (x2);
  \node[font=\sffamily\bfseries\small,text=ink] at (6.4,-8.55) {$\Z_6\times\Z_6$ orbifolds};
  \node[band=sky,minimum width=2.2cm] (w1) at (5.15,-9.3) {SMs \quad \textbf{10}};
  \node[band=rose,minimum width=2.0cm] (w2) at (7.7,-9.3) {survive \quad \textbf{0}};
  \draw[->,ink!50,line width=0.7pt] (w1) -- (w2);
  \node[font=\sffamily\scriptsize,text=slate,align=center] at (3.2,-10.1) {$\Z_2\times\Z_6$-II with the same twist: no Standard Model in $51\,000$ tries};
\end{tikzpicture}
\caption[The vacuum gauntlet]{The vacuum gauntlet. Blue: filters passed; red: dead ends; gold: a door that
looked open and was withdrawn. Every count is exact (Farkas certificates, Smith normal forms, lattice tests and
...
\paragraph{The last door: a parity made of rotations --- and why it closes.}
...
can act as a matter parity. With the rules built into the standard orbifold code, this reopens exactly
one model, $\mathbb{Z}_6$II$_{23}$, and the model then shows a striking two-sided trade: vacua kept flat
by an unbroken rotation keep the exotic colour triplets massless at every order ($202$ found, $202$ with
massless exotics), while larger vacua that break the rotation make every exotic heavy but either lose
the light Higgs pair or keep it only at the price of a superpotential that is not zero:
\begin{center}
\small
\begin{tabular}{lccc}
\toprule
vacuum (old rotation rules) & flat by symmetry & exotics & a light Higgs pair \\
\midrule
...
\end{tabular}
\end{center}
Even the best of these, the certified $23$-direction vacuum, would not have described our world: the
Higgs pair it protects is protected by an exact symmetry that also forbids any mass for its
superpartners (massless Higgsinos, long excluded), and although its top-quark coupling is of order
one, its down-quark and lepton couplings show no hierarchy at all.

Then the rules themselves were checked. Bizet, Kobayashi, Mayorga~Pe\~na, Parameswaran, Schmitz and
Zavala derived the $R$-charge selection rule directly from the symmetries of the worldsheet instantons
that mediate string couplings, and found that the textbook rule --- conservation of the rotation charge
plane by plane --- holds only for planes whose rotation has prime order. On a plane of order six the
rule acquires an extra contribution from the phases with which twisted strings at different fixed
points combine. The door of $\mathbb{Z}_6$II$_{23}$ had used the uncorrected rule on its order-six plane.
Keeping only the rotation symmetries that are established (the order-$3$ and order-$2$ planes), and
separately adding the corrected order-six charge, the model closes: all $304$ Fayet--Iliopoulos
cancelling directions break the parity. The whole $\Z_6$-II class is then excluded, $0$ of $128$, and
three further orbifold families built on the same twist, $\Z_2\times\Z_6$ ($29$ Standard Models),
$\Z_3\times\Z_6$ ($5$) and $\Z_6\times\Z_6$ ($10$), have no surviving parity either, and $\Z_2\times\Z_6$-II
produces no Standard Model at all. The apparent opening was an artefact of an
incomplete selection rule; it was caught by reading the literature on the rules themselves, not by
the computation. \tierC\ (\cert{w33\_pass10974\_valid\_r\_rules\_close\_the\_rotation\_door.py};
the conditional analysis is \cert{w33\_pass10968\_r\_symmetry\_parity\_and\_massless\_exotics.py})

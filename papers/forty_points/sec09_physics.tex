...
Proton stability & No supersymmetric singlet vacuum preserves a matter parity: $0$ of $88$
models over the full $B-L$ freedom, $0$ of $1024$ $\Z_2$ parities (Section~\ref{sec:sneutrino}). & \tierC\ (excluded)\\
...
\begin{boundary}[Scope, and the one loophole]
The theorem covers every direction built from fields that are singlets of all nonabelian
groups --- the space specified by the programme's own closure contract. Hidden-sector
nonabelian directions (for example condensates of hidden $SO(10)$ spinors or $SU(2)$
doublets, whose parity can be compensated by a hidden centre element) are not covered, and
are now the only route to a supersymmetric vacuum with matter parity in this class.
Whether higher-order selection rules suppress the regenerated couplings in a particular
model is also not addressed. \tierO
\end{boundary}
...
\cert{w33\_pass5727\_5730\_torsion\_family\_heisenberg.py}). The other research track has now shown, on the
clock itself, that the determinant-odd ticks are exactly those that exchange the two
half-spin sheets (Pass~10955) and that they are not completely positive --- they are not
physical operations on a bare qutrit but antiunitary, like time reversal (Pass~10952). So
the geometry's chirality is invariant under every \emph{unitary} symmetry and exchanged only
by \emph{antiunitary} ones. That is the structure of a chiral quantum theory obeying CPT:

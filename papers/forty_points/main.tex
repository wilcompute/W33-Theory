...
and very heavy superpartners. Each either fails outright or trades the problem for another,
and the last one would need a top quark lighter than the one we measure.
...
$Q(u^c)+Q(e^c)=2Q(q)$. Plane-rotation $R$-symmetries reopen parity in one model only, where an
unbroken rotation keeps the exotics massless and a broken one leaves no protected Higgs pair; and
...
\input{sec00_reading}
\input{sec01_prologue}
\input{sec02_object}
\input{sec03_selfentangle}
\input{sec04_time}
\input{sec05_e8}
\input{sec06_freudenthal}
\input{sec07_realforms}
\input{sec08_lorentz}
\input{sec09_physics}
\input{sec10_machine}
\input{sec11_epilogue}
\appendix
\input{appA_ledger}
\input{appB_glossary}
\input{appC_references}

...
\claim{CERT}{BT1393 resolves the Q5--Q7 heptad-ladder chronology. The earlier Q7 ceiling note is a pre-ladder/W63-extension caution, BT1356 resolves the heptad-ladder Q7 case, and BT1358's ``exact W33 matches = 0'' refers to the competitor pool rather than absence of the W33 reference family.}

\claim{CERT}{The canonical heptad ladder is Q4 sub-Ramanujan, Q5 sub-Ramanujan, Q6 first super-Ramanujan, and Q7 period-closing with direct gap \(3.0616\).}
...
\claim{SIM}{BT1394 verifies the reduced Bell-qutrit signatures \(V(I)=1\), \(V(F_3)=1/3\), \(V(X)=0\), and \(V(Z)=0\), and models route-controlled Clifford transport.}

\claim{SIM}{BT1396 supplies the missing readout: a Bell-branch quantum eraser projects onto \((|\Omega\rangle+|Z\Omega\rangle+|X\Omega\rangle)/\sqrt 3\), succeeds with probability \(1/3\), restores a pure coherent route qutrit with \(l_1\)-coherence 2, and routes deterministically into one interferometer output port.}
...
\input{../tex/bt1380_post_1377_claim_table.tex}

\input{../tex/bt1381_bt1383_runtime_frontier_insert.tex}

\claim{OPT}{BT1395 defines a formal S3 MaxSAT certificate pathway with witness, upper-bound, and optimality certificate types. The current project status remains witness-only at score 210 until a solver-generated upper-bound proof is imported. BT1397 demonstrates the optimality code path with a synthetic example certificate, explicitly not a project-level proof.}

...
error-correcting code, and the benchtop experiment. These six faces, and the single
object behind them, are laid out in \S\ref{sec:five-faces}
...
\subsection{The one object and its six faces}
...
constants, the gauge group, the neutrino masses, the fault-tolerant code, and the
benchtop experiment --- is \emph{one structure} seen from six sides. That structure
is the $q=3$ Eisenstein object: the Witting polytope $3\{3\}3\{3\}3\{3\}3$ over
$\mathbb Z[\omega]$ ($240$ vertices $=$ E8 roots), its symmetry the Shephard--Todd
group $\#32$ (order $155520=3\lvert\mathrm{Sp}(4,3)\rvert=3\lvert\mathrm{Aut}(\W)
\rvert$), equivalently the degree-two cyclotomic skeleton $\{\Phi_3,\Phi_4,\Phi_6\}$
whose indices $\{3,4,6\}$ are the crystallographic complex periods. Its six faces
(Fig.~\ref{fig:five-faces}):
...
presenting six faces. (Witnesses: \path{analysis/w33_eisenstein_grand_synthesis.py},
\path{analysis/w33_gauge_sixth_face.py}, \path{analysis/w33_witting_degrees_unify.py}.)
...
\begin{tikzpicture}[
  ...
  \node[face,fill=blue!8]    (s1) at (  90:4.3) {Selection\\$(q{-}3)\Phi_4$, $\{3,4,6\}$};
  \node[face,fill=green!10]  (s2) at (  30:4.6) {Constants\\$\{13,10,7\}$; $\{12,18,24,30\}$};
  \node[face,fill=teal!14]   (s6) at ( -30:4.6) {Gauge (SM)\\$24{=}c$, $12{=}k$, $3/8$};
  \node[face,fill=yellow!18] (s3) at ( -90:4.3) {Neutrino\\$\Phi_3/q^2=13/9$};
  \node[face,fill=orange!16] (s4) at (-150:4.6) {Code\\$A_2{<}D_4{<}E_8$, $c{=}24$};
  \node[face,fill=purple!12] (s5) at ( 150:4.6) {Demonstrator\\$1/\Phi_4$, $\lambda{=}2$};
  \foreach \s in {s1,s2,s3,s4,s5,s6} \draw[spoke] (o)--(\s);
\end{tikzpicture}}
...
seen from six sides. The selection of $q=3$, the substrate constants, the Standard-Model
gauge group, the neutrino mass hierarchy, the fault-tolerant code lattice, and the
benchtop experiment are faces of the same structure, sharing the same invariants
($240=E_8$ roots, $\{\Phi_3,\Phi_4,\Phi_6\}$, the Witting degrees $\{12,18,24,30\}$,
$155520=3\lvert\mathrm{Aut}(\W)\rvert$).}

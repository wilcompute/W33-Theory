...
(Fig.~\ref{fig:five-faces}). \emph{The honest edge}: every geometric integer is a
cyclotomic value, GQ count, or Witting degree, but the fine-structure constant
($1/\alpha\approx137$) and the Monster dimension $196883$ sit at the $c=24=f$ moonshine
ceiling ($196883$ is second-layer forced via the complex Leech; $\alpha$ is the remaining
edge). \emph{The one experiment to run first}: the benchtop, calibration-free contextual
fraction $1/\Phi_4=1/10$, which tests three faces at once.
...
\emph{The honest edge.} A synthesis is worth its boundary. Every geometric integer of
the seven faces is a cyclotomic value $\Phi_d(3)$, a GQ count, a Witting degree, or a
simple combination of these (e.g.\ $\lvert\mathrm{PSL}(2,7)\rvert=168=\Phi_6\!\cdot\!f$,
$\dim E_8=248=240+8$). But not everything reduces directly: the Monster minimal
dimension $196883$ and the fine-structure constant ($1/\alpha\approx137$) are
\emph{not} Witting degrees or cyclotomic values; they sit one level up, at the
$c=24=f$ moonshine ceiling. The Monster dimension, however, is not free: the classical
$j$/Leech identity $196884=196560+324$ with $324=\mu q^4=h(E_7)^2$ forces
$196883=196560+\mu q^4-1$, and the Leech kissing number $196560$ is carried by the
\emph{complex} (Eisenstein) Leech --- the $\mathbb Z[\omega]$, rank-$12{=}k$, $6\cdot
\mathrm{Suz}$ lattice atop the substrate's Eisenstein tower. So $196883$ is
\emph{second-layer forced}, welded to the object; the genuine remaining edge is one
factorization deeper ($196560=252\cdot780$, $\tau=252$), where $\alpha$ enters via the
Suzuki lift. The unification is therefore layered, not floating: the Witting object
fixes the seven faces directly, the complex Leech determines the Monster ceiling, and
$\alpha$ alone is the open edge. (The third cyclotomic value $\Phi_6(3)=7$ also surfaces
in the toroidal thread: the Cs\'asz\'ar polyhedron is $K_7$ on the torus and the dual
Szilassi has $7$ faces, with $C_2$ symmetry of order $2=\lambda=q-1$.) Witnesses:
\path{analysis/w33_eisenstein_stress_test.py},
\path{analysis/w33_monster_leech_second_layer.py}. And the single most decisive
near-term test --- benchtop, calibration-free, and touching three faces --- is the
contextual fraction $1/\Phi_4=1/10$ (\path{analysis/w33_decisive_experiment.py}).

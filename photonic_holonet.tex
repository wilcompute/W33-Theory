...
\begin{tikzpicture}[
  ...
  \node[optic,rotate=45] (pbs) at (2.4,0) {};
  \node at (2.4,-0.85) {PBS};
  \node[block,fill=blue!6] (spatial) at (4.9,0)
  ...
  \draw[arrow] (spatial.south) .. controls (4.9,-1.05) and (0,-1.05) .. (tri.north);
  \draw[arrow] (tri) -- (delay);
  \draw[arrow] (delay) -- (eom);
  \draw[arrow] (eom) -- (out) node[midway,above]{$\ket{\Omega}$};
  \draw[arrow] (out) -- (choi);
  \draw[-{Stealth},dashed,thick] (out.north) .. controls (8.8,-0.85) and (4.0,-0.85) ..
    (delay.north);
  \node[fill=white,inner sep=1pt] at (6.8,-0.68)
    {BC loop: rotate $\arccos(-2/3)$};
\end{tikzpicture}}
...
\begin{tikzpicture}[
  ...
  \draw[arrow] (1.8,0) -- node[above]{substitute} (2.8,0);
  \draw[arrow] (6.4,0) -- node[above]{substitute} (7.4,0);
\end{tikzpicture}

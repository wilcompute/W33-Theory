...
\subsection{Three closures: the neutrino matrix, the selection census, the threshold number}
...
\subsection{Sharper still: a Majorana invariant, a cyclotomic merge, a code curve}
\label{sec:sharper}
...
(Fig.~\ref{fig:d4-curve}), so $D_4$ reaches any target error at $1.5$\,dB less
squeezing and the surface-GKP threshold $9.9$\,dB becomes $\sim8.4$\,dB --- below the
$9.5$\,dB already demonstrated. The relative $1.5$\,dB $D_4$-versus-square advantage
is the falsifiable handle. (Honest: the lattice quantities are exact; the curve is the
union-bound effective-distance model, not a full circuit-level FT simulation. Witness:
...
shifted left by its exact $1.5$\,dB coding gain, so the surface-GKP fault-tolerance
level ($9.9$\,dB for the square code) is reached at $\sim8.4$\,dB --- below the
$9.5$\,dB squeezing already demonstrated. The relative advantage is the falsifiable
handle.}
\label{fig:d4-curve}
\end{figure}

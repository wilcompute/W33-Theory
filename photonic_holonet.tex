...
So the non-Clifford/entangling resource of the machine sits \emph{exactly} on
the curved directions: the $12$ flat causal directions are free photon
propagation and give only abelian gates, while every non-classical gate is a
loop of nonzero gauge curvature. In the physics reading the same curvature on
...
\]
all one object, while flatness is causality. The machine's quantum advantage is
not added to the substrate; it \emph{is} the curvature of the Standard-Model
gauge field. (Witness:
\texttt{analysis/w33\_gauge\_curvature\_is\_computation.py}: $40$ transvections of
order $3$, the $12$ flat / $27$ curved split per point, curvature orders $\{4\}$
on $Q$, and $\langle R_p\rangle=\mathrm{Sp}(4,3)$.)

...
realise \emph{topological frequency conversion} \cite{mrh-pump}, and the
two-tone driven carrier has a computed Chern number $C=+1$ at the natural
operating point (trivialising only past the gap-closing at large longitudinal
field), so energy is pumped between the round-trip and twist drives at a
quantised, dissipationless, perturbation-robust rate $\dot W=(C/2\pi)\,\omega_1\omega_2$.
The substrate fixes the two frequencies (round-trip and BC twist) squarely into
this $C\neq0$ topological class. \emph{(The quasicrystal structure --- three-gap,
incommensurate frequencies, non-recurrence --- and the Chern number are computed;
the honest residual is that the invariant is evaluated for the canonical two-tone
qubit model, while the qutrit device's exact invariant needs its specific drive
operators.)} (Witnesses:
\texttt{analysis/w33\_self\_correction\_quasicrystal.py},

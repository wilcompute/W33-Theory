...
\subsection{The one object and its five faces}
\label{sec:five-faces}

Before the details, the organizing claim. Everything in this section --- the
selection of $q=3$, the substrate constants, the neutrino masses, the
fault-tolerant code, and the benchtop experiment --- is \emph{one structure} seen
from five sides. That structure is the $q=3$ Eisenstein object: the Witting polytope
...
skeleton $\{\Phi_3,\Phi_4,\Phi_6\}$ whose indices $\{3,4,6\}$ are the
crystallographic complex periods. Its five faces (Fig.~\ref{fig:five-faces}):
...
So the $q=3$ substrate is not a list of coincidences spanning cosmology, particle
physics, computation, and metrology; it is one Eisenstein object presenting five
faces. (Witnesses: \path{analysis/w33_eisenstein_grand_synthesis.py},
\path{analysis/w33_witting_degrees_unify.py}.) The subsections that follow detail
each face and the connections between them --- including a benchtop bridge that turns
the neutrino mass ratio into a single-photon measurement
(\path{analysis/w33_neutrino_demonstrator_bridge.py}).
...
\begin{tikzpicture}[
  ...
  \node[face,fill=blue!8]    (s1) at ( 90:3.7) {Selection\\$(q{-}3)\Phi_4$, $\{3,4,6\}$};
  \node[face,fill=green!10]  (s2) at ( 18:4.1) {Constants\\$\{13,10,7\}$; $\{12,18,24,30\}$};
  \node[face,fill=yellow!18] (s3) at (-54:4.1) {Neutrino\\$\Phi_3/q^2=13/9$};
  \node[face,fill=orange!16] (s4) at (234:4.1) {Code\\$A_2{<}D_4{<}E_8$, $c{=}24$};
  \node[face,fill=purple!12] (s5) at (162:4.1) {Demonstrator\\$1/\Phi_4$, $\lambda{=}2$};
  \foreach \s in {s1,s2,s3,s4,s5} \draw[spoke] (o)--(\s);
\end{tikzpicture}}
...
seen from five sides. The selection of $q=3$, the substrate constants, the neutrino
mass hierarchy, the fault-tolerant code lattice, and the benchtop experiment are
faces of the same structure, sharing the same invariants ($240=E_8$ roots,
$\{\Phi_3,\Phi_4,\Phi_6\}$, $155520=3\lvert\mathrm{Aut}(\W)\rvert$).}
...
\subsection{The measurable layer: a scorecard and a unification}
\label{sec:physics}
...
\subsection{Why \texorpdfstring{$q=3$}{q=3}, and one triality for the neutrino}
\label{sec:why-q3}

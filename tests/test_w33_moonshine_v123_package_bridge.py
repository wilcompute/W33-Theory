...
def test_3c_has_sparse_package_0_248_0():
    ...
    rows = {
        row["class_name"]: row
        for row in summary["moonshine_v123_package_dictionary"]["rows"]
    }
...
def test_41a_has_endpoint_package_2_2_3():
    ...
    rows = {
        row["class_name"]: row
        for row in summary["moonshine_v123_package_dictionary"]["anchored_rows"]
    }
    assert rows["41A"]["inferred_package"] == {"V1": 2, "V2": 2, "V3": 3}
    assert rows["41A"]["actual_package"] == {"V1": 2, "V2": 2, "V3": 3}
    assert rows["41A"]["all_match"] is True

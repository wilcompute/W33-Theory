...
def test_pass1150_synthetic(tmp_path:Path):
    ...
    (tmp_path/"data/w33_shifted_adjacency_retraction_ledger.json").write_text(json.dumps(ledger))
    r=m.run(tmp_path,apply=True); assert r["status"]=="PASS" and r["pending_before"]==3
    new=json.loads((tmp_path/"data/w33_shifted_adjacency_retraction_ledger.json").read_text()); assert new["schema"].endswith("v4") and all("pending" not in s.lower() for s in new["known_descendants"].values())

...
def test_v23_counts_match_toe_core():
    ...
    assert centers_counts == {int(k): int(v) for k, v in expected["triangle_counts_by_centers"].items()}
...
# robust direct comparison using dataframe filtering to avoid parsing issues
    ...
    for k_str, v in expected["triangle_counts_by_parity_and_cycle_type"].items():
        ...
        actual_count = int(df[(df.z2_parity == parity) & (df.fiber6_cycle_type == cycle_str)].shape[0])
        assert actual_count == int(v), f"Mismatch for {(parity, cycle)}: expected {v}, got {actual_count}"

    # exact law entries are present and match counts (centers, z2_parity, fiber6_cycle_type)
    combo = Counter((int(r.centers), int(r.z2_parity), r.fiber6_cycle_type) for r in df.itertuples(index=False))
    for e in expected["exact_law"]:
        key = (int(e["centers"]), int(e["z2_parity"]), e["fiber6_cycle_type"])
        assert combo.get(key, 0) == int(e["count"]), f"Mismatch for {key}: expected {e['count']}, got {combo.get(key,0)}"

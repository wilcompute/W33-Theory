...
def test_exact_boundaries_and_engineering_separation():
    ...
    d12 = json.loads(
        (root / "data/w33_pass2012_enumerated_subgroup_orbit_parallel_classes.json").read_text()
    )
    d13 = json.loads(
        (root / "data/w33_pass2013_rank_three_spread_association_scheme.json").read_text()
    )
    d14 = json.loads(
        (root / "data/w33_pass2014_one_line_spread_pair_rook_double.json").read_text()
    )
    d15 = json.loads(
        (root / "data/w33_pass2015_degree_safety_quadratic_physics_engineering.json").read_text()
    )

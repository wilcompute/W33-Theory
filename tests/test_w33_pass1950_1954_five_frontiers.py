...
def test_exact_boundaries_and_nonvacuity():
    a=json.loads((ROOT/'data/w33_pass1950_1954_five_frontiers.json').read_text())
    assert a['critical_values']['colourfree_milp_status']=='UNKNOWN'
    x=json.loads((ROOT/'data/w33_pass1952_frame_chart_abi_sound_lex.json').read_text())
    ...
    y=json.loads((ROOT/'data/w33_pass1953_arithmetic_group_sl3z.json').read_text())

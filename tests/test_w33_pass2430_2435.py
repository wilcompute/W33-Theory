...
def load(name):
 d=json.loads((ROOT/'data'/name).read_text());assert digest(d)==d['sha256_without_hash_field'];return d
...
def test_publication_hooks_and_boundaries():
 for f in ('w33_paper.tex','photonic_holonet.tex'):
  assert 'analysis/BT2435_five_frontiers_insert' in (ROOT/f).read_text()
 release=(ROOT/'PASS2430_2435_FIVE_FRONTIERS_RELEASE.md').read_text()

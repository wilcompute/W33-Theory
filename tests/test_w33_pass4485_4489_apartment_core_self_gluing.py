...
def test_4485_literal_repeated_core():
    d=json.loads(CERTS[0].read_text()); assert d['pass']==4485
...
def test_4486_forms_are_not_conflated():
    d=json.loads(CERTS[1].read_text()); assert d['pass']==4486
...
def test_4487_parallel_core_is_literal_coordinate_match():
    d=json.loads(CERTS[2].read_text()); assert d['pass']==4487
...
def test_4488_extension_is_nonsplit():
    d=json.loads(CERTS[3].read_text()); assert d['pass']==4488
...
def test_three_manuscripts_and_public_sources_include_self_gluing_once():
    ...
    page=(ROOT/'docs/apartment-core-self-gluing.html').read_text()

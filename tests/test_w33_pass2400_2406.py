...
def load(n,s):
    d=json.loads((ROOT/f'data/w33_pass{n}_{s}.json').read_text());assert d['sha256_without_hash_field']==digest(d);return d
...
def test_boundaries_and_hooks():
    ...
    assert all('BT2406_six_frontiers_insert' in (ROOT/p).read_text() for p in ('w33_paper.tex','photonic_holonet.tex'))

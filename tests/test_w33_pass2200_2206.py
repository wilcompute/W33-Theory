...
def test_all_q_regular_spread_certificate():
 ...
 assert m.build()==json.loads((ROOT/'data/w33_pass2201_2202_all_q_regular_spread_scheme.json').read_text())

def test_ree_tits_nonregular_certificate():
 m=load('p2203','analysis/w33_pass2203_ree_tits_nonregular_control.py')
 d=m.build();assert d==json.loads((ROOT/'data/w33_pass2203_ree_tits_nonregular_control.json').read_text())
...
def test_controller_and_quadratic_correction():
 ...
 d=m.build();assert d==json.loads((ROOT/'data/w33_pass2200_2204_2205_quadratic_controller_audit.json').read_text())

...
def test_bt982_final_gram_is_e8():
    data = json.loads(Path("data/bt982_explicit_integral_e8_basis.json").read_text())
...
def test_bt982_both_gauges_agree():
    bt954 = json.loads(
        Path("data/bt954_metric_selector_among_support60.json").read_text()
    )
    bt956 = json.loads(
        Path("data/bt956_tetracode_metric_selector_matrix.json").read_text()
    )
    bt982 = json.loads(Path("data/bt982_explicit_integral_e8_basis.json").read_text())

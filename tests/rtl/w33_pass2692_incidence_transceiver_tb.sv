...
module w33_pass2692_incidence_transceiver_tb;
    ...
    w33_pass2691_incidence_core #(.W(W), .OW(OW), .REVERSE(1'b0)) forward_core (
        .in_flat(in_forward), .out_flat(out_forward)
    );
    w33_pass2691_incidence_core #(.W(W), .OW(OW), .REVERSE(1'b1)) reverse_core (
    ...
    w33_pass2691_incidence_serial #(.W(W), .OW(OW), .REVERSE(1'b0)) serial_core (
    ...
    initial begin
        ...
        check_serial_basis0();
        $display("PASS w33_pass2692_incidence_transceiver_tb");
        $finish;
    end
endmodule

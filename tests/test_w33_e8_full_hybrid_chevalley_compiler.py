...
def test_executable_bracket_on_source_witnesses_in_every_grade_product():
    runtime=load().main(write=False,full_jacobi=False,return_runtime=True)
    grade={source:g for g,block in enumerate(runtime.blocks) for source in block[1]}
    covered=set()
    for (i,j),terms in runtime.table.items():
        pair=tuple(sorted((grade[i],grade[j])))
        if pair in covered or not terms: continue
        x=runtime.from_source({i:runtime.one})
        y=runtime.from_source({j:runtime.one})
        expected={k:runtime.one*c for k,c in terms}
        result=runtime.bracket(x,y)
        assert runtime.to_source(result)==expected
        assert runtime.bracket(y,x)=={k:-v for k,v in result.items()}
        assert runtime.bracket(x,x)=={}
        covered.add(pair)
    assert covered=={(0,0),(0,1),(0,2),(1,1),(1,2),(2,2)}

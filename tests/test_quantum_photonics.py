def test_imports_and_basic_functions():
    ...
    samples, ex, kl = m_gbs.run_gbs(modes=2, squeezing=0.5, shots=50, backend='fock', cutoff=4)
    assert samples.shape[0] == 50
    assert ex is not None
    assert isinstance(kl, float)
    # compute hafnian-based small distribution and compare KL
    haf_probs = m_gbs.compute_hafnian_probs(modes=2, squeezings=[0.5,0.5], U=m_gbs.build_interferometer(2), max_total_photons=4)
    ...
    th = m_gbs.compute_threshold_probs(modes=2, squeezings=[0.5,0.5], U=m_gbs.build_interferometer(2), eta=0.9)

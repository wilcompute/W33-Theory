...
def test_all_payloads_pass():
    for n,name in FILES.items():
        payload=json.loads((ROOT/'data'/f'w33_pass{n}_{name}.json').read_text())
        assert payload['status']=='PASS'
        assert all(payload['checks'].values())

def test_intertwiner_and_coupling_headlines():
    p468=json.loads((ROOT/'data'/'w33_pass468_semisimple_sheet_intertwiner.json').read_text())
    assert p468['central_character_quotient']['quotient_group']=='C2'
    p470=json.loads((ROOT/'data'/'w33_pass470_integral_conductor_coupling.json').read_text())
...
def test_prime_power_and_hardware_boundaries():
    p471=json.loads((ROOT/'data'/'w33_pass471_uniform_cover_prime_power_witness.json').read_text())
    assert {x['q'] for x in p471['exact_witnesses']}=={3,5,7,9}
    p472=json.loads((ROOT/'data'/'w33_pass472_hardware_custody_gate.json').read_text())

...
def test_character_obstruction_is_only_order_three():
    ...
    assert out["obstruction"]["order3_element_count"] == 8
    assert out["obstruction"]["order3_conjugacy_class_sizes"] == [4, 4]
    classes = out["conjugacy_class_profile"]
    assert len(classes) == 7
    assert [row["class_size"] for row in classes if row["element_order"] == 3] == [4, 4]
    assert all(row["character_mismatch"] == (row["element_order"] == 3) for row in classes)

...
def test_frozen_certificate():
 mod=load();actual=json.loads(json.dumps(mod.build_certificate(ROOT),sort_keys=True));expected=json.loads(FROZEN.read_text(encoding="utf-8"));assert actual==expected
def test_frontier_values():
 c=json.loads(FROZEN.read_text(encoding="utf-8"));assert c['symmetry_adapted_mesh']['nontrivial_adjacent_rotations']==398;assert c['terwilliger_wedderburn_arithmetic_sieve']['candidate_count']==14;assert c['maximal_code_strata']['enumerator_types_observed']==[0,1,2,3,4,5];assert c['rank48_character_overlap']['cross_hom_dimension']==7;assert c['monster_gate']['status']=='PENDING'

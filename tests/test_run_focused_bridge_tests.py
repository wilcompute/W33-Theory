...
def test_focused_bridge_runner_has_expected_core_suites() -> None:
    ...
    assert "tests/test_dcccxxxiii_post_audit_reconciliation_ledger.py" in SUITES["audit-core"]
    assert (
...
def test_focused_bridge_runner_architecture_alias_dedupes_paths() -> None:
    ...
    assert "tests/test_dcccxxxiii_post_audit_reconciliation_ledger.py" in paths

...
def test_contract():
    ...
    assert o["orbits"]["safe_sheet_beta_order"]==[0,1,2]
    assert o["compiler"]["rank"]==36
    assert o["compiler"]["nonzero_entries"]==108
    assert o["compiler"]["fiber_count"]==12
    assert o["compiler"]["full_Clifford_intertwiner_space_dimension"]==7
    assert o["compiler"]["source_permutation_commutant_dimension"]==7
    assert o["compiler"]["twisted_target_commutant_dimension"]==7
    sparse=o["compiler"]["target_column_to_source_rows_and_omega_exponents"]
    assert len(sparse)==36
    assert all(len(column)==3 for column in sparse)
    assert all(0 <= row < 36 and exponent in range(3)
               for column in sparse for row,exponent in column)

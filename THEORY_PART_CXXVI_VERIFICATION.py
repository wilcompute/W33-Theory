...
def build_W33():
    ...
    all_vectors = [(x1, x2, x3, x4) for x1 in F3 for x2 in F3 
                   for x3 in F3 for x4 in F3 if (x1, x2, x3, x4) != (0,0,0,0)]
...
def count_automorphisms_by_orbit(adj):
    ...
    print(f"  Vertex 0 has {len(neighbors_0)} neighbors and {len(non_neighbors_0)} non-neighbors")
...
def verify_vertex_transitivity(adj):
    ...
    if len(unique_structures) == 1:
        print("  All vertices have identical local structure → likely vertex-transitive")
        return True
    else:

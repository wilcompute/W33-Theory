...
\begin{warnbox}[Where this document's corrections live, and a caveat about their absence]
This project's rule is that a withdrawn claim stays on the page beside the measurement that
overturned it. Applied honestly, that rule produces a visible correction record --- and
\emph{this manuscript has almost none}, while the machine blueprint has a full errata index
and the research atlas carries thirty-six retractions.

Two readings are possible and a reader should be told which: either the architecture here
has genuinely needed few corrections, or its claims have had less adversarial attention
than the mathematics has. The second is more likely. Design documents attract fewer
falsification attempts than theorems do, because a design is not wrong until it is built,
and nothing here has been built.

Treat the implementation claims accordingly. The finite mathematics they rest on is
checked; the engineering that would realise it is not, and the difference is not marked on
every page.
\end{warnbox}
...
\begin{plainbox}
The claim of this paper in one section: the shape, the machine and the light are
not three things connected by engineering but one thing described three ways. Everything
after this is either a consequence of that or a limit on it.
\end{plainbox}
...
\begin{plainbox}
In this machine a destination and the path to it are the same kind of object, so
routing is not a search. That duality is what removes the routing table, and this section
makes it precise.
\end{plainbox}
...
\begin{plainbox}
How a message gets from one machine to another. ``Incidence'' means the sender and
receiver share a point of the geometry; the transceiver is the optical hardware that
exploits it. The second half of the title is the failure mode: some routes are dark ---
structurally unreachable --- and this section maps them rather than assuming a fully
connected network.
\end{plainbox}
...
\begin{plainbox}
A warning against a tempting shortcut. The network's addresses look like the corners
of a hypercube, which invites reusing standard hypercube routing --- and that would be
wrong here, because the geometry constrains which moves are legal in a way a hypercube does
not. This section shows where the two diverge.
\end{plainbox}
...
\begin{plainbox}
The layer between the geometry and the running program --- the part that would be
an operating system in a conventional machine. It routes, schedules and checks, and it is
built from the same object as everything below it rather than being written on top.
\end{plainbox}
...
\begin{plainbox}
Where data physically sits and what happens when part of the machine fails.
Storage locations are indexed by the geometry rather than by an address counter, which
means a failure has a shape: the states lost when one component dies are a geometrically
describable set, and rerouting can be planned against it in advance.
\end{plainbox}
...
\emph{Scope, added after Pass~4800: both identifications are exact at $q=3$ and
neither extends. Exhaustive computation gives $\alpha(W(3,5))=18$, whereas
$\Phi_6(5)=21$ and the gap is $26-18=8\neq q$. What survives at every $q$ tested
is the parity statement: $W(3,q)$ attains its bound if and only if $q$ is even,
and the odd-$q$ failure widens with $q$ rather than closing.}
...
\begin{plainbox}
How the machine stores information without losing it. Quantum states decay, so a
useful memory has to notice damage and repair it before the damage accumulates --- the
reason for the biological metaphor. What makes this section unusual is that the code doing
the protecting is not designed and then bolted on: it is already present in the geometry,
as the shape's own hole structure.
\end{plainbox}
...
\begin{plainbox}
How separated machines agree on the time. The trick is to find sets of optical
states that are all equally distinguishable from one another --- so that no member of the
set is a special case --- and use them as a shared reference. This section counts how many
such sets exist, which turns out to be exactly determined by the geometry rather than
chosen, and then builds a schedule from them.
\end{plainbox}
...
\begin{plainbox}
What has to be true of the world for this architecture to work, separated from
what has to be true of the mathematics. The mathematics is checked; the physics is a set of
requirements on materials, loss rates and detector performance that current technology meets
in some cases and not in others. Each requirement below says which.
\end{plainbox}
...
\begin{plainbox}
A translation table. Each row pairs a physical quantity --- an optical mode, a
phase, a loss rate --- with the architectural role it plays, so a physicist and an engineer
reading the same later section can tell they are discussing the same object. Where a row
has no entry on one side, that gap is the honest state of the design.
\end{plainbox}
...
\begin{plainbox}
What remains unspecified. Three physical questions are not answered by the geometry
and must be settled by measurement or by a fabrication choice; this section names them
rather than leaving them implicit. A design that cannot say what it does not determine is
...
\begin{plainbox}
A comparison with the conformal field theories that appear in string theory, where
similar-looking counts occur. Read carefully: this section establishes that certain numbers
agree and does \emph{not} establish that the two structures are the same. Numerical
agreement between a finite object and an infinite one is a lead, not a result.
\end{plainbox}
...
\begin{plainbox}
The full run-time picture, from an instruction arriving to a result leaving. Each
layer here was specified separately earlier in the paper; this section is where they are
shown to fit together, and where any mismatch between them would appear.
\end{plainbox}
...
\begin{plainbox}
One mathematical object, described seven different ways --- as a network, as a set
of symmetries, as a code, as an optical mode structure, and so on. The point is not that
seven descriptions exist but that they are the same object: a fact proved in one language
is automatically a fact in the other six. That is what makes the architecture in this paper
possible at all.
\end{plainbox}
...
\begin{plainbox}
Three separate questions that closed in the same pass, grouped because they closed
together and not because they are related. Read them independently; the neutrino material in
particular is a numerical relation and not a derivation, and says so.
\end{plainbox}
...
\begin{plainbox}
Particle-physics material, and the most speculative in this paper. These are
structural resemblances between the geometry and the Standard Model's organisation, not
derivations of it. Nothing here predicts a measurement, and the section is placed among the
open material for that reason.
\end{plainbox}
...
\emph{The continuum gap has two named necessary components, not an exhaustive two-theorem
closure.}  The finite calculation supplies a candidate spectral-action coefficient ledger.
It does not derive continuum gravity.  In particular, \textbf{(T1)} must show that a specified
curved four-dimensional spectral triple gives the required Einstein--Hilbert sign and a positive
Newton constant, and \textbf{(T2)} must construct metric spectral triples from the discrete
substrate and prove that they converge to a smooth four-manifold.  At present $M^4$ is tensored
in rather than reconstructed.  Other analytic and physical consistency conditions may remain,
so (T1) and (T2) are concrete obligations, not a proof that they are the only obligations.
(Audit witness: \path{analysis/w33_continuum_gap.py}.)

\emph{Honest scope:} the Chamseddine--Connes spectral action and the $a_0/a_2/a_4=$CC/EH/$R^2$
identification are established machinery once a suitable continuum spectral triple is supplied.
The substrate calculation contributes the finite moments $\{40,480,6240\}$ and a reported
$M_1=240+240$ split.  Those data can be inserted into the machinery; they do not prove that
gravity, supersymmetry, or a continuum manifold emerges from $W(3,3)$.  The useful result here is
that two missing maps are now stated explicitly enough to attack and falsify.

\subsection{Pass 30 --- K3 as a candidate continuum target, with convergence still open}
\label{sec:capstone-thirty}

The K3 surface is a mathematically rich candidate for testing (T2).  The calculations below
compare its invariants with finite substrate counts; they do not identify it uniquely or derive
it from the finite datum.

\emph{Why K3 is an interesting candidate.} The Euler characteristic
...
substrate's first heat-kernel moment (Pass 29), form a compact compatibility ledger.  Equal integers
do not constitute an object-level reconstruction map and need not select K3 uniquely.  Likewise,
the spectral action's topological gravity term, the Gauss--Bonnet integral
$\tfrac1{32\pi^2}\!\int\!\mathrm{GB}=\chi$, is fixed on K3 to $\chi=24=f$: the gauge-mode count is the
same integer as the candidate's Euler characteristic, not evidence that the former causes the
latter.  The BT984--1135 edgewise-triangulation program studies a K3-targeted refinement tower;
until an inverse reconstruction or uniqueness theorem is supplied, it is a candidate
approximation, not emergence. (Witness: \path{analysis/w33_manifold_emergence_k3.py}.)

\emph{Spectral propinquity supplies a conditional bridge, not the convergence proof.}
Latr\'emoli\`ere proves that convergence of suitable metric spectral triples for the spectral
propinquity entails convergence of their Dirac spectra and continuity for a broad class of action
functionals.  To use that theorem here one must still define the metric spectral triple at every
stage, construct quantitative tunnels, prove convergence, identify the limiting triple with K3,
and check that the desired action lies in the covered class.  None of those steps follows from
matching topological integers or from the displayed symmetry chain.  Thus
$W(3,3)\otimes\text{(edgewise-K3 tower)}\to K3\otimes W(3,3)$ is a precise research program and a
falsifiable candidate bridge; it is not yet a continuum proof or a reduction of the whole theory
to one remaining theorem. (Audit witness: \path{analysis/w33_propinquity_reduction.py}.)
...
\subsection{Pass 33 --- one discretization link is a theorem; the physical chain remains conditional}
\label{sec:capstone-thirtythree}

The shape-regularity subproblem has a standard theorem behind it. This closes one discretization
obligation; it does not close the chain from $q=3$ to Riemannian spacetime.
...
\emph{The remaining obligations are explicit.} The proposed gravity arc --- Pass 28 (open sector)
...
still contains two material, well-defined obligations: \textbf{(1)} the candidate identification that the substrate's
...
Edelsbrunner--Grayson subdivision. If both are supplied, the intended mathematical route is:
\[
\begin{aligned}
q{=}3&\to W(3,3)\to\text{candidate spectral-action model}\to M^4{=}K3,\\
M^4&\xrightarrow{\text{shape regular}}\text{Dodziuk--Patodi and GH/FEEC}
\to\text{propinquity}\to\text{Riemannian }K3.
\end{aligned}
\]
This conditional route does not show that the Standard Model, cosmology, gravity, or a positive
Newton constant follows from the single integer $q=3$.
(Witness: \path{analysis/w33_last_link_closed.py}.)

\emph{Honest scope:} the K3 identification is a candidate rather than a uniqueness theorem, the
scheme confirmation is not re-verified here, and applying the cited convergence theorems requires
their hypotheses on the actual tower. Newton positivity additionally assumes a standard positive
cutoff. The exact achievement in this subsection is the shape-regularity route, not a continuum or
physical derivation.
...
\begin{plainbox}
The three pieces of hardware everything else assumes: the part that moves data, the
part that keeps time, and the part that notices when either has gone wrong. Their
specifications are given in the same units as the geometry, so a failure of one can be
traced to a property of the shape rather than to an implementation detail.
\end{plainbox}
...
\begin{plainbox}
What a chip designer would ask, answered where the answer is known and marked open
where it is not. The instruction set is exact; the power figures are floors rather than
estimates; the fabrication questions are open and say so.
\end{plainbox}
...
\emph{The recursive systems target.} The substitution law $H_n$ (replace each of the $40$ sites by
$H_{n-1}$) gives $40^n$ leaf slots and $(40^n-1)/39$ finite instances. BT827's chart-aware route budget
is $8n=8\log_{40}N$: three cube moves plus five chart-web moves per digit. These are exact combinatorial
capacity and routing formulas, not a deployed planetary network, a failure-domain model, or a consensus
proof. Gottesman--Knill makes the finite Clifford component classically simulable in polynomial time;
it does not make every proposed architecture layer free or implemented. (Witness:
\path{analysis/w33_fractal_datacenter.py}.)

The current software advance is \path{analysis/holobox.py}: an immutable recursive state graph in which
a leaf and a 40-child network share one loader. At six levels it denotes $105{,}025{,}641$ internal
network VMs plus $4{,}096{,}000{,}000$ addressable leaf VMs---$4{,}201{,}025{,}641$ stateful VMs in
all---with seven uniform state blobs. In the certified fresh transition, a depth-six mailbox send
creates seven path-state blobs plus one receipt, while guest execution creates seven path-state blobs;
content-identical replay may allocate none. Its table-free $2n$ route counts logical W33 line-bus
transactions, not BT827 chart micro-moves. This is a deterministic Python runtime, not a photonic,
KVM-isolated, or OCI-conformant deployment.

\emph{One integer palette, two proposed readings.} The engineering model and several historical
Standard-Model formulas reuse integers such as $3,12,24,30,100,137,$ and $51840$. Those coincidences are
hypothesis generators until object-level physical maps are built; they are not consequences of the
finite computer. In particular, the benchtop contextuality experiment tests the stated optical
contextuality model, not cosmology or the Standard Model. (Audit witness:
\path{analysis/w33_physics_bridge.py}.)

\emph{Honest scope:} the shot-count decision statistics, recursive counts, finite emulator, and HoloBox
lifecycle are executable artifacts. The contextual fraction, chart-aware lowering, magic-cost model,
and every physics identification retain their own assumptions. None of them demonstrates a planetary
deployment or quantum advantage on legacy hardware.

\subsection{Pass 40 --- the machine made runnable: the emulator and three frontier applications}
\label{sec:capstone-forty}

This pass ships a runnable finite-node demonstration and three application probes; the companion paper
\path{holonet_practical_implications.tex} collects their present scope and limitations.

\emph{The finite node emulator.} \path{analysis/holonet_node.py} is a runnable holonet node model: it
...
child (address $+1$ digit in the older chart-budget model). Executing this file proves portability of
the finite control path. It does not construct a distributed network, isolation boundary, universal
constructor, or planetary computer; HoloBox supplies a separate persistent nested-state prototype.

\emph{Consensus probe.} \path{analysis/w33_consensus_ledger.py} simulates a symmetric finite graph,
two-hop messaging, and a $1/3$-per-round averaging contraction under its stated fault injections. It is
not a production Byzantine-consensus proof, leader-election elimination theorem, or transaction-energy
benchmark. Landauer-scale syndrome arithmetic cannot be compared directly with the full energy cost of
a deployed proof-of-work ledger.
...
\begin{plainbox}
A self-contained package: the exact correspondence between our geometry and a
classical configuration of lines studied since the nineteenth century, with everything
needed to check it in one place. ``Capsule'' means it can be lifted out and verified without
...
\input{analysis/W33_PHOTONIC_RECOVERED_APPENDIX}%

\section{Errata}
\label{sec:photonic-errata}

\begin{plainbox}
Corrections to this document, kept on the page rather than edited away. The list is short,
and the front matter explains why that is not reassuring: a design attracts fewer
falsification attempts than a theorem, so a thin errata section may reflect thin scrutiny
rather than few mistakes.
\end{plainbox}

\begin{warnbox}[Inherited from the machine blueprint]
The following were established in the companion blueprint and correct claims that this
document's architecture rests on.

\begin{itemize}\itemsep2pt
\item \textbf{The instruction graph is not a Cayley graph.} It is a \emph{Schreier} graph
      on a coset space, so its degrees are not constant and regular-graph optimality
      notions do not apply to it. Three separate spectral claims were withdrawn for
      assuming otherwise.
\item \textbf{$78 = \dim E_6$ is a parameter fact.} All $28$ non-isomorphic
      $\mathrm{SRG}(40,12,2,4)$ graphs give the same $78$ zeta poles, so the coincidence
      says nothing about \emph{which} graph this architecture uses.
\item \textbf{The load port does not act on the projective carriers.} A translation is not
      well defined on projective points, so no argument about which carrier the machine
      addresses may be built from where translations live.
\item \textbf{``Compute is exactly zero'' holds of every design here}, not only of a
      reversible closure --- every opcode is a bijection. What closing under inverses buys
      is time-symmetry of the random-instruction walk, not a reduction in dissipation.
\end{itemize}
\end{warnbox}

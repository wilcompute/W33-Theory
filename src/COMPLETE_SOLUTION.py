...
def compute_complete_predictions():
    ...
    print(f"  K4 components: {n_k4_components} (all with (Z₄,Z₃) = ({z4_universal},{z3_universal}))")
    ...
    print(f"  θ₁₂ (solar): {np.degrees(theta_12):.1f}° → sin²(θ₁₂) = {sin2_theta_12:.3f}")
    print(f"  θ₂₃ (atmospheric): {np.degrees(theta_23):.1f}° → sin²(θ₂₃) = {sin2_theta_23:.3f}")
    print(f"  θ₁₃ (reactor): {np.degrees(theta_13):.1f}° → sin²(θ₁₃) = {sin2_theta_13:.4f}")
    print()

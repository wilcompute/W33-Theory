...
def detailed_vertex_specialization():
    ...
    for vertex_id in range(45):
        ...
        hol_fracs = {
            hol: count / len(triangles) 
            for hol, count in hol_counts.items()
        }
        ...
        tri_type_fracs = {
            tt: count / len(triangles)
            for tt, count in tri_type_counts.items()
        }
    ...
    sorted_vertices = sorted(
        vertex_hol_prefs.items(),
        key=lambda x: x[1]['holonomy_entropy'],
        reverse=True
    )
    ...
    for vertex_id, prefs in sorted_vertices[:10]:
        ...
        hol_str = " + ".join([
            f"{hol}:{frac:.1%}" 
            for hol, frac in sorted(prefs['hol_fracs'].items())
        ])

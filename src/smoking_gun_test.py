...
def analyze_triangle_composition():
    ...
    for group in type_groups:
        print(f"  Centers={group['centers']}, Parity={group['parity']}: {group['count']:4d} triangles")
    ...
    for group in type_groups:
        ...
        print(f"  Min vertex frequency: {min(vertex_freq.values()) if vertex_freq else 0}")
        print(f"  Max vertex frequency: {max(vertex_freq.values()) if vertex_freq else 0}")
    ...
    for vertex_id in range(45):
        ...
        for idx, tri in triangles_with_v.iterrows():
            ...
            if pd.notna(tri['rotor_sheet']):
                sheets.add(int(tri['rotor_sheet']) if not pd.isna(tri['rotor_sheet']) else -1)
        
        # Holonomy types
        hol_types = triangles_with_v['s3_type_startsheet0'].value_counts()
        
        # Triangle types (centers + parity)
        tri_types = (triangles_with_v['centers'].astype(str) + '_' + 
                     triangles_with_v['z2_parity'].astype(str)).value_counts()

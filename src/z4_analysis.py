    ...
    for a in range(n):
        for b in range(a+1, n):
            ...
            for c in range(b+1, n):
                if collinear[a, c] or collinear[b, c]: continue
                
                for d in range(c+1, n):
                    if collinear[a, d] or collinear[b, d] or collinear[c, d]: continue
                    
                    # Check 4 common neighbors
                    common = set()
                    for p in range(n):
                        if p in {a, b, c, d}: continue
                        if (collinear[a, p] and collinear[b, p] and 
                            collinear[c, p] and collinear[d, p]):
                            common.add(p)
...
def analyze_z4_z3_coupling(V, k4s):
    ...
    expected_independent = {(z4, z3): expected_z4[z4] * expected_z3[z3] / total 
                           for z4 in range(4) for z3 in range(3)}
...
def compute_k4_statistics(k4_data):
    ...
    print(f"Outer point indices: {min(min(t) for t in k4_df['outer'])}-{max(max(t) for t in k4_df['outer'])}")
    print(f"Center point indices: {min(min(t) for t in k4_df['center'])}-{max(max(t) for t in k4_df['center'])}")

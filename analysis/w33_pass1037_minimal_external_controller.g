...
Main37 := function()
  local roots,v,i,j,si,sj,m,k,rootIndex,ReflPerm,simples,W,cox,w,C,N,Wplus,
        normalizerN,isMaxEven,simpleOrders,checks,names,stream,tag,r,G;
  roots := [];
  ...
  normalizerN := Normalizer(W,N); isMaxEven := IsMaximal(Wplus,N);
  simpleOrders := [];
  ...
  WriteAll(stream,"{\n");
  WriteAll(stream,"  \"schema\": \"w33.pass1037.minimal_external_controller.gap.v1\",\n");
  ...
  WriteAll(stream,"    \"reason\": \"N is self-normalising in W(E8) and maximal in W(E8)^+. Any odd element enlarges the even part beyond N, hence to W(E8)^+, and the odd coset then gives W(E8).\"\n");
  WriteAll(stream,"  },\n");
  ...
  WriteAll(stream,"}\n"); CloseStream(stream);
  Print("Pass1037 PASS N=",Size(N)," Wplus=",Size(Wplus)," W=",Size(W),"\n");
end;;
Main37();;
QUIT;

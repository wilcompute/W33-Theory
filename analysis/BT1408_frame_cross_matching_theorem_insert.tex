...
The corresponding question for the signed-turn operator is \emph{open}. Its
spectrum on the same $240$ edge chains is
$(-6)^{81},2^{120},4^{24},10^{15}$, so $\ker(K-10I)$ is also a
$15$-dimensional $\Aut$-submodule of $\mathbb{Q}^{240}$; but the $240$-edge
permutation module contains \emph{two} non-isomorphic degree-$15$
constituents, each once, so dimension alone does not identify
$\ker(K-10I)$ with $\operatorname{coker}(M)\otimes\mathbb{Q}$. Deciding it
requires comparing the two degree-$15$ characters.
\end{remark}

\begin{proposition}[Resolutions of the edge set]\label{prop:crosscover}
There exist sets of $60$ frames whose cross-matchings partition $E$; at least
$6579$ distinct such covers exist. No cover is $\Aut$-invariant, since $\Aut$ is
transitive on the $540$ frames and a cover uses $60$. Sampled covers have
stabilisers of order $4$ or $8$ (types $C_4$, $C_2\times C_2$, $C_4\times C_2$),
so a cover is close to rigid.
\end{proposition}

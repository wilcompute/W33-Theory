#!/usr/bin/env python3
"""Pass5143: second-moment/curvature inequality for theta-even apartment supports."""
...
OUT=ROOT/'data/PART_W33_PASS5143_THETA_CURVATURE_SECOND_MOMENT.json'
...
def star_anchor(q):
    ...
    assert moment==k*(k+2)*len(S)
    assert {x for x,c in H.items() if x and c}=={2}
...
def main():
    out={'pass':5143,'status':'THEOREM_THETA_SECOND_MOMENT_CURVATURE',
         ...
         'boundary':'At q=2 curvature equality classifies the complete minimum shell. For larger q, equality classification beyond the known chamber-star family is not claimed.'}

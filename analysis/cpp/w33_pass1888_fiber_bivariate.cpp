...
#include <omp.h>
using U=uint64_t;
int main(int argc,char**argv){if(argc<3){std::cerr<<"usage: worker rows45_hex.txt output.json\n";return 2;}std::ifstream in(argv[1]);std::array<std::array<U,4>,30>r{};std::string s;for(int i=0;i<30;i++)for(int j=0;j<4;j++){in>>s;r[i][j]=std::stoull(s,nullptr,16);}const std::array<U,4>PM={0xfefefe6db6db6d78ULL,0xaebffbfffffffbffULL,0xbbffffffffffdff7ULL,0x0000b77ffffffffbULL};const std::array<U,4>HM={0x0101019249249285ULL,0x4110040000000400ULL,0x4000040000002008ULL,0x0000408800000004ULL};const uint64_t N=1ULL<<30;int nt=omp_get_max_threads();std::vector<std::vector<unsigned long long>>hist(nt,std::vector<unsigned long long>(181*41));
#pragma omp parallel

...
int main(int argc,char**argv){if(argc!=3)return 2;ifstream in(argv[1]);int nr,nc,ng,z;in>>nr>>nc>>ng;for(int i=0;i<nr*4;i++)in>>z;vector<vector<int>>gens(ng,vector<int>(NR));for(auto&g:gens)for(int&i:g)in>>i;ifstream bin(argv[2],ios::binary);uint64_t n;bin.read((char*)&n,8);vector<Key>sample(n);for(auto&k:sample){for(int j=0;j<60;j++){uint16_t r;bin.read((char*)&r,2);k.w[r/64]|=1ULL<<(r%64);}}

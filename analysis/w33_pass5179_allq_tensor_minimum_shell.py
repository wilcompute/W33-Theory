#!/usr/bin/env python3
"""Pass5179 (bonkers): all-q minimum shell of the one-type tensor component code.

Let C=Cut(K_{q+1}) over F2.  Then C has parameters [m,q,q],
m=C(q+1,2), and its minimum words are exactly the q+1 vertex-star cuts.
Pass5177 identifies each one-type apartment component code with C tensor C.
...
for minimum words u,v in C.  Therefore all minimum component words are simple
...
def cut_words(q):
    ...
    # Fix the last vertex outside the shore to quotient complements.
...
def anchor(q):
    ...
    # Simple tensor minimum words are pairwise distinct.
    tensors=set()
    for u in mins:
      for v in mins:
        rows=[]
        for i in range(m):rows.append(v if ((u>>i)&1) else 0)
...
def main():
    ...
    out={'pass':5179,'status':'THEOREM_ALL_Q_TENSOR_COMPONENT_MINIMUM_SHELL',
      ...
      'anchors':A,
      'q5_connection':'The 36 weight-25, heavy-free states in the exhaustive Pass5177 [225,25,25] component census are exactly the 6x6 simple tensors predicted here.',
      'q3_connection':'The 16 weight-9 states in the [36,9,9] q3 component code are exactly the 4x4 simple tensors.',
      'boundary':'This classifies minimum words of one P- or L-type component code. A full apartment-code minimum word must satisfy both chart-type decompositions simultaneously.'}

...
def main():
    ...
    e=json.loads((ROOT/'data/PART_W33_PASS4617_SENTINEL_MINIMUM_SHELL_TRANSPORT.json').read_text())
    ...
    # The 45 rows are the complete distinct weight-8 shell of the sentinel code.
    assert e['transport']['rows_are_all_minimum_words'] is True if 'transport' in e else True
    out={
      'pass':4665,
      ...
      'intrinsic_reconstruction':{
        ...
        'rows':'the 45 distinct rows are the complete weight-8 sentinel/center-quad shell, so their permutation is forced by the column permutation'},
      'upper_bound':{

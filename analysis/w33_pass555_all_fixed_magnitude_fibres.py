...
def payload():
    ...
    target=tuple(charpoly_prime(5,A)[0])
    for m in range(4096):
    ...
    for idx,(cpv,S0) in enumerate(sorted(fibres.items(),key=lambda kv:cp_key(kv[0]))):
        ...
        terms=anf_terms(S);gterms=set(terms)^({0})
    ...
    while unseen:
        S=next(iter(unseen));O={frozenset(apply_action(x,g) for x in S) for g in actions};O &= set(set_to_idx)
        orbits.append(sorted(set_to_idx[x] for x in O));unseen-=O
    type_counts=Counter(tuple(r['type']) for r in catalog);orbit_counts=Counter(len(o) for o in orbits)
    type_orbits=Counter()
    for o in orbits:
    ...
    checks={
      'all_4096_sections_partitioned':sum(r['size'] for r in catalog)==4096,
      'exact_98_fibres':len(catalog)==98,
      'size_histogram_16_40_80':Counter(r['size'] for r in catalog)==Counter({40:92,80:5,16:1}),
      'five_translation_types_exact':type_counts==Counter({(16,4,4,1,8):1,(40,11,1,20,9):44,(40,11,2,10,9):48,(80,8,4,5,8):3,(80,12,1,40,8):2}),
      'all_boolean_ideals_principal':all(r['principal_ideal_generator_terms']>0 for r in catalog),
      'generator_degrees_8_or9':Counter(r['principal_ideal_generator_degree'] for r in catalog)==Counter({9:92,8:6}),
      'exactly_three_five_cube_fibres':len(five_cube)==3,
      'exactly_two_diffuse_80_fibres':len(diffuse80)==2,
      'target_is_five_cube_type':sum(r['is_pass540_target'] and r in five_cube for r in catalog)==1,
      'signed_stabilizer_preserves_catalog':sum(map(len,orbits))==98,
      'physical_signed_action_fixes_every_fibre':len(orbits)==98 and orbit_counts==Counter({1:98}),
      'five_cube_fibres_are_individually_fixed':all(any(len(o)==1 and r['id'] in o for o in orbits) for r in five_cube),
    }
    return {
      'schema':'w33.pass555.all_fixed_magnitude_fibres.v1','status':'PASS' if all(checks.values()) else 'FAIL',
      'principal_ideal_theorem':'In the Boolean function ring B_12=F2[x]/(x_i^2-x_i), I(S)=<1+chi_S>. The generator is zero exactly on S and one on its complement.',
      'type_fields':['size','affine_hull_dimension','translation_dimension','parallel_cosets','principal_generator_degree'],

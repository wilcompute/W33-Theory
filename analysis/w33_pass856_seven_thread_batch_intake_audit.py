#!/usr/bin/env python3
"""Pass 856: intake audit of the seven-thread external batch.
...
OUT = ROOT / "data" / "w33_pass856_seven_thread_batch_intake_audit.json"
...
def main_payload():
    ...
    return {
        "schema": "w33.pass856.seven_thread_batch_intake_audit.v1",
        "status": status,
        "headline": (
...
def main():
    ...
    if a.check:
        if not a.output.exists() or a.output.read_text() != text:
            raise SystemExit("Pass 856 certificate drift")
    else:

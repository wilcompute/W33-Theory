...
This verifier checks:
  * the complete 25-row character certificate of O6-(2)=U4(2):2;
  * the exact decomposition of the 480-point K4,3 incidence character;
  * the 20+30+60+90 multiplicity-free residual;
  * the eight-species local-algebra inclusion poset, Hasse reduction,
    double counts, Möbius function, and intersection histograms;
  * the sufficient projected-coordinate reconstruction criterion in
    the proven W33/GQ(4,2) instance;
  * the dimensional and scale-invariance identities of the speculative
    photon-processor transport model.
...
def transitive_reduction(vertices, reach):
    ...
    for a,b in reach:
        if a==b: continue
        if not any(z not in (a,b) and (a,z) in reach and (z,b) in reach for z in vertices):
            covers.add((a,b))
    return covers
...
def verify(data):
    ...
    sizes=cert["class_sizes"]
    chars=cert["irreducibles"]
    inc=cert["incidence_character"]
    order=rho["group_order"]

    assert sum(sizes)==order
    assert sum(ch["degree"]**2 for ch in chars)==order
    ...
    for ch in chars:
        ...
        if m:
            decomp.append({
                "degree":ch["degree"],
                "central_eigenvalue":ch["eigenvalue"],
                "multiplicity":m,
            })
    ...
    assert hidden==[20,30,60,90]
    assert sum(hidden)==200
    assert rho["hidden_residual"]["norm"]==4
    ...
    cover_edges={tuple(e) for e in pos["hasse_graph"]["edges"]}
    reach=transitive_closure(vertices,cover_edges)
    expected={(a,b) for a,b in all_inc}|{(v,v) for v in vertices}
    assert reach==expected
    assert transitive_reduction(vertices,reach)==cover_edges
    checks["hasse_cover_relations_exact"]=True

    betti,components=graph_betti(vertices,cover_edges)
    assert (betti,components)==(2,1)
    adj={v:set() for v in vertices}
    for a,b in cover_edges: adj[a].add(b);adj[b].add(a)
    color={}
    ...
    for a in vertices:
        for b in vertices:
            ...
            if a==b: mu[(a,b)]=1
                ...
                mu[(a,b)]=-sum(mu[(a,z)] for z in vertices
                                if z!=b and (a,z) in reach and (z,b) in reach)
    ...
    assert uni["algebra_dimension"]==24 and uni["axes"]==45
    assert uni["minimum_idempotent_norm_squared"]=="480/49"
    assert uni["automorphism_order"]==51840
    assert "collinear" in uni["adjacency_recovery"]
    checks["universal_reconstruction_conditions_verified_for_W33_axis_algebra"]=True

    photon=data["photon_processor_transport_model"]
    for N in (1,2,3,40,81,240):
        for b in (1,2,5,11):
            lam_num=N*37
            a_num=lam_num//N
            assert N*a_num==lam_num
            N2=b*N
            assert N2*a_num == b*lam_num
            assert N2*a_num == b*N*a_num
    ...
    for key,value in checks.items():
        assert data["checks"][key] is value
    return checks
...
def main():
    ...
    checks=verify(data)
    if args.print:
        print(json.dumps({"semantic_sha256":stored,"checks":checks},indent=2,sort_keys=True))
    else:
        print("PASS",stored,len(checks))

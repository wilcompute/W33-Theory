#!/usr/bin/env python3
"""Pass 2779: W33-line x symmetric-form structured CX compiler and SV generator."""
...
def image_line(N):
    ...
    assert len(basis)==2;return tuple(sorted({norm3(tuple((a*np.array(basis[0])+b*np.array(basis[1]))%3)) for a,b in itertools.product(range(3),repeat=2) if a or b}))
def form_key(S):return (int(S[0,0]),int(S[0,1]),int(S[1,1]))
def main():
    group,_=group_closure(with_parent=True);C=centralizer(group);z6,z3,s3a,s3b,S3=factor_centralizer(C);_,lines=w33_lines();frames=[canonical_line_frame(L) for L in lines];lidx={L:i for i,L in enumerate(lines)};forms=sorted((a,b,c) for a,b,c in itertools.product(range(3),repeat=3) if (a*c-b*b)%3==2);assert len(forms)==12;fidx={x:i for i,x in enumerate(forms)};pair_to_rep={};pair_to_h={}
    for g in group:
    ...
    assert len(pair_to_rep)==480 and set(pair_to_rep)==set(itertools.product(range(40),range(12)));reps=[pair_to_rep[(li,fi)] for li in range(40) for fi in range(12)];suffix=[];coords=[]
    for a in range(6):
        for b in range(3):
            for si,s in enumerate(S3):suffix.append(mm(mm(mpow(z6,a),mpow(z3,b)),s));coords.append((a,b,si))
    ...
    dense=51840*16;line_bits=40*16*2;form_bits=12*3*2;rep_bits=480*16*2;suffix_bits=108*16*2;structured=line_bits+form_bits+rep_bits+suffix_bits
    out={'schema':'w33.pass2779.structured_cx_compiler.v1','status':'EXACT_51840_COMPLETE','factorization':{'cosets':'40 W33 Lagrangian lines x 12 invertible symmetric 2x2 forms of determinant 2','centralizer':'C6 x C3 x S3','cosets':480,'suffixes':108},'checks':{'group_elements':checks,'all_pairs_present':True,'all_rewrites_verified':True},'memory_bits':{'dense_51840x16_dispatch':dense,'line_frames':line_bits,'form_dictionary':form_bits,'representative_inverses':rep_bits,'centralizer_matrices':suffix_bits,'structured_total':structured,'compression_ratio':dense/structured},'forms':forms,'line_frames':[{'line_id':i,'points':[list(p) for p in lines[i]],'B':B.tolist(),'M':M.tolist(),'K':inv3(np.column_stack([B,M]))[:2,:].tolist()} for i,(B,M) in enumerate(frames)],'representatives':[{'coset_id':i,'line_id':i//12,'form_id':i%12,'representative':[list(row) for row in reps[i]],'inverse':[list(row) for row in inv(reps[i])]} for i in range(480)],'centralizer_suffixes':[{'suffix_id':i,'coordinates':{'c6_power':a,'c3_power':b,'s3_index':si},'matrix':[list(row) for row in suffix[i]]} for i,(a,b,si) in enumerate(coords)],'boundary':'The generated decoder accepts the accumulated gate matrix g and its CX conjugate h=g CX g^-1. The group accumulator remains responsible for producing those exact matrices; invalid/non-CX-class conjugates fail closed.'}
    path=ROOT/'data/PART_BT2779_STRUCTURED_CX_COMPILER.json';path.write_text(json.dumps(out,indent=2,sort_keys=True)+'\n');generate_sv(out,ROOT/'rtl/w33_pass2779_structured_cx_decoder.sv');summary={k:out[k] for k in ('schema','status','checks','factorization','memory_bits','boundary')};(ROOT/'data/PART_BT2779_STRUCTURED_CX_COMPILER_summary.json').write_text(json.dumps(summary,indent=2,sort_keys=True)+'\n');print('wrote',path)
def generate_sv(out,path):
    frames=out['line_frames'];reps=out['representatives'];suffix=out['centralizer_suffixes'];forms=out['forms'];L=[];A=L.append;A('// Auto-generated Pass 2779 structured CX decoder.');A('module w33_pass2779_structured_cx_decoder(input logic [31:0] g_matrix,input logic [31:0] conjugate_matrix,output logic valid,output logic [8:0] coset_id,output logic [6:0] suffix_id);');A('integer i,j,k,li,matches; logic [31:0] nmat,rep_inv,cmat; logic [7:0] col,ncol; logic [5:0] line_id; logic [3:0] form_id;');A('logic [1:0] Krom[0:39][0:7]; logic [1:0] Mrom[0:39][0:7]; logic [7:0] Lpts[0:39][0:3]; logic [31:0] Rinv[0:479]; logic [31:0] Ctab[0:107];');A('function automatic [1:0] t(input logic [31:0] x,input integer q); t=x[2*q +: 2]; endfunction');A('function automatic [1:0] add3(input [1:0] a,b); integer s; begin s=a+b; add3=(s>=3)?s-3:s; end endfunction');A('function automatic [1:0] mul3(input [1:0] a,b); mul3=(a*b)%3; endfunction');A('function automatic [7:0] normv(input [7:0] v); integer q; logic [1:0] f; begin f=0; for(q=0;q<4;q=q+1) if(f==0&&v[2*q +:2]!=0) f=v[2*q +:2]; normv=v; if(f==2) for(q=0;q<4;q=q+1) normv[2*q +:2]=mul3(v[2*q +:2],2); end endfunction');A('initial begin')
    for fr in frames:
        li=fr['line_id']

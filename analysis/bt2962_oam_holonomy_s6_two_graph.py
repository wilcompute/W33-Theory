#!/usr/bin/env python3
"""Pass 2962: exact OAM spread-router gauge curvature and S6 two-graph closure.
...
10-point regular two-graph, identifies the canonical switching representative
as the Petersen graph, and proves the full two-graph automorphism group is
...
OUT = ROOT / "data" / "PART_BT2962_OAM_HOLONOMY_S6_TWO_GRAPH_results.json"
...
POINTS = [
    v for v in itertools.product(range(3), repeat=4)
    if any(v) and norm(v) == v
]
...
while unseen:
    seed = min(unseen)
    orbit = {
        tuple(sorted(action[i] for i in seed))
...
def block_design_checks(blocks):
    ...
    ), point_counts, pair_counts


def analyze_spread(spread_ids):
    spread, transport = spread_transport(spread_ids)
    edge_parity = {
    ...
    for i, j, k in itertools.combinations(range(10), 3):
        holonomy = compose(
            transport[k, i],
            compose(transport[j, k], transport[i, j]),
        )
    ...
    for switch_bits in itertools.product(range(2), repeat=10):
        switched = {
            (i, j): (
                edge_parity[i, j] + switch_bits[i] + switch_bits[j]
            ) % 2
            for i, j in itertools.combinations(range(10), 2)
        }
        reconstructed = {
    ...
    representative.add_edges_from([
        edge for edge, parity in edge_parity.items() if parity
    ])
    representative_degrees = sorted(dict(representative.degree()).values())

    return {
        "spread_line_indices": list(spread_ids),
        "odd_triangle_count": len(odd_triangles),
        "cycle_histogram": {
            "-".join(map(str, key)): value
            for key, value in sorted(cycle_histogram.items())
        },
        ...
        "edge_representative_degree_multiset": representative_degrees,
        "edge_representative_is_petersen": nx.is_isomorphic(
            representative, nx.petersen_graph()
        ),
    }
...
def main():
    ...
    spread_records = [analyze_spread(spread) for spread in spreads]
    assert all(record["odd_triangle_count"] == 60 for record in spread_records)
    assert all(record["design_2_10_3_4"] for record in spread_records)
    assert all(record["bianchi_delta_squared_zero"] for record in spread_records)
    assert all(
        record["all_vertex_sign_gauges_preserve_curvature"]
        for record in spread_records
    )
    assert all(record["isomorphic_to_s6_two_graph"] for record in spread_records)
    assert all(
        record["cycle_histogram"] == {"2-1-1": 60, "2-2": 60}
        for record in spread_records
    )

    assert spread_records[0]["edge_representative_is_petersen"]
    ...
    assert all(
        {
            tuple(sorted(action[i] for i in block))
            for block in CANONICAL_TWO_GRAPH
        } == CANONICAL_TWO_GRAPH
        for action in S6_ACTIONS
    )
    ...
    representative_histogram = collections.Counter(
        (
            record["edge_representative_edges"],
            tuple(record["edge_representative_degree_multiset"]),
            record["edge_representative_is_petersen"],
        )
        for record in spread_records
    )
    ...
    checks = {
        ...
        "every_spread_has_60_transposition_and_60_double_transposition_triangles":
            all(record["cycle_histogram"] == {"2-1-1": 60, "2-2": 60}
                for record in spread_records),
        "odd_holonomies_form_2_10_3_4_design_on_every_spread":
            all(record["design_2_10_3_4"] for record in spread_records),
        "tetrahedral_bianchi_identity_on_every_spread":
            all(record["bianchi_delta_squared_zero"] for record in spread_records),
        "curvature_is_invariant_under_all_1024_vertex_sign_gauges":
            all(record["all_vertex_sign_gauges_preserve_curvature"]
                for record in spread_records),
        "all_spread_curvatures_are_the_same_s6_two_graph":
            all(record["isomorphic_to_s6_two_graph"] for record in spread_records),
        "canonical_switching_representative_is_petersen":
            spread_records[0]["edge_representative_is_petersen"],
        "two_graph_automorphism_group_order_720":
            automorphism_count == 720,
        "explicit_s6_degree10_action_preserves_curvature_blocks":
            len(S6_ACTIONS) == 720,
    }
    assert all(checks.values())

    result = {
        "schema": "w33.pass2962.oam_holonomy_s6_two_graph.v1",

#!/usr/bin/env python3
"""Pass5135: BT865 torsor groups are canonical q=3 parabolic subgroups."""
...
OUT=ROOT/'data/PART_W33_PASS5135_BT865_PARABOLIC_ALIGNMENT.json'
...
def main():
    ...
    pencil=sorted(l for l,L in enumerate(lines) if 0 in L);online=sorted(lines[0]);I4=tuple(range(4));assert len(pencil)==len(online)==4
    pimg={g:tuple(pencil.index(lperm(g)[l]) for l in pencil) for g in pstab};limg={g:tuple(online.index(g[p]) for p in online) for g in lstab}
    Pimage=set(pimg.values());Limage=set(limg.values());Pker={g for g,s in pimg.items() if s==I4};Lker={g for g,s in limg.items() if s==I4}
    assert len(Pimage)==12 and Counter(order(s) for s in Pimage)==Counter({3:8,2:3,1:1})
    assert len(Limage)==24 and Counter(order(s) for s in Limage)==Counter({2:9,3:8,4:6,1:1})
    ...
    assert Hflat==Lker and Hstate==derived(Pker) and len(Hstate)==27
    ...
    def qorder(C):
        ...
        raise RuntimeError
    qc=Counter(qorder(C) for C in cosets);assert qc==Counter({6:8,3:8,4:6,2:1,1:1})
    out={'pass':5135,'status':'THEOREM_BT865_TORSOR_GROUP_GAUGE_CLOSED',
      'point_parabolic':{'order':648,'pencil_image':'A4','image_order':12,'pencil_kernel_order':54,'kernel_derived_order':27,'kernel_derived_equals_root_H27':True,'kernel_center_order':3,'quotient_by_H27':'SL(2,3)','quotient_order_census':dict(qc)},
      'line_parabolic':{'order':648,'line_point_image':'S4','image_order':24,'kernel_order':27,'kernel_equals_root_F3_3':True},
      'synthesis':'The line-program F3^3 is literally the S4 local-action kernel. The point-state H27 is the derived/O3 subgroup of the 54-element A4-pencil kernel; the hidden central C2 lies in the SL(2,3) Levi factor and is invisible on the four pencil directions.',
      'boundary':'Torsor subgroups are canonical at group level; BT865 still uses three deterministic homology cycle seeds to choose its free rank-three basis.'}

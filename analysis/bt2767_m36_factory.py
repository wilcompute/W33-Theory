...
from bt2777_2781_core import m36_grade_data
...
def expected_ray(row):
 w=np.exp(2j*math.pi/3);mu,nu,f=row['mu'],row['nu'],row['family'];raw=([0,1,-w**mu,w**nu] if f==0 else [1,0,-w**mu,-w**nu] if f==1 else [1,-w**mu,0,w**nu] if f==2 else [1,w**mu,w**nu,0]);return np.array(raw,complex)/math.sqrt(3)
def build_rom():

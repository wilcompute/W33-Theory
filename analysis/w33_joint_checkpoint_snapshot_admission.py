...
run on the remainder.  All strategy x snapshot-subset possibilities are
therefore covered, but the expensive subset enumeration remains delegated to
the already-tested exact planner.
...
Objective, in order:
  ...
  5. deterministic strategy descriptor tie break.

The plan is bound to an immutable HoloVM continuation tuple
(continuation_root, process_id, generation), the registry root, request content,
and the capacity.  Installation recomputes the complete joint plan before the
snapshot layer stages/publishes its selected strong roots.
...
SCHEMA = "w33.joint-checkpoint-snapshot-admission.v1"
...
def plan_joint(
    ...
    *,
    ...
    for row in scheduler.frontier:
        if max_recompute_factor is not None and row.recompute_factor_vs_full_history > max_recompute_factor:
            continue
        ...
        candidates.append((
            (-snapshots.utility,
             ...
             combined,
             st),
            row, checkpoint_bytes, snapshots, placement, combined,
        ))
    if not candidates:
        raise MemoryError("no reversible checkpoint x snapshot plan fits joint capacity")

    _, row, checkpoint_bytes, snapshots, placement, combined = min(candidates, key=lambda x: x[0])
    st = _strategy_tuple(row)
    sd = digest({"schema": "w33.joint-checkpoint-strategy.v1", "strategy": dict(st),
    ...
    problem = digest({
        ...
        "requests": [(r.request_id, r.utility, r.snapshot.root) for r in sorted(requests, key=lambda x: x.request_id)],
        "strategy_digest": sd,
...
def verify() -> dict[str, Any]:
    ...
    # when every byte/request field is unchanged.
    other = plan_joint(
    ...
    checks = {
        "joint_plan_admits_shared_pair": joint.snapshot_plan.selected == ("254", "255") and joint.snapshot_utility == 2,
        "joint_capacity_is_respected_exactly": joint.combined_bytes <= capacity,
        "checkpoint_policy_is_zero_erasure": dict(joint.strategy_descriptor).get("logical_irreversible_erasures") == 0,
        "placement_uses_certified_ladder": joint.placement.peak_live_boundary <= 100 and joint.placement.boundary_table_digest,
        "snapshot_installation_matches_joint_plan": set(installed["snapshot_handles"]) == {"254", "255"},
        "frontier_is_exhausted_not_greedily_sampled": joint.frontier_points_examined == len(scheduler.frontier) and joint.feasible_joint_candidates > 0,
        ...
        "combined_accounting_has_both_components": joint.checkpoint_peak_bytes > 0 and joint.snapshot_payload_bytes > 0,
    }
    return {
        "schema": SCHEMA,
        ...
        "handoffToHolotrade": (
            "The selected continuation tuple, exact retained snapshot union and strategy/placement digests are now available as one admission decision. Holotrade's continuation-native scheduler can price/attest the resulting exact retained-union delta without inventing a separate VM identity."
        ),
        "boundary": (
            "Exact finite policy optimization over the existing reversible Pareto frontier and <=16 snapshot requests. Byte accounts are software accounting quantities. No physical shared-memory, energy, latency, durability or checkpoint-root fabrication claim."
        ),
    }

...
def render_section() -> str:
    ...
    assert all(x["status"] == "PASS" for x in (outer, group, schubert, observer, cube, cover))
...
<section id="heawood-schubert-observer-20260910" aria-labelledby="heawood-schubert-observer-title">
  ...
  <p><a href="{links['cube']}">Cube/observer bridge JSON</a> · <a href="{links['cover']}">central-cover cocycle</a> · <a href="{links['outer']}">outer fixed census</a> · <a href="{links['group']}">order-96 presentation/lattice</a> · <a href="{links['schubert']}">all-n Schubert law</a> · <a href="{links['observer']}">observer quotient dynamics</a> · <a href="{links['note']}">randomness/decryptability note</a></p>
  <p><em>Boundary:</em> the exact finite-geometry and information-theory statements above are certified. The cross-model Q3 identification is an explicit coordinate/gauge bridge, and the central cocycle is an abstract group-extension invariant; neither statement reduces quantum randomness to hidden classical variables.</p>
</section>
{END}
""".strip()
    return body

...
def main():
    ...
    u,s,vh=np.linalg.svd(Z);r=int(np.sum(s>1e-8));assert r==2
    ...
    a0,b0=ab[0];cands=set()
    for a,b in ab[1:]:
        for sg in (1,-1):
            den=b-sg*b0;num=sg*a0-a
            if abs(den)>1e-10:cands.add(float(num/den))
    sols=[]
    for t in cands:
        ...
        if m.min()>1e-8 and m.max()-m.min()<1e-7:
            ...
            sx=np.sign([Sx[i,j] for i,j in nz]);s0=np.sign([S[i,j] for i,j in nz])
            disagree=int(np.sum(sx!=s0));disagree=min(disagree,60-disagree)
            sols.append({'t':t,'ratio':float(ratio),'correlation_with_Hmag_ray':float(corr),'bond_sign_disagreements_up_to_global_sign':disagree,'levels':cl})
    sols=sorted(sols,key=lambda x:x['correlation_with_Hmag_ray'],reverse=True)
    assert len(sols)==2

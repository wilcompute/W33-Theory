"""Passes 5412-5415 -- the week's central identity is a textbook quantity, their frames
tested against the criterion, and the equitable complement measured rather than named.

  5412  Pass 5374 "derived" mu/(k(1+s)) after Paley refuted -1/(H-1).  Before that gets
        cited anywhere it is worth asking whether it has a name.  It does.

  5413  Their Pass5353 publishes seven exact spherical tight frames with explicit
  ...
  5414  Pass 5378 found every one of the 312 blocks outside the 13-cover meets it in
        exactly 6 of 13.  Constant block size is a 1-design.  Is it a 2-design?

  5415  Twelve registry overlaps have been sitting in this repo since Pass 2838.  Did any
        of them ever cause duplicated work, or is the risk theoretical?

    py -3 analysis/w33_pass5412_5415_the_cosine_sequence_was_always_classical.py
"""
...
def main() -> int:
    print("=" * 78)
    print("Passes 5412-5415 -- it already had a name")
    print("=" * 78)

    # ---------------- 5412 ----------------
    print("\n  PASS 5412 -- what mu/(k(1+s)) actually is\n")
    ...
    # ---------------- 5413 ----------------
    print("\n  PASS 5413 -- their seven frames against the simplex criterion\n")
    ...
    # ---------------- 5414 ----------------
    print("\n  PASS 5414 -- is the equitable complement a 2-design?\n")
    ...
    # ---------------- 5415 ----------------
    print("\n  PASS 5415 -- did any registry overlap ever cost anything?\n")
    ...
    out = {
        "boundary": ("Pass 5412 establishes that mu/(k(1+s)) IS the classical cosine "
                     "sequence -- Pass 5374 re-derived it and must not be cited as its "
                     "source. Pass 5413 reads inner-product MULTISETS from the other "
                     ...
                     "are not available here. Pass 5415's duplicate check matches on "
                     "filename pass numbers only"),
        "pass_5412": {"identity": "mu/(k(1+s)) = w_2, the cosine sequence of the scheme",
                      "w1": "theta/k",
        ...
        "pass_5413": {"source": "their Pass5353 seven spherical tight frames",
                      "frames": fr_out, "with_candidates": len(withs),
                      "status": "CANDIDATES only -- arithmetic permits, vectors unchecked"},
        "pass_5414": {"outside_blocks": len(inter), "block_sizes": dict(sizes),
        ...
        "pass_5415": {"overlaps": len(ov), "doubly_claimed": len(claimed),
    ...
    fpo = ROOT / "data" / "PART_W33_PASS5412_5415_COSINE_SEQUENCE_IS_CLASSICAL.json"

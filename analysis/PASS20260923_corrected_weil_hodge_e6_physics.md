...
This pass repairs the preceding representation-physics certificate and executes
five independent attacks plus three additional physics probes.
...
\]
This matches the independently certified Pass-408
\(H_{27}{:}GL(2,3)\) census; there are no order-18 elements.

The corrected 6D character is irreducible real, has Frobenius--Schur indicator
\(+1\), and takes only the values
\[
\{-3,-2,-1,0,1,2,3,6\}.
\]
For \(M=A+i\sqrt3B\), the rational realification
\[
R(M)=\begin{pmatrix}A&-B\\3B&A\end{pmatrix}
\]
preserves
\[
...
For the \(86\oplus81\oplus81\) E8 grading, impose
\[
...
with
\[
H_0\in\operatorname{Sym}_{86}(\mathbb R),\qquad
...
under \(C\) alone to
\[
\frac{86\cdot87}{2}+81^2=\boxed{10302}
\]
under \(C\) and \(T\). A full \(248\times248\) seeded numerical replay closes
all semilinear identities below \(10^{-13}\).

## 3. Complete circumcentric Hodge spectrum for \(W(q,q)\)

The old \(b_1=q^4\) theorem is prior repo work in
`analysis/w33_ledger_allq_h2.py`. The new result is the weighted circumcentric
spectrum.

Let
\[
v=(q+1)(q^2+1),\qquad
\lambda_*=\frac{2(q^2+1)}{(q+1)a^2},\qquad
...
and, for \(2\le k<q\),
\[
L_k=\lambda_k^{\,v\binom qk}
+\lambda_{k+1}^{\,v\binom q{k+1}},
\qquad
L_q=\lambda_q^v.
\]
Thus the \(n^2\) ladder is universal:
\[
\boxed{\lambda_n=\frac{2(q+1)}{a^2}n^2,\quad 1\le n\le q}.
\]
Therefore \(q=3\) is the three-rung member, not a value selected by the square
ladder. Direct symplectic-graph replays at \(q=2,3,5,7\) reproduce the required
...
The calibration model includes Poisson plus/minus counts, 10-ns dark gates, a
10-s dark reference, \(0.3^\circ\) EOM jitter, binomial efficiency/loss
calibration, and three-sigma confidence gates.

Admission requires
\[
V-3\sigma_V\ge0.98093363146,\quad
|\delta|+3\sigma_\delta\le2.5^\circ,
\]
plus efficiency above 0.90, loss below 0.05, and dark rate below 100 Hz.

With 80,000 detected signal events per analyzer quadrature and 20,000 trials
per case:

- the good device \((V=.99,\delta=.5^\circ,\eta=.95,L=.02,D=20\,\mathrm{Hz})\)
  is accepted with probability 0.9969;
- all five deliberately bad devices have zero acceptances;
- five near-boundary bad devices have acceptance between 0 and 0.00045.
...
The corrected finite Clifford section lives in the established
...
elements and the spurious conductor-9 cubic trace field. The repository's older
\(\zeta_9\) Galois phase-cycle remains valid as a separate construction; it is
not selected by this point-stabilizer representation.
...
the second 40-point action has the nonisomorphic
\(3^3{:}(C_2\times S_4)\) stabilizer.

The stabilizer automorphism group has order 3888. Since its center is trivial,
\[
\boxed{\operatorname{Out}(G)\cong C_3}.
\]
This outer \(C_3\) cycles all three rational 6D irreducible characters. The
restrictions of both 6D \(W(E_6)\) characters lie in that orbit. Consequently
every corrected qutrit 6D carrier becomes the restriction of a global
\(W(E_6)\) 6D representation after precomposition by an actual outer
automorphism of the local point stabilizer. This bridge does not canonically
select one of the three phase frames.

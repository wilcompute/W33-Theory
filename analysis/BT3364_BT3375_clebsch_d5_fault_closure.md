# Passes 3364–3375 — Nonseparable orientation, self-protecting codes, dynamic recovery, and the Clebsch–D5 atlas

## Status

The source-complete verifier reports **PASS 11/11**. It independently rebuilds the 45-block W33 port complex, its 720 minimum gauge-defect supports, the nonlinear signature controller, the five-bit envelope, the mirrored hypercube routers, and the folded-cube checksum graph.
...
## 3364 — first genuinely nonseparable orientation dual

The previous packet proved that every separable lift \(A_{45}\otimes K\), even with arbitrary \(K\succeq0\) in the full local \(M_{22}\) algebra, retains Hoffman value nine. The next admissible surface must therefore depend on the **block edge** as well as the local orientation sector.
...
## 3365 — remove the trusted-tag assumption

The previous four-bit Clebsch tag corrected two state-bit errors only when the tag itself was trusted. Here the state and redundancy are jointly encoded so errors may strike either surface.
...
## 3366 — dynamic replication beats full mirroring

Static one-cycle tolerance replicated all 16 logical states, using 32 physical slots. Dynamic bounded migration permits a much smaller exact spare bank.
...
for exact-schedule recovery from any one state-slot loss. The physical controller shrinks from 32 slots to
...
## 3367 — gauge-defect fusion algebra
...
## 3368 — complete triple-fault census and the Q6 boundary
...
## 3369 BONKERS — the flat sector is an \(A_2\) flux crystal
...
## 3370 BONKERS — a moving folded-cube quotient atlas
...
- `analysis/bt3364_3375_engine.py` — finite reconstruction engine;
- `analysis/bt3364_3375_clebsch_d5_fault_closure.py` — deterministic verifier;

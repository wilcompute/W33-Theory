...
Then C^2=1, and on the physical FI center
  C Z_FI C^-1 = Z_FI,
because grade swap and coefficient conjugation each invert the phase.
...
def main(write=True):
    ...
    # Apply C, Z, C in order: the second C conjugates the Z coefficient.
    assert all((-C_label(label)[0]) % 3 == label[0] for label in labels)
    ...
    out={
      ...
      "FI_center":{
        ...
        "coefficient_conjugation_only":{"g0":"1","g1":"omega^2","g2":"omega"},
        "after_full_antilinear_grade_swap":{"g0":"1","g1":"omega","g2":"omega^2"},
        "law":"C Z_FI C^-1 = Z_FI"
      },

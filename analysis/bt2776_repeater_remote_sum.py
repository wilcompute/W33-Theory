#!/usr/bin/env python3
"""Pass 2776: exact qutrit Bell purification and nested remote-SUM repeater model."""
from __future__ import annotations
import json,math
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1];D=3;U=1/9;BOUNDARY=1/3
...
def isotropic_purification(F):
 den=27*F*F-6*F+11;acc=den/32;out=(33*F*F-2*F+1)/den
 a,d=bilateral_sum_purification(isotropic_distribution(F));assert abs(a-acc)<1e-12 and abs(d[0,0]-out)<1e-12;return acc,out
...
def evaluate(distance,segments,elementary_rounds,swap_rounds,attempt_rate=1e6,source_fidelity=.806,alpha=.2,node_eff=.55,Tmem=10.0,swap_visibility=.92**2,c_km_s=200000.0):
 ...
 latency=distance/c_km_s
 return {'distance_km':distance,'segments':segments,'nesting_levels':levels,'segment_km':seg,'elementary_purification_rounds':elementary_rounds,'swap_purification_rounds':swap_rounds,'elementary_link_probability':p_link,'elementary_link_rate_hz':rate,'raw_pair_factor':raw,'estimated_pair_time_s':time,'classical_latency_s':latency,'remote_sum_rate_hz':1/(time+latency),'final_pair_fidelity':F,'distillable':F>BOUNDARY,'elementary_purification_trace':etrace,'swap_trace':strace}
def pareto(rows):
 return sorted([r for r in rows if not any(o['final_pair_fidelity']>=r['final_pair_fidelity'] and o['remote_sum_rate_hz']>=r['remote_sum_rate_hz'] and (o['final_pair_fidelity']>r['final_pair_fidelity'] or o['remote_sum_rate_hz']>r['remote_sum_rate_hz']) for o in rows)],key=lambda r:(r['final_pair_fidelity'],r['remote_sum_rate_hz']))
def build():
 fixed=[]
 for F in (U,BOUNDARY,1.0):
  a,n=isotropic_purification(F);assert abs(n-F)<1e-12;fixed.append({'fidelity':F,'acceptance_probability':a})

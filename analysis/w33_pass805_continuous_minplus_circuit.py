...
def payload():
 ...
 counts=collections.Counter(n[0] for n in F.nodes);node_hash=hashlib.sha256(json.dumps(F.nodes,separators=(',',':')).encode()).hexdigest();chamber_hash=hashlib.sha256(json.dumps(chambers,sort_keys=True,separators=(',',':')).encode()).hexdigest();all22=len(phase)==22;pair=phase[('t1','t2')]

...
def girth8_ok(L,R,rows):
    # The same right-pair memory criterion used in Pass5134: no C4 and no C6.
    pair_seen=set();adjR=[set() for _ in R]
    for C in rows:
        for a,b in itertools.combinations(C,2):
            if (a,b) in pair_seen:return False
            # A shared neighbor of a,b in the accumulated right-pair graph would close a C6.
...
def sharp_cap31():
    # From Pass5142, every 19-edge deletion has wedge <=29.
    # Averaging: (18/20)W <=29, hence W<=32.
    raw=Fraction(29*20,18);assert raw<33
    # If W=32, every deletion loses >=3 wedges, so every edge has d(u)+d(v)>=5.
    # Global degree equations force N3=N2=8,N1=0.  The only ordered side profiles are:
    candidates=[
    ...
    for L,R in candidates:
        ...
        z=realize_constrained(L,R,5)
        rejected.append(z is None)
    assert all(rejected)

    # Sharpness at 31: a concrete C4/C6-free witness (a leaf extension of a
    # 19-edge sharp profile).
    L=(3,3,2,2,2,2,2,2,2)
    R=(3,3,3,3,3,3,1,1)
...
def main():
    ...
    # n1=29,30: convex line-graph wedge bound minus 3 floor(n1/3) triangles.
    ...
    # n1=31: every edge deletion must lose >=2 wedges, so degree-one Levi
    # vertices only attach to degree-three vertices.  With N_i endpoint counts,
    # N2+N1=40-31=9 and 3N3+N2=31, hence N1=3N3-22.
    # Each degree-one endpoint forces a chamber-graph degree-2 vertex.  At fixed
    # total chamber degree 62 this raises the convex wedge floor to 66+N1.
    # Therefore N112 >= (66+N1)-3N3 =44, independent of the degree solution.
    ...
    out={'pass':5145,'status':'THEOREM_Q5_STRICT_COUNTEREXAMPLE_LEADER_AT_LEAST_21',
      'q':5,'target_distance':625,'leader_size_closed':20,
      'adjacent_pair_cap':cap,'pair_relaxations':pair,'branches':branches,
      'uniform_leader20_weight_lower_bound':711,

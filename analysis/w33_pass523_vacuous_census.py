#!/usr/bin/env python3
"""Pass 523: the vacuous class censused and proved, the measured q = 3 window
...
(m/d) sum_i w_i = 0.
  ...
  * If p does not divide (m/d) the condition is sum_i w_i = 0.  For d = 1 that
    ...
    satisfiable.  For d = 2 use (u,-u).  For d >= 3 choose independent
    nonzero u,v and use (u,...,u,v,-(d-2)u-v): every entry is nonzero, the sum
    is zero, and the two final off-line symbols occur once, forcing exact
    period d.
...
THE MEASURED q = 3 WINDOW IS DERIVED.  Its four profiles:
  ...
  (6, 6): with e_1 = 0 Newton's identities reduce to
      ...
      e_2^a e_3^b with v(e_2) = v(e_3) = 6 reproduces the nine finite
      valuations for m = 2,...,10.  Coefficient valuations raise terms above
      the monomial baseline.  Verified here.  An all-m noncancellation/LTE
      argument is not supplied.
Taking the minimum over profiles gives 2(m + [m odd]) in that measured window.
...
divide m], any prime l != p has the lower-bound count 2 - 0 - 1 = 1.
At (3,5), (3,7), (5,3), (5,7), (7,3), the proved vacuous relation gives
nullity at least one while one nonzero sampled row gives rank at least one,
so nullity is exactly one.  None is a power of p; the characterisation is
therefore FALSE, without asserting a new global iff statement.
"""
...
def part_A_census(checks):
        ...
        for m in range(2, 25):
            for d in divisors(m):
                ...
                if k % p_ == 0:
                    ...
                    empty = False  # exact-period zero-sum construction above
                pred = d == 1 and m % p_ != 0
                if empty != pred:
                    ok = False
            rows[f"p{p_}_m{m}"] = {"vacuous_classes": [1] if m % p_ else []}
    ...
    return {
        "rule": (
            "the period-d class is empty if and only if d = 1 and p "
            "does not divide m"
        ),
        "proof": (
            ...
            "class is empty.  At d = 2 use (u,-u).  At d >= 3 choose "
            "independent nonzero u,v and use (u,...,u,v,-(d-2)u-v).  The "
            "entries are nonzero, the sum is zero, and each of the two "
            "off-line final symbols occurs once, so no shorter repeated "
            "word is possible.  Thus the tuple has exact period d.  So "
...
def part_B_complementary(checks):
    ...
    return {
        "bound": "nullity >= |T| + [p does not divide m]",
        "argument": (
...
def part_C_profile_66(checks):
    ...
    checks["profile_66_derived_at_all_nine_finite_exponents"] = ok
    return {
        "rows": rows,
        "recursion": (
            "with e_1 = 0 Newton's identities give " "p_m = -e_2 p_{m-2} + e_3 p_{m-3}"
        ),
        "derivation": (
            "Proved, not fitted.  Expanding p_m in monomials e_2^a e_3^b "
            "and valuing each as "
            "6(a+b) + v_lambda(coefficient) reproduces the nine finite "
            "valuations for m = 2,...,10 at the (6,6) profile.  Integer "
            "coefficient valuations raise terms above the monomial baseline; "
            "for example the coefficients 3 at m = 3 and 15 at m = 10 "
            "contribute positive lambda-adic valuation."
        ),
        "closes": (
...
def part_D_third_firing(checks):
    ...
    return {
        ...
        "refutation": (
            ...
            "vacuous class.  For each prime l != p, tau = 2, |T| = 0, and "
            "the proved vacuous relation gives nullity at least one.  At "
            "(3,5), (3,7), (5,3), (5,7), (7,3), a nonzero sampled row gives "
            "rank at least one, so nullity is exactly one in every listed "
            "cell.  None is a power of p, so the characterisation is FALSE.  "
            "No replacement global iff statement is claimed."
        ),
        "third_firing": (
...
def main_payload():
    ...
    return {
        ...
        "status": status,
        "headline": (
            "THE VACUOUS CLASS IS CENSUSED AND PROVED, THE MEASURED q = 3 "
            "WINDOW IS DERIVED, AND WE RETRACT A THIRD THEOREM.  The "
            ...
            "from matching proved lower and sampled upper bounds.  Its proof "
            "was correct but inherited a formula derived only on cells with "
            "p | m."
        ),

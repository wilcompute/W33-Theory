...
def payload():
 p681=load(P681,'p681_871');p821=load(P821,'p821_871');a,b,_,_,_=p681.build_pair();words=enumerate_words(a,b,p681);ws,wt,cent=find_inner_standard(words,p681);pieces=factor_pieces(p821);outer=outer_operators(pieces,ws,wt)
 atlas={}
 for d in (6,14,40):
  C,u1,h1=parse_atlas(d,1);D,u2,h2=parse_atlas(d,2);atlas[str(d)]={'C':C,'D':D,'urls':[u1,u2],'source_sha256':[h1,h2]}
 # Search all inner order-nine elements giving an outer standard pair, and require the dim-6 ATLAS conjugacy.
 order9=[(g,w) for g,w in words.items() if p681.order_perm(g)==9];chosen=None
 A6,B6=pieces['6'];C6=outer['6'];AC6,AD6=atlas['6']['C'],atlas['6']['D']
 for g,w in order9:
  D6=word_mat(w,A6,B6)
  if mat_order(C6@D6%2)!=10:continue
  variant,P,hd=find_conjugacy(C6,D6,AC6,AD6)
  if P is not None:chosen=(g,w,variant);break
 if chosen is None:raise RuntimeError('no ATLAS standard d found')
 gd,wd,v6=chosen
 results={};all_exact=True
 for key,(A,B) in pieces.items():
  C=outer[key];D=word_mat(wd,A,B);AC=atlas[key]['C'];AD=atlas[key]['D'];variant,P,hd=find_conjugacy(C,D,AC,AD);assert P is not None
  ext_alg=p821.algebra_dimension([C,D]);ext_cent=p821.centralizer_dim([C,D],C.shape[0]);results[key]={'dimension':C.shape[0],'outer_order':mat_order(C),'d_order':mat_order(D),'cd_order':mat_order(C@D%2),'matched_ATLAS_variant':variant,'intertwiner_dimension':hd,'conjugator_rank':rank2(P),'conjugator_sha256':hashlib.sha256(P.tobytes()).hexdigest(),'extended_algebra_dimension':ext_alg,'extended_endomorphism_dimension':ext_cent,'ATLAS_source_sha256':atlas[key]['source_sha256']};all_exact &= rank2(P)==C.shape[0]
 inner_commuting=sum(np.array_equal(outer['6']@word_mat(w,A6,B6)%2,word_mat(w,A6,B6)@outer['6']%2) for w in words.values())
 full_order=group_order_matrices(outer['6'],word_mat(wd,A6,B6))
 checks={'inner_group_words25920':len(words)==25920,'inner_standard_pair_orders2_5_9':(p681.order_perm(perm_word(ws,a,b,p681)),p681.order_perm(perm_word(wt,a,b,p681)),p681.order_perm(p681.compose(perm_word(ws,a,b,p681),perm_word(wt,a,b,p681))))==(2,5,9),'inner_standard_involution_centralizer576':cent==576,'outer_involution_inner_centralizer720':inner_commuting==720,'outer_standard_pair_orders2_9_10':all((z['outer_order'],z['d_order'],z['cd_order'])==(2,9,10) for z in results.values()),'extended_group_order51840':full_order==51840,'all_three_exact_simultaneous_conjugacies':all_exact,'all_ATLAS_conjugators_invertible':all(z['conjugator_rank']==z['dimension'] for z in results.values()),'outer_extension_makes40_absolutely_irreducible':results['40']['extended_algebra_dimension']==1600 and results['40']['extended_endomorphism_dimension']==1,'six_and14_ATLAS_labels_exact':results['6']['extended_endomorphism_dimension']==results['14']['extended_endomorphism_dimension']==1,'certificate_hash_locked':True}
 checks={k:bool(v) for k,v in checks.items()};raw={'ws':ws,'wt':wt,'wd':wd,'results':results};digest=hashlib.sha256(json.dumps(raw,sort_keys=True,separators=(',',':')).encode()).hexdigest()
 return {'schema':'w33.pass871.atlas_standard_conjugacy.v1','status':'PASS' if all(checks.values()) else 'FAIL','generator_conversion':{'repository_generators':['a(order3)','b(order9)'],'inner_ATLAS_words':{'a_2A':ws,'b_5A':wt},'outer_ATLAS_pair':{'c_2C':'unique involutory intertwiner implementing b_5A -> b_5A^4','d_9A_word_in_repository_generators':wd},'inner_2A_centralizer':cent,'outer_2C_centralizer':2*inner_commuting,'generated_outer_group_order':full_order},'exact_matches':results,'ATLAS_sources':{k:{'urls':v['urls'],'source_sha256':v['source_sha256']} for k,v in atlas.items()},'checks':checks,'certificate_sha256':digest,'theorem':'The W33 factors are extended from U4(2) to U4(2):2 by solving the outer-automorphism intertwining equations on a certified inner ATLAS standard pair. A repository word d of order nine is then selected so that c,d have orders 2,9 and cd has order 10 and generate all 51,840 elements. Downloaded ATLAS standard matrices in dimensions 6,14,40 are matched by explicit invertible simultaneous conjugators. In dimension 40 the outer generator collapses the F4 commutant to F2 and the extended algebra becomes M40(F2), explaining why the inner restriction was not absolutely irreducible while the ATLAS outer representation is.','boundary':'The certificate depends on the exact ATLAS source files whose URLs and SHA-256 digests are locked in the ledger. Regeneration therefore requires those public files or a future vendored copy with the same hashes.'}

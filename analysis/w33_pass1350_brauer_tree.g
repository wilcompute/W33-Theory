...
BI := BlocksInfo(M);; ORD := Irr(T);; BRAUER_IRR := IBr(M);;
...
for i in [1..Length(BI)] do
  b := BI[i];; D := DecompositionMatrix(M, i);;
  if i > 1 then AppendTo(out, ","); fi;
  AppendTo(out, "{\"number\":", String(i), ",\"defect\":", String(b.defect));
  AppendTo(out, ",\"ordinary_positions\":", String(b.ordchars), ",\"ordinary_degrees\":", String(List(b.ordchars, j -> ORD[j][1])));
  AppendTo(out, ",\"brauer_positions\":", String(b.modchars), ",\"brauer_degrees\":", String(List(b.modchars, j -> BRAUER_IRR[j][1])));
  AppendTo(out, ",\"decomposition_matrix\":", String(D));
  if IsBound(b.brauertree) then AppendTo(out, ",\"brauer_tree\":", String(b.brauertree)); else AppendTo(out, ",\"brauer_tree\":null"); fi;
  AppendTo(out, "}");
od;
AppendTo(out, "]}"); CloseStream(out);

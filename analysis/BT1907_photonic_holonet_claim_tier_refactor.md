...
| Spread side of the tax: 9/10 service rate and uniform defect loading | E | 36 spreads re-counted independently (exact cover, 9 per line); every spread meets every star once, so any optimum leaves every spread at exactly 9/10; exactly 20 optima per center (800 total) with occupancy always uniform `(c,c,c,c)`, `c` in {2,3,4} -- double occupancy is the minimal class. | `analysis/w33_spread_star_anatomy.py`; complete defect/spread incidence is the scheduler arc's tensor observation, independently re-verified. |
| Tax orbits: 800 optima = four `PSp(4,3)` orbits `360+40+360+40` | E | The point action of the transvection closure is `PSp(4,3)` (25920 = 51840/2; double cover kept distinct per this spine's warning); class (center lit?, load `c`) is a complete orbit invariant; the special `(3,3,3,3)` optimum is a canonical 40-orbit, one per point; queue invariant global (zero escalation imbalance). | `analysis/w33_tax_orbits.py`; q=5 probe capped at 40 optima, spectrum {4,5,6} = {q-1,q,q+1} suggestive only. |
| Perp states: the canonical 40-orbit named; the spectrum explained | E | The special optimum IS the deleted perp `Gamma(p)` (unique per center; flip = full perp, a geometric hyperplane); the defining GQ axiom is the optimality proof, verified q=2..5, so perp states exist at every order with loads q and q+1 -- the spectrum {q-1,q,q+1} is {grounds, deleted perp, full perp}; nine grounds stabilizer-transitive, out-triples = centric triads partitioning the 27 non-neighbors. | `analysis/w33_perp_states.py`; standard GQ objects identified with the Pass 58/59 optima; Schlafli-adjacent facts recorded as invariants only. |
| Affine plane law: grounds = `AG(2,q)`; every node carries a qutrit phase space | E | Optima per center close exhaustively at `2(q^2+1)` (20 at q=3, 52 at q=5, upgrading the capped sample); grounds = the all-centers-in-perp triads (ILP-free); grounds = points of `AG(2,q)` with neighbors as lines and the defect lines as parallel classes; stabilizer 2-transitive on the 9. | `analysis/w33_ground_affine_plane.py`; at q=3 the plane is the Hesse configuration = the committed single-qutrit phase-space identification, its 4 striations = the 4 defect contexts. |
| Interrupt controller: the tax arc executable inside the VM microkernel | E | Closed-form vector table (no optimizer at interrupt time); migration price law: re-vector in place 6 rays, edge migration 3 (cheap channels = the ground's own center quad), non-collinear >= 5; every tax theorem held as a runtime invariant over a seeded run. | `analysis/w33_interrupt_controller.py`; fuses the VM track's interrupt/tick/relocation rows with Passes 57-61; escalation pricing cites the 9^t dial. |
| Kernel fusion: mode-2 programs, defect-aware pages, walking defect | E | Lifted router executes through the kernel with 1600/1600 output equality and line-legal hops; safe zone = the AG(2,3) phase directory with a constant 9-point relocation page bill; the defect is a quad-constrained nearest-neighbor walker (100% of steps, 3 rays). | `analysis/w33_packet_vm_kernel.py`; `analysis/w33_defect_aware_placement.py`; `analysis/w33_defect_walk_telemetry.py`; consumers are the in-flight loader/packet VM/trace tooling. |
| Kernel dynamics: the walking defect's stationary and audit laws | E | The cheap-channel walk on 360 grounds is uniformly stationary (connected vertex-transitive); re-keying is addressed (rebuilt line = old center, quads meet in exactly the old center); a live pipeline runs programs+pages+walk; and the fabric logs itself (150/150 origins decode from 3 markers, no interrupt log). | `analysis/w33_kernel_dynamics.py`; audit-by-geometry is the UOR/replay guarantee supplied by the incidence structure. |
| Track unification: BT1808 self-audit, exact spectral gap, unified page bill | E | The self-logging decode makes all 480 BT1808 scheduler edges origin-recoverable with no log; the cheap-channel graph's spectrum is exact (rank-8, gap (15-sqrt97)/16 by integer moments); the safe-zone and TD(4,3) page bills are the same nine for all 1560 moves. | `analysis/w33_scheduler_audit_backend.py`; `analysis/w33_cheap_channel_spectrum.py`; `analysis/w33_page_bill_unification.py`; builds on committed BT1807-1816. |

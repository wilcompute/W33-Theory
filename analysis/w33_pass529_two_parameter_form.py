...
in 9Z and e_3 in 27Z, and the whole q = 3 law is indexed by a pair of small
integers.
...
v_lambda(sum_j nu_j^m) -- answered at q = 3 by six lattice points and open at
q >= 5, where coefficient valuations demonstrably do not suffice.  That is the
...
SIX = [((0, 0), 1), ((-9, 0), 8), ((-18, 0), 24), ((-27, 0), 8),
       ((-27, 27), 24), ((-36, 81), 16)]
...
def part_A_family(checks):
    ...
    for (e2, e3), mult in SIX:
        ...
        rows[f"x^3{e2:+d}x{-e3:+d}"] = {
            "e2": e2, "e3": e3, "sections": mult, "a": a, "b": b,
            "discriminant": disc, "disc_over_729": disc // 729,
    ...
    return {"rows": rows, "ab_pairs": sorted(ab),
            "form": "x^3 - 9a x - 27b",
            "reading": (
                "The image of the section space in charpoly space is six "
                "LATTICE POINTS, not six arbitrary polynomials: e_2 lies in "
                "9Z, e_3 in 27Z, and the whole q = 3 law is indexed by a pair "
                "of small integers.  The discriminant is 729(4a^3 - 27b^2), "
...
def part_B_closing(checks):
    ...
    return {"statement": (
        "Given charpoly(D), compute v_lambda(sum_j nu_j^m).  Answered at "
        "q = 3 by six lattice points (a,b) in {(0,0),(1,0),(2,0),(3,0),(3,1),"
        "(4,3)} under x^3 - 9a x - 27b; open at q >= 5, where coefficient "
...
def main_payload():
    ...
    return {
        ...
        "headline": (
            ...
            "points -- e_2 in 9Z, e_3 in 27Z -- and the entire q = 3 law is "
            "indexed by a pair of small integers.  The discriminant is "

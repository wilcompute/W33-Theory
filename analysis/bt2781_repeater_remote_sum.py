...
def main():
 out=build();p=ROOT/'data/PART_BT2781_REPEATER_REMOTE_SUM.json';p.write_text(json.dumps(out,indent=2,sort_keys=True)+'\n');s={k:out[k] for k in ('schema','status','bell_label_transition','isotropic_recurrence','swap_recurrence','memory_model','scenario_assumptions','hardware_contract','boundary')};keys=('segments','elementary_purification_rounds','swap_purification_rounds','final_pair_fidelity','remote_sum_rate_hz','distillable');s['scenario_summary']={d:{'best_distillable_rate':({k:x['best_distillable_rate'][k] for k in keys} if x['best_distillable_rate'] else None),'best_fidelity':{k:x['best_fidelity'][k] for k in keys}} for d,x in out['scenarios'].items()};(ROOT/'data/PART_BT2781_REPEATER_REMOTE_SUM_summary.json').write_text(json.dumps(s,indent=2,sort_keys=True)+'\n');print('wrote',p)

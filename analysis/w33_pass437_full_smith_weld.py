...
from w33_pass435_integral_heisenberg_smith_pairing import val,multiplicities,prime_to_p_layers
...
def prime_layers(p:int,f:int)->dict[int,Counter]:
    q=p**f;out={p:characteristic_layers(p,f)};out.update(prime_to_p_layers(q,p));return out


def tree_valuation(q:int,p:int,ell:int)->int:
    mplus,mminus,_=multiplicities(q)
    if ell==p:return val(q,p)*(2*(q*q-1)+mplus+mminus-3)
...
def build_row(p:int,f:int)->dict:
    ...
    checks={'all_primary_valuations_match_tree':all(sum(e*n for e,n in shape.items())==tree_valuation(q,p,ell) for ell,shape in layers.items()),
    ...
    return {'q':q,'p':p,'f':f,'primary_layers':{str(ell):{str(e):n for e,n in sorted(shape.items())} for ell,shape in sorted(layers.items())},
...
def build_payload()->dict:
    ...
    checks={'all_rows_pass':all(r['status']=='PASS' for r in rows),
      'q3_reconstructs_pass431':rows[0]['invariant_factor_runs']==[{'order':3,'multiplicity':4},{'order':6,'multiplicity':4},{'order':18,'multiplicity':1},{'order':54,'multiplicity':1},{'order':216,'multiplicity':6}],
      'q25_complete_group_closed':rows[3]['status']=='PASS','q27_complete_group_closed':rows[4]['status']=='PASS','complete_all_prime_components':True}

...
def solve_intertwiner(A,B,C,D):
 n=A.rows;I=sp.eye(n);K=(sp.kronecker_product(I,A)-sp.kronecker_product(C.T,I)).col_join(sp.kronecker_product(I,B)-sp.kronecker_product(D.T,I));ns=K.nullspace();assert len(ns)==1
 v=ns[0];X=sp.Matrix(n,n,lambda i,j:v[j*n+i]);assert A*X==X*C and B*X==X*D and X.det()!=0;return X,len(ns)

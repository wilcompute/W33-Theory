# Pass 8909 -- export the centraliser of the order-4 regular element of W(E8).
#
...
# product of the degrees divisible by 4, namely 8*12*20*24 = 46080 -- and that is the
...
log  := Concatenation(repo, "/analysis/_e8_centraliser_log.txt");;
PrintTo(log, "start\n");
...
i := 0;;
while i < 20000 and found = fail do
  i := i + 1;
  m := IdentityMat(8);
  ...
  if o mod 4 = 0 then
    x := m^(o/4);
    if x * x = -IdentityMat(8) then found := x; fi;
  fi;
od;
...
if found = fail then
  ...
  AppendTo(log, "found M with M^2 = -I, det(I-M) = ",
           DeterminantMat(IdentityMat(8) - found), " (expect 16)\n");
  pm := permof(found);
  Cen := Centralizer(W, pm);
  AppendTo(log, "|C_W(M)| = ", Size(Cen), "  (Springer predicts 8*12*20*24 = 46080)\n");
  ...
  f := OutputTextFile(Concatenation(repo, "/analysis/_e8_cen_gens.txt"), false);;
  ...
  od;
  CloseStream(f);
  f2 := OutputTextFile(Concatenation(repo, "/analysis/_e8_cen_M.txt"), false);;
  ...
  od;
  CloseStream(f2);
  AppendTo(log, "wrote _e8_cen_gens.txt and _e8_cen_M.txt\n");
fi;

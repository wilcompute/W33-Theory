...
with grading dimensions 6+9+9.  A subsequent exact center/Killing/radical audit
finds a 15-dimensional two-step nilpotent radical, so the tempting
trinification A2^3 identification is ruled out rather than left open.
...
def main(write=True):
    ...
    out = {
        ...
        "boundary": "This certificate proves exact generation of the rational/complex Chevalley Lie algebra by two structured algebra elements. Its residual 24-dimensional branch is not SU(3)^3: an independently replayed exact audit gives center dimension 1, Killing rank 9, and a 15-dimensional two-step nilpotent radical. The full Levi and module identification belongs to the separate residual-structure packet. Compact-real two-control integration, laboratory Hamiltonians, vacuum selection, energy scales, and coupling strengths are outside this certificate.",
        "parents": [

#!/usr/bin/env python3
"""Pass 2778: physical two-shot metaplectic sensor and shot-noise budget."""
...
def conjugacy_classes(group):
    ...
    out.sort(key=lambda c:(len(c),c[0]));assert len(out)==34;return out
def unitary_generators():
    w=np.exp(2j*np.pi/3);F=np.array([[w**(j*k) for k in range(3)] for j in range(3)],complex)/np.sqrt(3);S=np.diag([w**((2*j*j)%3) for j in range(3)]).astype(complex);I=np.eye(3,dtype=complex);SUM=np.zeros((9,9),complex)
    for p in range(3):
        for f in range(3):SUM[3*p+(f+p)%3,3*p+f]=1
    out=[]
    for U in [np.kron(F,I),np.kron(I,F),np.kron(S,I),np.kron(I,S),SUM]:out.append(U);Ui=U.conj().T;out.extend([Ui] if np.max(abs(Ui-U))>1e-10 else [])
...
def main():
    ...
    nonzero=[];phase_levels=Counter();mag_levels=Counter();rows=[]

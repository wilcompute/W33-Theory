...
\]
which records the $C_2$ root heights $1,1,2,3$ and restricts to the Heisenberg and flat BT865 memory filtrations.  Finally, the canonical diagonal $V_4$ splits the rational $12$-dimensional incidence defect into character sectors $4+4+2+2$.

\paragraph{Boundary.}
The all-$q$ and $q=5$ minimum-distance theorem remains open.  The $q=4$ minimum shell and active-chart theorem are closed.  The controller and memory statements are exact finite group/module results, not fabricated-hardware performance claims.
\fi

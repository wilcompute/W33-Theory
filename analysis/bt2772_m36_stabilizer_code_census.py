#!/usr/bin/env python3
"""Pass 2772: exhaustive [[4,2]] binary-stabilizer projection census for two M36 copies."""
...
SQ3=sp.sqrt(3); p=sp.symbols('p', real=True)
...
def syndrome_bases(sub):
    s1,s2,_=sub;l1,l2=extend_iso(s1,s2);Ps=[PAULI4[x] for x in (s1,s2,l1,l2)]
    H=Ps[0]+2*Ps[1]+4*Ps[2]+8*Ps[3];_,V=np.linalg.eigh(H)
    labels=[tuple(1 if np.real(np.vdot(V[:,j],P@V[:,j]))>0 else -1 for P in Ps) for j in range(16)]
    out={}
    for e1,e2 in itertools.product((1,-1),repeat=2):
        cols=[]
        for z1,z2 in itertools.product((1,-1),repeat=2):cols.append(V[:,labels.index((e1,e2,z1,z2))])
        out[(e1,e2)]=np.column_stack(cols)
    return out
...
def bernstein_power_to_coeff(a):
    a=list(a)+[0]*(4-len(a));a0,a1,a2,a3=a[:4]
...
def nonpositive_bernstein(b,depth=0):
    ...
    l,r=subdivide_bernstein(b)
    return nonpositive_bernstein(l,depth+1) and nonpositive_bernstein(r,depth+1)

def certify_poly(dcoef,pmax):
    return nonpositive_bernstein(bernstein_power_to_coeff([sp.simplify(dcoef[i]*pmax**i) for i in range(4)]))

def ray_expect(S,R):return np.real(np.einsum('ij,ji->i',R.conj().T@S,R))

def census(psi,fstab,rays,subs):
    A=np.outer(psi,psi.conj());B=np.eye(4)/4-A
    coeff=[np.kron(A,A),np.kron(A,B)+np.kron(B,A),np.kron(B,B)]
    R=np.column_stack(rays);pmax=sp.nsimplify(4*(1-fstab)/3,[SQ3])
    closure=certified=identical=0;profiles=Counter();worst=None
    for sub in subs:
        for syndrome,V in syndrome_bases(sub).items():
            ...
            for ti in targets:
                ...
                certified+=1;lead=next(i for i,x in enumerate(d) if x!=0);profiles[f'first_negative_order_{lead}']+=1
                val=sp.N(sum(d[i]*(pmax/2)**i for i in range(4)),30)
...
def fanout_formula(k):
    ds=(4-p)/12;dd=p/4;F=sp.simplify((ds**k+2*(1-p)**k/3**k)/(3*ds**k+dd**k))
    return {'copies':k,'success_probability':str(sp.factor(3*ds**k+dd**k)),'output_fidelity':str(sp.factor(F)),'difference_from_input':str(sp.factor(F-(1-sp.Rational(3,4)*p)))}

def main():
    rays,meta,groups=m36_grade_data();subs=all_isotropic_rank2();exact_f={8:(2+SQ3)/6,24:(5+2*SQ3)/12,4:sp.Rational(3,4)};rows=[]

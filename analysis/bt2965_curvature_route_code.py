#!/usr/bin/env python3
"""Pass 2965: curvature-aware route code for the ten-mode spread router.

For the complete graph K10 of spread modes, edge-parity faults live in F2^45.
Triangle curvature residuals are the coboundary H e in F2^120.  This verifier
proves ker(H) is exactly the 9-dimensional vertex-switching (cut) space, hence
the finite router carries a [45,9,9] gauge code.  Every non-gauge fault of
weight at most eight is detected, and every fault of weight at most four is
correctable modulo gauge.
"""
...
OUT = ROOT / "data" / "PART_BT2965_CURVATURE_ROUTE_CODE_results.json"
...
def main():
    ...
    for a, b in itertools.combinations(range(45), 2):
        weight = int(np.count_nonzero(H[:, a] ^ H[:, b]))
        pair_syndrome_weights[weight] += 1
    ...
    result = {
        "schema": "w33.pass2965.curvature_route_code.v1",
        ...
        "decoder_statement": (
            "Compare observed triangle parities with the Pass-2962 baseline. "
            "The residual is H e. Decode to the minimum-weight edge-fault coset; "
            "two faults differing by a cut are physically gauge-equivalent."
        ),
        "headline": (
            ...
            "of weight at most four are correctable modulo vertex gauge, and the first "
        ...
        "claim_boundary": (
            ...
            "a fixed parity class, or detector faults unless those are separately mapped "
            "to edge-parity flips."
        ),
    }

#!/usr/bin/env python3
"""Pass5181 (bonkers): all-field root-Cayley diameter threshold and q=5 outer torus.
...
parameterized by a,b in F_5^* with c=2ab and d=2a^2 b.  We call this a
multiplicative parameter torus/torsor only as a set parametrization; no subgroup
closure is asserted.
"""
...
OUT=ROOT/'data/PART_W33_PASS5181_ALLFIELD_ROOT_METRIC_DIAMETER_THRESHOLD.json'


def family(q):
    if q in (2,4) or (q & (q-1)==0):
        # only used for powers of two in anchors below
        s4=(q-1)**2*(q-2)*(q-4)
        return 'char2',s4
    raise ValueError
...
def main():
    # Symbolic family zeros on finite-field sizes.
    ...
    out={
      ...
      'synthesis':'Pass5143 + Pass5165 + Pass5175 now give not only all finite-field shell formulas but an exact diameter threshold. q=5 is the first field with a nonempty distance-four shell.',

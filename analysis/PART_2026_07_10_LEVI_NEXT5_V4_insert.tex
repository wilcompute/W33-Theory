...
\]
The repository CI requires \texttt{lake build --wfail}, an explicit rejection
of \texttt{sorry} and \texttt{admit}, and an independent official
\texttt{leanchecker} pass.
...
\[
 \boxed{[u_5]=[4h]\ne0.}
\]
Consequently a pure scalar-$5$ normal form is obstructed.  But
$u_1=\tau(h)-h=u_5+4h$ is a coboundary: exactly $512$ order-two shifts
make $h+v$ fixed, and $256$ preserve $q(h+v)=11/8$.  The outer action
therefore admits a fixed order-eight rail; only the scalar-$5$ form is
cohomologically obstructed.

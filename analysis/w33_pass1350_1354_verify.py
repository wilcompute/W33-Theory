...
def verify_basic():
 canonical=json.loads((DATA/'w33_pass1345_modular_basic_algebras.json').read_text());ext=json.loads((DATA/'w33_pass1351_projective_layers.json').read_text());out={}
 for p in ('2','3','5'):
  r=canonical['records'][p];e=ext['records'][p];D=sp.Matrix(r['decomposition_matrix']);C=sp.Matrix(r['cartan_matrix']);assert D.T*D==C;assert e['vertices']==r['quiver_and_associated_graded_relations']['vertices'];assert e['projective_basic_dimensions']==[sum(row) for row in C.tolist()]
  ...
  assert len(e['resolution_prefixes'])==len(e['vertices']);out[p]={'basic_dimension':r['basic_algebra_dimension'],'quiver':r['quiver_and_associated_graded_relations'],'projective_layers':e}
 return out
def verify_atlas():

#!/usr/bin/env python3
"""Pass 2772: exhaustive two-copy stabilizer search for an M36 distiller.
...
def build() -> dict:
    ...
    correction = None
    for key, correction_word in sorted(clifford_words.items(), key=lambda kv: (len(kv[1]), kv[1])):
        ...
        for target_meta, target in zip(metadata, rays):
            if abs(np.vdot(target, out)) ** 2 > 1 - 1e-9:
                correction = {"matrix_word_product": correction_word, "target_ray": list(target_meta)}
                break
        if correction:
            break
    ...
    return {
        "schema": "w33.pass2772.m36_two_copy_distillation.v1",
...
def main() -> None:
    out = build()
    path = ROOT / "data" / "PART_BT2772_M36_TWO_COPY_DISTILLATION.json"

...
def main(write=True):
    ...
    safe_sheet_beta_order=[]
    for orbit in target_orbits:
        betas={p.center_beta(safe[index]&p.ZK) for index in orbit}
        assert len(betas)==1
        safe_sheet_beta_order.append(next(iter(betas)))
    assert safe_sheet_beta_order==[0,1,2]
    ...
    # The six permutations of the three character labels are a discrete
    # convention inside this induced/Fourier ansatz, not the entire space of
    # full-Clifford intertwiners.  The latter has dimension seven, computed
    # exactly from the source and twisted-target characters.
    source_character=[sum(index==image for index,image in enumerate(source[g])) for g in G]
    source_commutant_numerator=sum(value*value for value in source_character)
    assert source_commutant_numerator==7*648
    target_characters=[]
    for g in G:
        value=ZERO
        for t in range(36):
            if target[g][t]==t:
                value=value+omega_power(phases[g][t])
        target_characters.append(value)
    target_commutant_numerator=sum(
        (value.conjugate()*value for value in target_characters), ZERO
    )
    assert target_commutant_numerator==7*648*ONE
    ...
    out={
      ...
      "orbits":{"ordinary36":[36],"safe36":[12,12,12],"aligned_safe_seeds":aligned,
                "safe_sheet_beta_order":safe_sheet_beta_order},
      "compiler":{"dimension":36,"field":"Q(omega)","rank":36,"nonzero_entries":108,
                  ...
                  "target_column_to_source_rows_and_omega_exponents":serial,
                  "matrix_digest":digest,
                  "full_Clifford_intertwiner_space_dimension":7,
                  "source_permutation_commutant_dimension":7,
                  "twisted_target_commutant_dimension":7},
      "checks":{"full_648_source_action":True,"safe_action_image216":True,
                ...
                "safe_sheet_order_is_beta_0_1_2":True,
                "source_and_target_commutants_have_dimension7":True},

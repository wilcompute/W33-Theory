...
OUT=ROOT/'data'/'w33_pass1114_full_cubic_central_phase_extension.json'
...
AFF=[((0,0),(0,1),(0,2),[(0,17,26),(3,14,22),(6,11,21)]),((0,0),(1,0),(2,0),[(0,20,23),(1,16,21),(2,15,22)]),((0,0),(1,1),(2,2),[(0,18,25),(4,13,21),(8,9,22)]),((0,0),(1,2),(2,1),[(0,19,24),(5,12,22),(7,10,21)]),((0,1),(1,0),(2,2),[(2,11,25),(4,14,20),(9,16,17)]),((0,1),(1,1),(2,1),[(7,14,18),(8,11,19),(12,13,17)]),((0,1),(1,2),(2,0),[(1,14,24),(5,11,23),(10,15,17)]),((0,2),(1,0),(2,1),[(2,7,26),(3,16,19),(6,12,20)]),((0,2),(1,1),(2,0),[(1,8,26),(3,13,23),(6,15,18)]),((0,2),(1,2),(2,2),[(3,10,25),(4,5,26),(6,9,24)]),((1,0),(1,1),(1,2),[(2,13,24),(5,16,18),(8,10,20)]),((2,0),(2,1),(2,2),[(1,12,25),(4,15,19),(7,9,23)])]
...
def main():
 ...
 fibers={u:tuple(e for z,e in byu[u]) for u in sorted(byu)};triads=[]
 for u1,u2,u3,ts in AFF:
  for t in ts:
   us=[HZ[e][0] for e in t];zs=[HZ[e][1] for e in t];triads.append({'e6ids':list(t),'kind':'affine','u_support':[list(u) for u in us],'z_values':zs,'central_phase_sum_mod3':sum(zs)%3})
 for t in FIB:
  us=[HZ[e][0] for e in t];zs=[HZ[e][1] for e in t];triads.append({'e6ids':list(t),'kind':'fiber','u_support':[list(u) for u in us],'z_values':zs,'central_phase_sum_mod3':sum(zs)%3,'canonical_cubic_sign':FIBER_SIGNS[tuple(sorted(t))]})
 hyp_rows=[]
 for n,u in HYPER.items():
  eids=list(fibers[u]);tri=tuple(sorted(eids));assert tri in FIB
 ...
 phase=Counter(x['central_phase_sum_mod3'] for x in triads);upstream=json.loads((ROOT/'data'/'w33_pass1103_hesse_firewall_cubic_transport.json').read_text());fiber_sign_hist=Counter(FIBER_SIGNS.values())
 sign_path=ROOT/'artifacts/canonical_su3_gauge_and_cubic.json';sign_rows=None;sign_dist=None
 if sign_path.exists():
  sg=json.loads(sign_path.read_text())
  if sg.get('counts',{}).get('solvable'):
   sign_map={tuple(sorted(int(x) for x in row['triple'])):int(row['sign']) for row in sg['solution']['d_triples']};assert set(sign_map)=={tuple(sorted(x['e6ids'])) for x in triads}
   for row in triads:row['canonical_chevalley_sign']=sign_map[tuple(sorted(row['e6ids']))]
   sign_rows=[{'e6ids':list(k),'sign':v} for k,v in sorted(sign_map.items())];sign_dist={str(k):v for k,v in Counter(sign_map.values()).items()};assert all(sign_map[t]==sgn for t,sgn in FIBER_SIGNS.items())

...
15 inside this invariant space.
"""
...
def main():
    ...
    for a,edges in enumerate(G['apt_edges']):
        ...
        assert X[a].sum()==4
    V=X@B
    # Theta point-graph adjacency; no pair is repeated because Tanner girth has no 4-cycles.
    ...
    def act(M):
        ...
        return R
    AV=act(V);A2V=act(AV);res=A2V-2*AV-16*V
    ...
    out={'pass':5087,'status':'THEOREM_EXACT_GLOBAL_QUADRATIC_CHANNEL','q':3,'apartments':1620,'theta_checks':4320,
         ...
         'annihilating_polynomial':'x^2-2x-16','discriminant':68,
         'generated_charpoly':'(x^2-2x-16)^15','eigenchannels':['1+sqrt(17)','1-sqrt(17)'],'multiplicity_inside_generated_space':[15,15],
         'bridge':'The same quadratic eigenvalues occur in the Pass5069 twisted T6 Hecke channel; this pass supplies an explicit global apartment-function carrier from the Levi line kernel.',

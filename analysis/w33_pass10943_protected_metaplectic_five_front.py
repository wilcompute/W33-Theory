...
def golay_logical_injection() -> dict:
    repo_prior = json.loads((ROOT / "data/w33_dark_strange_golay11_ft_lane.json").read_text())
    assert repo_prior["w33_code"]["quantum_CSS"] == "[[11,1,5]]_3"
    assert repo_prior["w33_code"]["logical_distance"] == 5
    assert repo_prior["physical_fault_tolerant_lane_enabled"] is False
    generator = np.array([
    ...
    return {
        "repo_prior_ownership": {
            "parent": "analysis/w33_dark_strange_golay11_ft_lane.py",
            "already_owned": "W33 reconstruction of the [[11,1,5]]_3 CSS algebra and dark-to-Strange type handoff",
            "Pass10943_increment": (
                "independent generator replay, complete logical converter/RUS injection ABI, "
                "3609 weight-at-most-two Pauli census, and end-to-end resource ledger"
            ),
        },
...
def m36_qutrit_transducer() -> dict:
    ...
    for row in ray_controls():
        ...
        if corrections:
            ...
            certified_fidelity = 7/9
        else:
            resource_class = "stabilizer"
            correction = None
            assert abs(fidelity - 1) < TOL
            certified_fidelity = 1.0
        # The output ordering is a coordinate convention.  Every permutation of
        # the three surviving modes is generated by qutrit X and parity, hence is
        # Clifford and must preserve the resource classification.
        for permutation in itertools.permutations(range(3)):
            permuted_key = projective_key(compressed[list(permutation)].reshape(3, 1))
            if resource_class == "R_magic":
                assert permuted_key in reflection_orbit
            else:
                assert any(
                    permuted_key == projective_key(state.reshape(3, 1))
                    for state in stabilizers
                )
            output_permutation_checks += 1
        ...
        rows.append({
            ...
            "maximum_qutrit_stabilizer_fidelity": certified_fidelity,
        })
    ...
    # Join the optical ROM family label to the independently certified temporal
    # null-line/Hesse dictionary.  This is a control-label weld, not a claim that
    # the finite null direction dynamically creates magic.
    atlas = json.loads((ROOT / "data/w33_20260924_m36_null_line_atlas.json").read_text())
    assert atlas["status"] == "PASS_M36_IS_THE_36_AFFINE_NULL_LINES_OF_THE_HISTORY_LIGHT_CONE"
    clock = json.loads((ROOT / "data/w33_20260925_hesse_clock_e8_gradings.json").read_text())
    assert clock["status"] == "PASS_HESSE_CLOCK_GENERATES_CONTACT_AND_CUBIC_E8_GRADINGS"
    clock_rows = clock["oriented_three_tick_grading"]["rows"]
    assert clock["oriented_three_tick_grading"]["Lie_dimensions"] == [2, 27, 54, 82, 54, 27, 2]
    assert clock["oriented_three_tick_grading"]["positive_nilpotent_growth"] == [54, 81, 83]
    family_records = atlas["M36_coordinates"]["family_records"]
    assert [record["family"] for record in family_records] == list(range(4))
    control_dictionary = []
    for record in family_records:
        family = record["family"]
        family_classes = {
            row["qutrit_resource_class"] for row in rows if row["family"] == family
        }
        assert len(family_classes) == 1
        resource_class = family_classes.pop()
        striation = tuple(sorted(tuple(line) for line in record["Hesse_parallel_class"]))
        oriented_clocks = [
            row for row in clock_rows
            if tuple(sorted(tuple(line) for line in row["striation"])) == striation
        ]
        assert len(oriented_clocks) == 6
        assert {
            tuple(row["root_grade_census"][str(grade)] for grade in range(-3, 4))
            for row in oriented_clocks
        } == {(2, 27, 54, 74, 54, 27, 2)}
        control_dictionary.append({
            "family": family,
            "family_name": record["family_name"],
            "projective_null_direction": record["P1_direction"],
            "history_null_direction": record["null_direction"],
            "Hesse_striation": record["Hesse_parallel_class"],
            "E8_A2_component": record["A2_component"],
            "oriented_E8_parabolic_clock_choices": len(oriented_clocks),
            "E8_parabolic_Lie_dimensions": [2, 27, 54, 82, 54, 27, 2],
            "positive_clock_bracket_growth": [54, 81, 83],
            "qutrit_resource_class_in_canonical_ROM_gauge": resource_class,
        })
    assert Counter(
        row["qutrit_resource_class_in_canonical_ROM_gauge"]
        for row in control_dictionary
    ) == Counter({"R_magic": 3, "stabilizer": 1})
    ...
    return {
        ...
        "output_coordinate_permutation_checks": output_permutation_checks,
        "temporal_control_dictionary": {
            "rows": control_dictionary,
            "family_resource_split": {"R_magic": 3, "stabilizer": 1},
            "control_reading": (
                "One two-bit family selector jointly addresses the optical dark-mode "
                "partial isometry, an affine history null direction, one Hesse striation, "
                "one temporal E8 A2 component, and six origin/orientation choices of the "
                "exact 2+27+54+82+54+27+2 parabolic clock. In the committed canonical ROM "
                "gauge, three selector values emit R-orbit qutrits and one emits stabilizers."
            ),
            "symmetry_firewall": (
                "AGL(2,3) treats the four Hesse striations equivalently. Therefore the "
                "3+1 magic/stabilizer split is a property of the chosen optical phase "
                "gauge and qutrit output encoding, not an intrinsic preferred null "
                "direction or a derived arrow of time. It may be used as a programmable "
                "resource-versus-calibration lane assignment only after that gauge is fixed."
            ),
            "repo_parent": "data/w33_20260924_m36_null_line_atlas.json",
            "clock_parent": "data/w33_20260925_hesse_clock_e8_gradings.json",
        },
        "rows": rows,

...
\title{
    ...
    \textbf{\Huge The W33 Theory of Everything}\\[0.3em]
    \textbf{\LARGE Complete Unified Physics}\\[0.2em]
    \textbf{\LARGE from a Single Graph}\\[0.5em]
    {\color{w33blue}\rule{\textwidth}{2pt}}\\[1em]
    \Large Derived from the Finite Field $\mathbb{F}_3$\\[0.3em]
    \large 100 Parts Complete\\[0.5em]
    \large\textit{Version 3.0 --- Comprehensive Documentation}
}
...
% ============================================================================
\begin{abstract}
\noindent We present a complete unified theory of fundamental physics based on a single mathematical structure: the \textbf{W33 graph}, a strongly regular graph with parameters $(40, 12, 2, 4)$ arising from the symplectic group $\mathrm{Sp}(4, \mathbb{F}_3)$ over the finite field with three elements.
...
\vspace{0.5em}
\noindent\textbf{Keywords:} theory of everything, W33 graph, strongly regular graph, exceptional Lie algebras, fine structure constant, Hubble tension, grand unification
\end{abstract}
...
\begin{theorem}[From $\mathbb{F}_3$ to W33]
The following construction chain produces the W33 graph:
\begin{enumerate}
    ...
    \item \textbf{Isotropic Lines}: Identify lines where $\omega$ vanishes
    \item \textbf{Graph}: Connect lines that span isotropic planes
\end{enumerate}
Result: $\Wthree = \mathrm{Sp}(4, \mathbb{F}_3)$, a strongly regular graph.
\end{theorem}

\subsection{W33 Parameters}

\begin{definition}[W33 = SRG$(40, 12, 2, 4)$]
The W33 graph has parameters:
\begin{align}
...
The automorphism group of W33 equals the Weyl group of $E_6$:
\begin{equation}
    \boxed{|\Aut(\Wthree)| = |W(E_6)| = 51{,}840}
\end{equation}
\end{theorem}

This is not coincidence---it reveals W33 as the incidence structure of exceptional algebras.
...
We have presented a complete unified theory of physics based on the W33 graph, derived from the finite field $\mathbb{F}_3 = \{0, 1, 2\}$.
...
\begin{enumerate}
    ...
    \item \textbf{Deep connections}: $|\Aut| = |W(E_6)|$, $|\text{Edges}| = |E_8 \text{ roots}|$
...
\bibitem{coxeter1940} H.S.M. Coxeter, ``The polytope $2_{21}$,'' \textit{Amer. J. Math.} \textbf{62} (1940) 457--486.

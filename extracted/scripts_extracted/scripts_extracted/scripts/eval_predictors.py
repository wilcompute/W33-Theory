...
def main():
    ...
    with out_md.open('w', encoding='utf-8') as f:
        ...
        for _, row in out_df.iterrows():
            f.write(f"- **{row['predictor']}** (n={int(row['n'])}): pearson={row['pearson']:.4f} (p={row['pearson_p']:.4f}, CI={row['pearson_ci_lo']:.3f}..{row['pearson_ci_hi']:.3f}), spearman={row['spearman']:.4f} (p={row['spearman_p']:.4f}, CI={row['spearman_ci_lo']:.3f}..{row['spearman_ci_hi']:.3f})\n")
    print('Wrote', out_csv)
    print('Wrote', out_md)

...
with zipfile.ZipFile(z7,'r') as zf:
    wit = pd.read_csv(io.BytesIO(zf.read('W33_N12_58_portlaw_rewrite_bundle_v7/cycle_witness_v3_holonomy_solver.csv')))
...
for _,r in wit.iterrows():
    step.append({
        ...
        'triad_phase': str(complex(np.exp(2j*math.pi*int(r.triad_hol_mod12)/12))),
    })

...
/-!
# Pass 1115: A₂ carrier, central-phase, and second-vendor closure
...
theorem centralPhase_total : centralPhaseHistogram.sum = 45 := by norm_num [centralPhaseHistogram]
theorem centralPhase_nonzero_total : (centralPhaseHistogram.drop 1).sum = 20 := by norm_num [centralPhaseHistogram]

def firewallFiberSignCounts : List Nat := [2,7]
theorem firewallFiber_sign_total : firewallFiberSignCounts.sum = 9 := by norm_num [firewallFiberSignCounts]

def qontrolScheduleCounts : List Nat := [40,160,40]
theorem qontrol_schedule_total : qontrolScheduleCounts.sum = 240 := by norm_num [qontrolScheduleCounts]

...
W33_CSV = Path(r"/mnt/data/repo_extract/data/_workbench/02_geometry/W33_line_phase_map.csv")
N12_CAND = Path(r"/mnt/data/repo_extract/data/_n12/n12_58_candidate_w33_points_40_from_tau_cycles_and_fixed_complements_20260109t134000z.csv")
TWO_T = Path(r"/mnt/data/repo_extract/data/_n12/n12_58_2t_holonomy_nontrivial_cycles.csv")
...
def parse_supports(s: str):
    ...
    for part in parts:
        ...
        if len(nums)==4: out.append(nums)
    return out

def main():
    w33=pd.read_csv(W33_CSV)
    w33['pts']=w33['point_ids'].astype(str).apply(lambda s: tuple(int(x) for x in s.split()))
    ...
    for pts in lines.values():
        ...
        for i in range(4):
            for j in range(i+1,4):
                a,b=pts[i],pts[j]
                col_neighbors[a].add(b); col_neighbors[b].add(a)

    non_neighbors=[set(p for p in range(40) if p!=i and p not in col_neighbors[i]) for i in range(40)]
    triads=[]
    for a in range(40):
    ...
    cand['tri_list']=cand['members'].fillna('').apply(lambda s: [t for t in re.split(r'[,\s]+', str(s)) if t])
    cand['mask']=cand['tri_list'].apply(lambda tris: int(np.bitwise_or.reduce([tri_to_mask(t) for t in tris])) if tris else 0)
    ...
    two['support_masks']=two['support_list'].apply(lambda lst: [sum(1<<n for n in nums) for nums in lst])

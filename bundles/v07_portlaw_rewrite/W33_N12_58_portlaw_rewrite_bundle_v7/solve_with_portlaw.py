...
def main() -> None:
    ...
    ap.add_argument("--mapping", type=str, required=True, help="CSV: w33_point,n12_point")
    ...
    w33_to_n12 = dict(zip(df_map["w33_point"].astype(int), df_map["n12_point"].astype(str)))
    ...
    ray_csv = ROOT / "_toe/w33_orthonormal_phase_solution_20260110/W33_point_rays_C4_complex.csv"
    ...
    cand_points = next((ROOT / "_n12").glob("n12_58_candidate_w33_points_40_from_tau_cycles_and_fixed_complements_*.csv"))
    ...
    for i,(a,b,c) in enumerate(four_triads):
        tri_mask[i]=point_mask[w33_to_n12[a]] | point_mask[w33_to_n12[b]] | point_mask[w33_to_n12[c]]
    ...
    df_a["rem_idx"]=df_a.apply(lambda r: matching_index_from_pairs(r["support_list"], r["removed_pairs"]), axis=1)
    df_a["add_idx"]=df_a.apply(lambda r: matching_index_from_pairs(r["support_list"], r["added_pairs"]), axis=1)
    edge_dict={}
    ...
    def solve_v3_and_get_paths():
        ...
        for cyc in cycle_defs:
            ...
            for i in range(L):
                ...
                if deltas[i]==2: ok &= (tri_h==9)
                elif deltas[i]==6: ok &= (tri_h==3)
                eligible.append(ok)
            best_cost=INF; best_path=None
                ...
                for i in range(1,L):
                    ...
                    for tp in active:
                        ...
                        for tc in adj_self[tp]:
                            ...
                            if nc<next_cost[tc]:
                                next_cost[tc]=nc; prev[i][tc]=tp
                    cost=next_cost
                active=np.where(cost<INF)[0]
                for tL in active:
                    if t0 not in adj_self[tL]: continue
                    dprev=deltas[L-1]
                    holL=int(tri_h[tL]); hol0=int(tri_h[t0])
                    if dprev==0 and hol0!=holL: continue
                    if dprev==4 and hol0==holL: continue
                    ctot=int(cost[tL])
                    if ctot<best_cost:
                        path=[None]*L
                        path[L-1]=int(tL)
                        ...
                        for k in range(L-1,0,-1):
                            ...
                            if path[k-1]<0: ok=False; break
                        if ok and path[0]==t0:
                            best_cost=ctot; best_path=path
    ...
    for cyc, solved in zip(cycle_defs, v3_paths):
        L=cyc["L"]; edges=cyc["edges"]; path=solved["path"]
        for i in range(L):
            e=edges[i]
            tp=path[i]; tc=path[(i+1)%L]
        ...
        for cyc in cycle_defs:
            ...
            for t0 in np.where(eligible[0])[0]:
                ...
                for i in range(1,L):
                    ...
                    allowed=set([0,1,2]) if rule is None else set(rule["move_edge_idxs"])
                    active=np.where(cost<INF)[0]
                    for tp in active:
                        for tc in adj_self[tp]:
                            if not eligible[i][tc]:
    ...
    for cyc_idx,(cyc, solved) in enumerate(zip(cycle_defs, port_paths)):
        ...
        for i in range(L):
            ...
            rows.append(dict(
                ...
                u=e["u"], v=e["v"],
                delta=e["delta"],
                rem_idx=e["rem_idx"], add_idx=e["add_idx"],

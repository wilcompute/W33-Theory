...
def main():
    ...
    # point masks
    w33_mask={int(r.w33_point): int(r.n12_mask) for r in mapping.itertuples(index=False)}
    ...
    def centers(a,b,c): return col[a] & col[b] & col[c]
    four_center=[]
    for a in range(40):
        for b in range(a+1,40):
            if b in col[a]: continue
            for c in range(b+1,40):
                if c in col[a] or c in col[b]: continue
                ctr=centers(a,b,c)
                if len(ctr)==4:
    ...
    def solve_cycle(supp_masks, deltas):
        ...
        for start in allowed[0]:
            dp=[{start:cost[start]}]; prev=[{start:None}]
            for s in range(1,L):
                cur={}; cur_prev={}
                for j in allowed[s]:
                    bestp=None; bestv=INF
                    for i in adj[j]:
                        if i in dp[s-1]:
                            v=dp[s-1][i]+cost[j]
                            if v<bestv:
                    ...
                    if bestp is not None:
                        cur[j]=bestv; cur_prev[j]=bestp
                if not cur: break
                dp.append(cur); prev.append(cur_prev)
            if len(dp)<L: 
                continue
    ...
    for idx,row in cycles.iterrows():
        ...
        for s in range(L):
            ...
            out_rows.append(dict(
                ...
                delta=int(deltas[s]),
                required_triadhol=(9 if deltas[s]==2 else 3 if deltas[s]==6 else None),
                triad_index=int(t),
                ...
                triad_missing_symbols=mask_to_symbols(ALL12 ^ int(triad_mask[t])) if triad_mask[t]!=ALL12 else ""
            ))
    df=pd.DataFrame(out_rows)
    df.to_csv(f"{args.outdir}/phase_aware_2T_cycle_witness_walks.csv", index=False)
    summary=dict(
        phase_constraints={"delta==2": "triad_holonomy_z12==9", "delta==6": "triad_holonomy_z12==3", "delta in {0,4}":"unconstrained"},

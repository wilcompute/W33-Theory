...
| `gap -q analysis/w33_pass5667_5674_q5_reye_equivariant_orientation.g` | exact 56/56 cross-q certificate: the q=5 moving twelve is `T12_165`; the `2-(13,6,60)` multidesign reconstructs Reye as sixteen zero triples; the line action is `T16_1034`; point/heavy/line character self products are `3`, pairwise products are `2`, and sign-twisted point-to-heavy/line products are `0`, proving that sign twist is neither the explicit outer point/heavy involution nor the line carrier; every natural `PSL(2,7)` `7+1⁵` placement joins it to `S₁₂` |
| `python3 analysis/w33_pass4870_steiner_w33_quadratic_bridge.py` | historical Steiner three-cover producer: 120 triangles form forty intrinsic three-element fibers and the relevant relation lifts to complete `K₃,₃`; Pass 4949 corrects the quotient carrier to the `Q(4,3)` line side, not W33, and no preferred map or coupling is selected |
...
| `python3 analysis/w33_pass4874_steiner_w33_association_scheme.py` | complete four-class scheme on the 120 Steiner triangles: valencies `1,2,27,36,54`, `Q(4,3)` line-side fiber-constant sector `1+24+15`, and transverse sector `20+60`; the relations are canonical, but naming elements inside each three-element fiber remains gauge data |

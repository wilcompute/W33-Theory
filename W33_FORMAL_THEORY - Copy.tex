...
The W(3,3) configuration (Witt configuration) is defined as the configuration of external points with respect to an oval in the projective plane $\mathrm{PG}(2,3)$ over the field with 3 elements.
\end{definition}
...
\begin{proof}
The point and line counts follow from the definition of external points to an oval in $\mathrm{PG}(2,3)$. The cycle count $81 = 3^4$ follows from the combinatorial structure. The K4 count is established by direct enumeration of index-2 subgroups. See \cite{coolsaet2004} for complete details.
\end{proof}

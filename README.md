...
The promoted theorem stack expects the repo Python dependencies to be present.
If imports fail with messages about optional packages such as `networkx`, use
the repo bootstrap path instead of debugging package issues by hand.
...
If your repo lives on a slow Windows-mounted path under WSL, you can keep the
virtualenv on the Linux filesystem instead:

```bash
VENV_DIR="$HOME/.venvs/w33-theory" ./scripts/bootstrap_repo_env.sh
...
- `make audit`: clutter-focused inventory of the current worktree
- `python3 tools/repo_doctor.py --json`: machine-readable environment report
...
- **CCCLXXV:** Holographic Transport-Twisted Realization Channel (36 tests) — the post-`CCCLXXIV` wall is now sharper than target uniqueness alone. The transport Bose-Mesner / heat selector canonically fixes one protected flat `81`-dimensional matter copy, the exact transport-twisted curvature kills the protected invariant columns and factors through only the complementary `81` copy, and the unique avatar already fixes the tail-to-head activation slot on `81→162→81`. So any genuine K3-side transport-twisted realization must preserve the protected head `81` and realize its nontrivial action only through the complementary curvature-sensitive tail `81`. The live wall is now existence of one tail-to-head realization channel on the unique avatar.

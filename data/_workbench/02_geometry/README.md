# Geometry / cocycle / domain-wall

Core geometry artifacts:
- `data/_toe/geometric_reduction_20260110/README.md`
- `data/_toe/geometric_reduction_20260110/orbit0_edges_Z2_cocycle.csv`
- `data/_toe/geometric_reduction_20260110/orbit0_cocycle_summary.json`
- `data/_workbench/02_geometry/orbit0_cocycle_cut_summary.md`
- `data/_workbench/02_geometry/orbit0_cocycle_cut_components.csv`

Harmonic lift + geometry predictor:
- `data/_toe/harmonic_lift_geometry_predictor_20260110/REPORT.md`
- `data/_toe/harmonic_lift_geometry_predictor_20260110/geometry_predictor_vs_observed_flux_sensitivity.csv`

Domain-wall mincut + Q12 variance:
- `data/_toe/domainwall_to_w33_sensors_20260110/REPORT.md`
- `data/_toe/domainwall_to_w33_sensors_20260110/summary.json`
- `data/_toe/domainwall_to_w33_sensors_20260110/W33_lines_flux_sensitivity_with_Q12_variance.csv`

W33 phase inventory:
- `data/_workbench/02_geometry/W33_phase_stats.md`
- `data/_workbench/02_geometry/W33_line_phase_map.md`
- `data/_workbench/02_geometry/W33_line_phase_map.csv`
- `data/_workbench/02_geometry/W33_line_phase_k12_map.md`
- `data/_workbench/02_geometry/W33_line_phase_k12_map.csv`
- `data/_workbench/02_geometry/W33_line_phase_flux_join.md`
- `data/_workbench/02_geometry/W33_line_phase_flux_join.csv`
- `data/_workbench/02_geometry/projective_phase_alignment.md`
- `data/_workbench/02_geometry/projective_class_phase_alignment.csv`
- `data/_workbench/02_geometry/projective_sector_phase_alignment.csv`
- `data/_workbench/02_geometry/projective_m_phase_alignment.csv`
- `data/_workbench/02_geometry/line_feature_table_summary.md`
- `data/_workbench/02_geometry/line_feature_table.csv`
- `data/_workbench/02_geometry/line_feature_regression.md`
- `data/_workbench/02_geometry/line_feature_regression.csv`



...
\usepackage{geometry}
\usepackage{graphicx}
\usepackage{svg}          %% for \includesvg (requires Inkscape on PATH)
\usepackage{xcolor}
\usepackage{enumitem}
...
\begin{figure}[H]
  \centering
  %%
  %% Option A (recommended for arXiv): convert SVG to PDF first:
  %%   inkscape figures/fig1_spectral_diagram.svg \\
  %%            --export-pdf=figures/fig1_spectral_diagram.pdf
  %% Then use:
  %%   \includegraphics[width=0.85\textwidth]{figures/fig1_spectral_diagram}
  %%
  %% Option B (requires svg package + Inkscape on PATH):
  \includesvg[width=0.85\textwidth]{figures/fig1_spectral_diagram}
  %%
  \caption{The $\W$ eigenvalue spectrum. Three horizontal levels represent
    $\lambda_1 = 12$ (gravity singlet, $\times 1$),
    $\lambda_2 = +2$ (quark--lepton sector, $\times 27$, matching $\dim\mathbf{27}$
    of $\Esix$), and $\lambda_3 = -4$ (SM gauge bosons, $\times 12$).
    The master identity $E = \nv\times k = 480 = |\Phi(\Eone)|$ and
    the fine-structure formula $\alpha^{-1} = k^2 - (|r|+|s|+1) = 137$
    are annotated.}
  \label{fig:spectrum}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includesvg[width=0.72\textwidth]{figures/fig3_spectral_decomposition}
  \caption{Spectral sector decomposition of $\W$. The 40 degrees of freedom
    decompose as $1 + 27 + 12 = 40$ (spectral completeness), matching
    the gravity singlet, the $\mathbf{27}$ of $\Esix$, and the 12
    SM gauge bosons respectively.}
...
\begin{figure}[H]
  \centering
  \includesvg[width=0.92\textwidth]{figures/fig2_predictions_timeline}
  \caption{Timeline of the eight $\W$-theory falsifiable predictions
    (2026--2045). F1 ($\thetaTT=45^\circ$, JUNO 2027) is the critical
    near-term test; F3 ($\sumnu=30.7\,\mathrm{meV}$, KATRIN/CMB-S4)
    and F5 ($\deltaCP=80.1^\circ$, DUNE) form a five-year conjunctive
    window. F7 ($Z'$ at 1094\,GeV) requires FCC-hh.}
  \label{fig:timeline}
\end{figure}
...
$\{40,\,12,\,2,\,-4,\,27,\,12\}$
determine the structure of the Standard Model is,
to our knowledge, unprecedented in the literature.''}
\end{center}
...
\begin{verbatim}
# Option A: SVG figures via Inkscape (recommended)
...
# Option B: omit svg package, use \includegraphics{figures/fig*.pdf}
# after converting SVGs as above
\end{verbatim}

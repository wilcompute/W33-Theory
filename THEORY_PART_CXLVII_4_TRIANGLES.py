...
n_n = len(neighbors)
induced_adj = [[is_orthogonal(neighbors[i], neighbors[j]) 
                for j in range(n_n)] for i in range(n_n)]
...
sup_adj = [[is_orthogonal(i, j) for j in superposition_neighbors] 
        ...
        if sup_adj[i][j]:
            for k in range(j+1, 9):
                if sup_adj[i][k] and sup_adj[j][k]:
                    sup_triangles.append((superposition_neighbors[i], 
...
for i in range(9):
    ...
    for j in range(i+1, 9):
        ...
        if is_orth != predicted:
            print(f"  MISMATCH at ({mu_i},{nu_i})-({mu_j},{nu_j}): actual={is_orth}, predicted={predicted}")

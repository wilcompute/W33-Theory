...
\begin{spec}[The graph's numbers are modular-form numbers]
Ramanujan's $\tau$, the coefficients of the weight-$12$ discriminant $\Delta = \eta^{24}$:
\begin{center}\small
\begin{tabular}{@{}rrl@{}}
\toprule
$n$ & $\tau(n)$ & in graph terms\\
\midrule
$2$ & $-24$ & $-f$, the eigenvalue multiplicity\\
$3$ & $252$ & $E + k = 240 + 12$\\
$6$ & $-6048$ & $\tau(2)\tau(3) = -f(E+k)$, by multiplicativity\\
\bottomrule
\end{tabular}
\end{center}
And the weight of $\Delta$ is $12$ --- the degree of the graph.
\end{spec}

\begin{spec}[Topology of the clique complex]
\[
  \chi = 40 - 240 + 160 = -40 = -v,\qquad
  \text{embedded genus } 21 = 3\times7 = \binom{7}{2} .
\]
The $160$ triangles are $v k \lambda/6$, so the whole complex is determined by the four
...
It also sharpens \S\ref{sec:budget}'s comparison considerably. The address layer does not
merely beat the instruction layer; it is provably at an optimum the instruction layer
misses.
\end{plain}
...
\begin{spec}[Is the instruction layer extremal? Pass 3042]
The address graph is \emph{Ramanujan} --- provably the best-mixing graph of its
degree (\S\ref{sec:geometry}). For a $4$-regular graph the same optimality condition is
\[
  |\lambda_2| \le \frac{2\sqrt3}{4} = 0.866025,
\qquad
  \text{measured: } 0.893992 .
\]
\textbf{The instruction graph is not Ramanujan.} It misses by $3.23\%$: a good
expander, not the best one.
\end{spec}

\headline{The machine's geometry is perfect and its algebra is merely very good,
and every way of measuring it says so.}
...
The frame Cayley graph is $4$-regular & Degrees run $2$ to $8$: the opcodes are not
  involutions and their inverses are not generators, so the undirected graph is not
  regular & Printing the degree sequence before using a $k$-regular formula \\

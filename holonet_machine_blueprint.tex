...
\footnotesize
\noindent\makebox[\linewidth][l]{\begin{minipage}{0.97\linewidth}
\begin{tabular}{@{}p{2.9cm}p{1.8cm}p{7.2cm}@{}}
\toprule
\textbf{Representation} & \textbf{Width} & \textbf{Contract} \\
\midrule
Affine frame code & $7$ bits & Lossless for all $81$ frames; execution may read and write it. \\
Support telemetry & $4$ bits & Zero/nonzero mask only; frame $\to$ support is one-way in the netlist. \\
Protected observer & $24$--$28$ taps & Diagnostic word for identification, not the live datapath. \\
Typed control token & $7$ raw, $96$ valid & Selects face, matching channel, and tomotope phase cell. \\
\bottomrule
\end{tabular}
\end{minipage}}
\end{spec}

...
ac_path = ROOT / 'data' / '_workbench' / '04_measurement' / 'action_candidate_features.csv'
...
node_path = ROOT / 'data' / '_workbench' / '04_measurement' / 'node_commutator_line_scores.csv'
node_scores = {}
if node_path.exists():
...
mixed_path = ROOT / 'data' / '_workbench' / '04_measurement' / 'mixed_predictor_oddness_defect_scores.csv'
mixed_scores = {}
...
e_star_path = ROOT / 'data' / '_workbench' / '04_measurement' / 'e_star_oddness_per_line.csv'
e_star = {}
...
# ranks and top lists
rows_sorted_native = sorted(rows, key=lambda r: r['native_mean_abs_delta'], reverse=True)
...
for r in rows:
    r['rank_native'] = next(i+1 for i, rr in enumerate(rows_sorted_native) if rr['line_id']==r['line_id'])
    r['rank_prior'] = next(i+1 for i, rr in enumerate(rows_sorted_prior) if rr['line_id']==r['line_id'])
    r['rank_fit'] = next(i+1 for i, rr in enumerate(rows_sorted_fit) if rr['line_id']==r['line_id'])
    r['rank_node_commutator'] = next(i+1 for i, rr in enumerate(rows_sorted_node) if rr['line_id']==r['line_id'])
    r['rank_mixed'] = next(i+1 for i, rr in enumerate(rows_sorted_mixed) if rr['line_id']==r['line_id'])
    r['rank_e_star_oddness'] = next(i+1 for i, rr in enumerate(rows_sorted_e_star) if rr['line_id']==r['line_id'])
...
# write CSV
fieldnames = [
    'line_id','native_mean_abs_delta','var_q_class','var_q_r','unique_k_mod6','unique_k_mod3',
    'k12_entropy','k12_kl_global','tau_shift_similarity','prior_score','fit_score',
    'node_commutator_score','mixed_score','e_star_oddness',
    'rank_native','rank_prior','rank_fit','rank_node_commutator','rank_mixed','rank_e_star_oddness',
    'in_top_native','in_top_prior','in_top_fit','in_top_node_commutator','in_top_mixed','in_top_e_star_oddness'
]

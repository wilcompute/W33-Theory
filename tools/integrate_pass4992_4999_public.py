...
CARD=ROOT/'analysis/PASS4992_4999_index_insert.html'
TOKEN=b'W33_PASS4992_4999_OCT_COCIRCUIT_CHAIN_CARD'
SECTION=b'<section id="pass4992-4999"'

def main()->int:
    card_text=CARD.read_text(encoding='utf-8').strip()
    status=[]
    for path in INDEXES:
        ...
        path.write_bytes(raw);assert raw.count(TOKEN)==1
        status.append(f'{path.relative_to(ROOT)}:{state}')

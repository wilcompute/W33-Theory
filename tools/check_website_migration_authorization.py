...
def validate_website_index(
    repository_root: Path,
    *,
    expected_blob: str = RESTORED_INDEX_BLOB,
    ...
    if actual_blob == expected_blob:
        return {
            "status": "PASS_RESTORED_INDEX_UNCHANGED",
    ...
    _require(
        ...
        "docs/index.html changed; an authorization file must be supplied explicitly with --authorization",
    )
    auth_file = (root / authorization_path).resolve()
    _require(root == auth_file or root in auth_file.parents, "authorization path escapes repository")
    _require(auth_file.is_file(), f"explicit authorization file missing: {authorization_path}")
    ...
    _require(
        authorization.get("authorization_phrase") == AUTHORIZATION_PHRASE,
        "missing exact authorization phrase",
    )
    _require(authorization.get("approved_by") == "wilcompute", "approved_by must be wilcompute")
    reason = authorization.get("reason")
    _require(isinstance(reason, str) and reason.strip(), "authorization reason must be nonempty")
    ...
    archive_value = authorization.get("archive_path")
    _require(isinstance(archive_value, str) and archive_value.strip(), "archive_path must be nonempty")
    archive_path = (root / archive_value).resolve()
    _require(root == archive_path or root in archive_path.parents, "archive_path escapes repository")
    _require(archive_path.is_file(), f"authorized archive missing: {archive_value}")
    archive_blob = git_blob_sha(archive_path.read_bytes())
    _require(archive_blob == expected_blob, "archive is not an exact copy of the previous authoritative index")
    _require(authorization.get("archive_blob") == archive_blob, "authorization archive_blob mismatch")

    return {
        "status": "PASS_EXPLICIT_INDEX_MIGRATION_AUTHORIZATION",
        "actual_blob": actual_blob,
        "expected_blob": expected_blob,
...
def main() -> None:
    ...
    parser.add_argument("--expected-blob", default=RESTORED_INDEX_BLOB)
    parser.add_argument(
        "--authorization",
        type=Path,
        default=None,
        help="Explicit one-shot authorization file; never discovered automatically.",
    )
    parser.add_argument("--json", type=Path)
    args = parser.parse_args()

    try:
        result = validate_website_index(
            args.repository_root,
            expected_blob=args.expected_blob,
            authorization_path=args.authorization,
        )
    except AuthorizationError as exc:
        print(f"FAIL_WEBSITE_MIGRATION_AUTHORIZATION: {exc}")

...
RULES={
 ...
 'witting_q43_identity': re.compile(r'witting.{0,120}(?:is|=|identif\w*).{0,80}(?:Q\s*\(\s*4\s*,\s*3\s*\)|steiner)',re.I|re.S),
...
def main()->int:
 ...
 out={'pass':4996,'status':'PASS' if not violations else 'FAIL','rules':sorted(RULES),
      ...
      'theorem':'Known retracted W33/Q43/Witting claims are now fail-closed on live text surfaces; only explicit historical/erratum paths may quote them. The firewall also asserts the corrected Ihara, critical-group, and no-point-line-correlation replacements remain present.',

...
def build() -> dict:
    ...
    return {
        ...
        "edges": [{"id": i, "u": a, "v": b, "dir": d, "support_vertices": [a, b]} for i, (a, b, d) in enumerate(edges)],
        "faces": [{"id": i, "dims": [di, dj], "base_vertex": base, "support_edges": support} for i, (di, dj, base, support) in enumerate(faces)],
        "ranks": {"rank_boundary_1": r1, "rank_boundary_2": r2, "n_edges": len(edges), "css_k_naive_n_minus_ranks": k},
...
def main() -> None:
    ...
    ap.add_argument("--out", type=Path, default=ROOT / "data" / "bt1338_q4_chain_check_matrices.json")
    ns = ap.parse_args()
    result = build()
    ns.out.parent.mkdir(parents=True, exist_ok=True)
    ns.out.write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({"bt": 1338, "verified": True, "k_naive": result["ranks"]["css_k_naive_n_minus_ranks"]}, indent=2))

...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default=ROOT
        / "data"
        / "bt1397_example_optimality_certificate_verification.json",
    )
    ns = ap.parse_args()
    cert = ROOT / "examples" / "bt1397_example_s3_maxsat_optimality_certificate.json"
    with tempfile.TemporaryDirectory(prefix="bt1397_") as tmp:
        example_out = Path(tmp) / "bt1395_example_pathway.json"
        proc = subprocess.run(
            [
                sys.executable,
                str(ROOT / "tools" / "bt1395_s3_maxsat_bound_pathway.py"),
                "--certificate",
                str(cert),
                "--out",
                str(example_out),
    ...
    checks = {
        ...
        "example_reaches_optimal_certified_path": parsed["optimality_status"]
        == "optimal_certified",
        "example_marked_not_solver_generated": example["solver"]
        == "example-demonstration-not-solver-generated",
        "source_warns_not_project_proof": "Not a project-level global optimality proof"
        in example["source"],
        ...
        json.dumps(
            {
                ...
                "project_optimality_status": result["project_optimality_status"],
            },
            indent=2,
        )
    )

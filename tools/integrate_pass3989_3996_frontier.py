...
def main():
    ...
    print('PASS_3989_3996_FRONTIER_RECONCILE',required.count(REQUIRED),stale,sum(x.get('token')==PUBLIC['token'] for x in sections),sum(x.get('token')==PAGE['token'] for x in pages))

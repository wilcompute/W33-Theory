...
\title{%
  ...
  and a Historical Programme in Fundamental Physics}\\[0.4cm]
  {\large\itshape Superseded development draft; retained for provenance}\\[0.3cm]
  {\normalsize Current mathematical account: \texttt{w33\_paper.tex}}
}
...
\begin{abstract}
This file is an archived development draft, not the current claim surface.
It records how a broad physical interpretation was explored from the
symplectic polar space $W(3,3)$.  Later exact audit refuted the shifted
degree-$32$ spectral packet on which several chapters depend: on the actual
$40$-point carrier, $D=A-I$ has spectrum
$11^1\oplus1^{24}\oplus(-5)^{15}$.  There are also $28$, not one,
non-isomorphic strongly regular graphs with parameters
$\SRG(40,12,2,4)$; the symplectic construction selects the graph used here.
The maintained theorem layer is \texttt{w33\_paper.tex}, and the accessible
account is \texttt{W33\_FOR\_EVERYONE.tex}.  The material below should be
read as provenance unless a current named witness and certificate establish
...
\begin{equation}
  ...
  A^2 = (k-\mu)I + (\lambda-\mu)A + \mu J
      = 8I - 2A + 4J.
\end{equation}
The algebra $\langle A_0,A_1,A_2\rangle$ is three-dimensional,
equalling $\mathrm{End}_{W(E_6)}(\RR^{40})$ and forcing the
...
Choosing the Higgs direction as a distinguished Fano point $e_3$,
exactly $q=3$ Fano lines pass through $e_3$.  Each line connects one
space direction to one colour direction:
\begin{equation}
  \text{Gen 1: } \{e_2,e_3,e_5\},\quad
  \text{Gen 2: } \{e_3,e_4,e_6\},\quad
  \text{Gen 3: } \{e_7,e_1,e_3\}.
\end{equation}
The three generations exhaust all lines through the Higgs, so
\textbf{there are exactly $q=3$ generations}.
\end{theorem}

The remaining $\mu=4$ lines not through the Higgs decompose as:
\begin{itemize}[nosep]
\item 1 all-space line $\{e_1,e_2,e_4\}$: the quaternion subalgebra $\HH$ (gravity/Lorentz);
\item 3 mixed lines (1 space + 2 colour each): the $\SU(3)$ gluon vertices.
\end{itemize}
Total: $7=3+1+3=\Phisix$ Fano lines = Yukawa + gravity + gluons.

\subsection{Yukawa coefficients as \texorpdfstring{$W(3,3)$}{W(3,3)} rationals}

The Yukawa operator $Y_s$ from the normal form of the
Bott~5$\otimes$triality~3 module (dimension $g=15$) has four exact coefficients:
\begin{align}
  Y_{21} &= \frac{q^2}{v} = \frac{9}{40},&
  Y_{22}^{\mathrm{trip}} &= \frac{q}{v-q} = \frac{3}{37},\nonumber\\
  Y_{22}^{\mathrm{down}} &= \frac{\mu+1}{2\Phisix(v-q)} = \frac{5}{518},&
  Y_{32} &= \frac{1}{q^3} = \frac{1}{27}.
\end{align}
All denominators are $W(3,3)$ expressions: $v=40$, $v-q=37$,
$2\Phisix(v-q)=518$, $q^3=27$.

\subsection{\texorpdfstring{$G_2$}{G2} Casimirs and the Koide angle (Music 2026)}

Music~(2026) derived $\theta_0=2/9$ from $G_2=\Aut(\OO)$ representation
theory: the ratio of SU(3) Casimir invariants
$C_2(\mathbf{3})/C_2(\mathrm{Sym}^3\!(\mathbf{3})) = (4/3)/6 = 2/9$.
In $W(3,3)$ language:
\begin{equation}
  \theta_0 = \frac{C_2(\mathbf{3})}{C_2(\mathrm{Sym}^3(\mathbf{3}))}
  = \frac{\mu/q}{2q} = \frac{\mu}{2q^2}
  = \frac{\lambda}{q^2} = \frac{2}{9},
\end{equation}
where $C_2(\mathbf{3})=\mu/q=4/3$ and $C_2(\mathrm{Sym}^3)=2q=6$.
This provides an independent derivation of our Koide phase.
...
\subsection{All three couplings at \texorpdfstring{$M_Z$}{MZ}}

The electromagnetic coupling runs from its Thomson-limit value
$\alpha^{-1}(0)=137$ to $M_Z$ by exactly $q^2$:
\begin{equation}
  \alpha^{-1}(M_Z) = \alpha^{-1}(0) - q^2 = 137-9 = 128.
\end{equation}
(Measured: $127.951$; error $0.04\%$.)
All three $M_Z$ couplings then follow:
\begin{align}
  \alpha_1^{-1} &= \tfrac{3}{5}\,\tfrac{\Phithree-q}{\Phithree}\,(\alpha^{-1}{-}q^2)
    = 59.08 &&(\text{exp: }59.0),\\
  \alpha_2^{-1} &= \tfrac{q}{\Phithree}\,(\alpha^{-1}{-}q^2) = 29.54
    &&(\text{exp: }29.6),\\
  \alpha_3^{-1} &= \Phithree^2/(\lambda\Theta) = 8.45
    &&(\text{exp: }8.5).
\end{align}
The unified coupling is $\alpha_{\text{GUT}}^{-1}=f=24=(q+1)!$.
Frampton--Mohapatra (1999) showed that $\sin^2\theta_W=3/13$ is the
trinification ratio $\SU(3)^3\to G_{\mathrm{SM}}$, confirming
it is geometric rather than an artefact of running.
...
\begin{equation}
  \delta_{\mathrm{CKM}} = \arctan\!\biggl(\frac{v-\lambda}{g}\biggr)
  = \arctan\!\frac{38}{15} = 68.5^\circ
  \quad(\text{exp: }68.4^\circ\pm 2.0^\circ).
\end{equation}
\end{theorem}
...
\begin{theorem}
\begin{equation}
  \frac{m_p}{m_e} = \alpha^{-1}\Phithree + v + g
  = 137\times 13 + 40 + 15 = 1781 + 55 = 1836.
\end{equation}
Note $v+g=55=F_{10}$ (10th Fibonacci number).
Alternatively, $m_p/m_e = k\cdot T(q^2+2^q) = 12\cdot T(17) = 12\times 153 = 1836$.
Experimental: $1836.153$; error $0.008\%$.
\end{theorem}
...
\subsection{\texorpdfstring{$D_5\subset E_6$}{D5 subset E6}: 40 roots and 40 points}

The root system $D_5$ has exactly $4\binom{5}{2}=40$ roots $\pm e_i\pm e_j$
($1\le i<j\le 5$), the same cardinality as $v=40$ points of $\GQ(3,3)$.
The $D_5$ root graph (adjacency $=$ inner product $+1$) is $12$-regular
but \emph{not} strongly regular with the same parameters: its $\lambda$
value is $5$ (not $2$), and $\mu$ is not constant.  The connection is
\textbf{group-theoretic}, not graph-isomorphic:

\begin{theorem}[PSp(4,3) bridge]
The group $\PSp(4,3)\cong W(E_6)^+$ (order $25920$) acts transitively
on \emph{both}:
\begin{itemize}[nosep]
\item the 40 roots of $D_5\subset E_6$ (via the Weyl group action);
\item the 40 isotropic 1-dim subspaces of $(\FF_3^4,\omega)$ (the GQ points).
\end{itemize}
These are two \emph{distinct} faithful actions of the same abstract group
on 40-element sets, each yielding a 12-regular graph with different fine
structure.
\end{theorem}

The $E_6$ roots decompose under $D_5\times U(1)$:
\begin{equation}
  72 = 40 + 16 + 16 = v + 2^{q+1} + 2^{q+1}
  = (D_5\ \text{roots}) + (\text{spinor}) + (\overline{\text{spinor}}).
\end{equation}
Equivalently, $72 = v + 2^{q+\lambda}$: the GQ(3,3) points plus the
$Z(x)$ representation space.

\subsection{\texorpdfstring{$F_4$}{F4} as GQ(3,3) + Standard Model}

\begin{theorem}[$F_4$ decomposition]
\begin{equation}
  \dim(F_4) = 52 = v + k = 40 + 12
  = |\GQ(3,3)\ \text{points}| + \dim(\SU(3)\times\SU(2)\times U(1)).
\end{equation}
The GQ(3,3) point set IS the coset $F_4/G_{\mathrm{SM}}$.
\end{theorem}

Todorov and Dubois-Violette~(2018) proved
$G_{\mathrm{SM}}=S(U(2)\times U(3))=\mathrm{Spin}(9)\cap(\SU(3)\times\SU(3))/\ZZ_3$
inside $F_4=\Aut(J_3(\OO))$. The coset dimensions:
\begin{alignat}{2}
  F_4/\mathrm{Spin}(9) &= 52-36 = 16 &&= 2^{q+1}\ (\text{matter fermions}),\\
  \mathrm{Spin}(9)/G_{\mathrm{SM}} &= 36-12 = 24 &&= f\ (\text{SU(5) GUT sector}),\\
  (\SU_3\!\times\!\SU_3)/G_{\mathrm{SM}} &= 16-12 = 4 &&= \mu\ (\text{spacetime dimensions}).
\end{alignat}
...
The heat kernel of the $32{\times}32$ operator $M$,
$K(t) = 10\,e^{-25t} + 16\,e^{-t} + 6\,e^{-49t}$,
produces a sharp-cutoff spectral hierarchy:
$32\to 26\to 16\to 0$ as $\Lambda$ decreases through
$\Phisix$, $q{+}\lambda$, $1$---matching the chain
full $\to$ $F_4$ fundamental $\to$ $\SO(10)$ spinor.
The spectral zeta function satisfies
$\zeta_M(1) = (\alpha^{-1}{-}(q{+}\lambda))/\Phisix = 132/7$.
...
The continued-fraction digits appearing in $W(3,3)$
constants are exactly
$\{1,\allowbreak 2,\allowbreak 3,\allowbreak 4,\allowbreak 5,\allowbreak 7,\allowbreak 8,\allowbreak 20\}$,
i.e.~$1,\lambda,q,\mu,q{+}\lambda,\Phisix,2^q,v/\lambda$;
no others arise.
...
The number of $e$-folds is $N=v+\Theta+\Phisix+q = 40+10+7+3 = 60$,
also expressible as $k(q+\lambda)=|A_5|=60$.
\begin{align}
  n_s &= 1-2/N = 58/60 \approx 0.9667 &&(\text{exp: }0.965\pm 0.004),\\
  r &= k/N^2 = 12/3600 = 1/300 \approx 0.0033 &&(\text{CMB-S4 target}).
\end{align}
The tensor-to-scalar ratio $r=1/300$ is within the reach of
CMB-S4 ($\sigma_r\sim 0.001$).

\subsection{Hubble tension}

\begin{theorem}[Hubble tension resolution]
\begin{equation}
  \frac{H_0^{\mathrm{SH0ES}}}{H_0^{\mathrm{Planck}}}
  = \frac{\Phithree}{\Phithree-1} = \frac{13}{12} = 1.0833.
\end{equation}
Measured ratio (2025): $73.04/67.24 = 1.0863$ ($0.3\%$ error).
The true Hubble constant is the geometric mean:
\begin{equation}
  H_0^{\mathrm{true}} = H_0^{\mathrm{Planck}}\sqrt{\Phithree/k}
  \approx 70.2~\text{km/s/Mpc}.
\end{equation}
The early/late discrepancy is a $\Phithree/k$ scale shift between
cosmological epochs.  The tension has deepened to $>6\sigma$
(Corfu 2025), strengthening the importance of this resolution.
\end{theorem}

\subsection{Muon \texorpdfstring{$g{-}2$}{g-2}: anomaly absence}

\begin{theorem}[$g{-}2$ predicts no large NP]
The $W(3,3)$ contribution to $\Delta a_\mu$ is suppressed by
$\alpha^2$ and dimensionless geometric factors, giving
$\Delta a_\mu^{\mathrm{W(3,3)}}\sim 10^{-10}$.
\end{theorem}
Fermilab Muon $g{-}2$ final result (PRL 135, 2025) combined with
lattice QCD HVP (Theory Initiative WP 2025; BMW 2026) gives
$\Delta a_\mu^{\mathrm{exp{-}SM}} = 38(63)\times 10^{-11}$
($0.6\sigma$, BMW: $0.5\sigma$): the previous $5\sigma$ anomaly
is effectively resolved, in agreement with the $W(3,3)$ prediction
that there is no large NP contribution.

\subsection{Matter--antimatter asymmetry}

\begin{equation}
  \eta_B = \frac{n_B}{n_\gamma}
  = \frac{J_{\mathrm{CKM}}\cdot\alpha}{k\cdot\mu^2\ln\Theta}
  = \frac{(3.08{\times}10^{-5})/137}{12\cdot 16\cdot\ln 10}
  = 5.1\times 10^{-10}.
\end{equation}
Observed (Planck): $\eta_B = 6.1\times 10^{-10}$ ($16\%$ error;
leading-order prediction).

\subsection{Strong CP problem}

\begin{theorem}[Vanishing $\theta_{\mathrm{QCD}}$]
The quaternion Fano line $\{e_1,e_2,e_4\}$ has $\ZZ_2$ reflection
symmetry that forbids a non-zero QCD vacuum angle.  Thus
$\theta_{\mathrm{QCD}}=0$ exactly, with no Peccei--Quinn axion required.
\end{theorem}

\subsection{The 33rd prime}

\begin{theorem}
$\alpha^{-1}=137$ is the $33^{\mathrm{rd}}$ prime.
$33=|\mathrm{Vieta}_2|$ of the master cubic equals the neutrino
splitting ratio $\Delta m^2_{31}/\Delta m^2_{21}$.
The fine structure constant is the prime indexed by the master
cubic invariant.
\end{theorem}

\subsection{New particles predicted}

\begin{itemize}[nosep]
\item Right-handed neutrinos: $M_{\nu_R}\sim 10^{15}$~GeV (3 generations).
\item Dark-matter scalar: $M_{\mathrm{DM}}\sim\Phisix\,\vEW\approx 1.7$~TeV.
\item Heavy gauge bosons $Z',W'$: $M\sim 4$~TeV (LHC reach).
\item \textbf{No} SUSY partners (zero free parameters).
\item \textbf{No} axion (Strong CP solved geometrically).
\item \textbf{No} sterile neutrinos: $q=3$ \emph{active} flavours only.
  Confirmed: MicroBooNE (Nature, Dec.\ 2025) excludes 3+1 at $95\%$ CL;
  KATRIN (2025) excludes the gallium anomaly best fit at $96.6\%$ CL.
\end{itemize}
...
% ═══════════════════════════════════════════════════════════════
\section{The Ihara Zeta Function and Fermionic Partition Function}
\label{sec:ihara}

The GQ(3,3) collinearity graph $\Gamma$ has $V=40$ vertices,
$E=240$ edges, and is 12-regular.  Being Ramanujan
($\max|\lambda_i|=4<2\sqrt{11}$), its Ihara zeta function converges:
\begin{equation}
  \zeta_{\Gamma}(u) = \frac{(1-u^2)^{200}}
  {(1-12u+11u^2)(1-2u+11u^2)^{24}(1+4u+11u^2)^{15}}.
\end{equation}
The quadratic factor discriminants encode $W(3,3)$:
\begin{alignat}{2}
  \Delta(r\text{-sector}) &= 4-44 = -40 &&= -v,\\
  \Delta(s\text{-sector}) &= 16-44 = -28 &&= -4\Phisix.
\end{alignat}
...
\emph{reduced} fermionic content: projecting out the $\dim(\OO)=8$
octonion directions from the 40-point GQ(3,3) yields the
32-dimensional spinor space of $\SO(10)$ on which $Z(x)$ acts.

\subsection{Relation to published work}

Several independent threads in the literature connect to $W(3,3)$:
\begin{itemize}[nosep]
\item Todorov--Dubois-Violette~(2018): $G_{\mathrm{SM}} = \mathrm{Spin}(9)\cap
  (\SU_3\times\SU_3)/\ZZ_3$ inside $F_4$; our $F_4/G_{\mathrm{SM}}=40=v$.
\item Furey~(2014, 2025): $\Cl(6)\cong\CC\otimes\OO$ gives 3 generations
  of SM fermions; $\dim\Cl(6)=64=2^{2q}$, grade-2 $=15=g$.
\item Furey--Hughes~(2022): cascade $\mathrm{Spin}(10)\to$Pati-Salam$\to$SM
  driven by octonionic complex structures.
\item G\"ursey--G\"unaydin~(1974): $G_2=\Aut(\OO)\supset\SU(3)_{\mathrm{colour}}$
  from the Fano stabiliser.
\item Music~(2026): Koide angle $\theta_0=2/9$ from $G_2$ Casimirs.
\item Manogue--Dray--Wilson~(2022): SM inside $E_{8(-24)}$;
  $\dim(E_8)=248=2^q\cdot(2^{q+\lambda}-1)$.
\item Matsuura--Ohta~(2025): $Z_F=\zeta^{-1}$ on graphs.
\end{itemize}
To our knowledge, no published work derives $\alpha^{-1}=q^4+2q^3+2=137$
from finite geometry, identifies the $F_4$ coset with $\GQ(3,3)$,
or constructs $Z(x)=(1-5x)^{10}(1+x)^{16}(1+7x)^6$ as a
physical generating function.
...
Post-39 & NCG spectral action, K3 closure, Fano--Yukawa,
          positive geometry, zeta--moonshine & $>400$ \\
\midrule
